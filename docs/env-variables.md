...
временная агрегация, сэмплер, экспортеры, пропагаторы) читаются без учета регистра; нераспознанное
значение записывается в лог предупреждением и заменяется значением по умолчанию.
...
| Переменная | По умолчанию | Описание |
|------------|-------------|----------|
| `OTEL_BSP_MAX_QUEUE_SIZE` | `2048` | Максимальный размер очереди, больше 0 |
| `OTEL_BSP_SCHEDULE_DELAY` | `5000` | Интервал запуска экспорта (мс) |
| `OTEL_BSP_MAX_EXPORT_BATCH_SIZE` | `512` | Максимальный размер пакета, больше 0 |
...
> Размер очереди или пакета меньше 1 записывается в лог предупреждением и заменяется значением по умолчанию.
