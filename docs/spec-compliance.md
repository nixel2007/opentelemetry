...
| Нет приватных конструкторов | Спека требует, чтобы `Span` создавался только через `Tracer.StartSpan()` и API не предоставлял иных способов создания | `ПриСозданииОбъекта` в OneScript всегда публичен. Ограничение задокументировано в коде (`src/Трассировка/Классы/ОтелСпан.os`); пользовательский код ОБЯЗАН использовать `Трассировщик.НачатьСпан()`, прямой вызов `Новый ОтелСпан(...)` запрещён. Соответствующие требования помечены `n_a`. |
