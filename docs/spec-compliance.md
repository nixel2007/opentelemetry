...
| ✅ Реализовано (found) | 597 (85.4%) |
| ⚠️ Частично (partial) | 72 (10.3%) |
| ❌ Не реализовано (not_found) | 30 (4.3%) |
| ➖ Неприменимо (n_a) | 8 |
| **MUST/MUST NOT found** | 411/425 (96.7%) |
| **SHOULD/SHOULD NOT found** | 186/274 (67.9%) |
...
| Resource Sdk | 16 | 3 | 1 | 0 | 20 | 80.0% |
| Trace Api | 108 | 13 | 1 | 4 | 122 | 88.5% |
| Trace Sdk | 67 | 13 | 3 | 0 | 83 | 80.7% |
| Logs Api | 19 | 1 | 0 | 1 | 20 | 95.0% |
| Logs Sdk | 54 | 9 | 2 | 0 | 65 | 83.1% |
| Metrics Api | 89 | 10 | 1 | 0 | 100 | 89.0% |
| Metrics Sdk | 133 | 20 | 15 | 3 | 168 | 79.2% |
| Otlp Exporter | 18 | 1 | 6 | 0 | 25 | 72.0% |
| Propagators | 39 | 1 | 0 | 0 | 40 | 97.5% |
...
- ⚠️ **[Resource Sdk]** [MUST] The SDK MUST extract information from the `OTEL_RESOURCE_ATTRIBUTES` environment variable and merge this, as the secondary resource, with any resource information provided by the user, i.e. the user provided resource information has higher priority.  
  OTEL_RESOURCE_ATTRIBUTES извлекается и применяется к ресурсу, но логика приоритета 'user-provided resource > env' отсутствует: автоконфигурация просто накладывает env-атрибуты поверх дефолтных, явный merge с пользовательским ресурсом не реализован. (`src/Конфигурация/Модули/ОтелАвтоконфигурация.os:151`)

- ⚠️ **[Trace Api]** [MUST NOT] However, alternative implementations MUST NOT allow callers to create `Span`s directly.  
  Конструктор класса ОтелСпан публичен (в OneScript иначе нельзя). Тем не менее задокументированный путь создания спана - только через Tracer/SpanBuilder. Архитектурное ограничение платформы. (`src/Трассировка/Классы/ОтелСпан.os:599`)

- ⚠️ **[Trace Api]** [MUST NOT] There MUST NOT be any API for creating a `Span` other than with a `Tracer`.  
  Конструктор ОтелСпан публичен (особенность платформы OneScript - все конструкторы публичны). Документированный/идиоматический путь - только через Трассировщик.ПостроительСпана() / НачатьСпан(). (`src/Трассировка/Классы/ОтелТрассировщик.os:27`)

- ⚠️ **[Trace Api]** [MUST NOT] This operation itself MUST NOT perform blocking I/O on the calling thread.  
  Завершить() сам по себе не делает I/O, но синхронно вызывает Процессор.ПриЗавершении. ОтелПростойПроцессорСпанов.ПриЗавершении выполняет экспорт синхронно (потенциальный блокирующий I/O в HTTP-экспортёре). ОтелПакетныйПроцессорСпанов добавляет в очередь без блокировки (`src/Трассировка/Классы/ОтелСпан.os:466`)
...
- ⚠️ **[Trace Sdk]** [MUST] For backwards compatibility it MUST also be able to access the `InstrumentationLibrary` [deprecated since 1.10.0] having the same name and version values as the `InstrumentationScope`.  
  Spand экспонирует ОбластьИнструментирования (InstrumentationScope), но не имеет отдельного устаревшего геттера InstrumentationLibrary - доступ к данным возможен только через новое имя. (`src/Трассировка/Классы/ОтелСпан.os:182`)

- ❌ **[Trace Sdk]** [MUST] Based on the decision from the wrapped root sampler, `AlwaysRecord` MUST behave as follows:  
  Декоратор AlwaysRecord, преобразующий DROP в RECORD_ONLY от обёрнутого сэмплера, в SDK не реализован. ОтелСэмплер не содержит сущности, оборачивающей другой сэмплер для конвертации решений. (-)

- ⚠️ **[Metrics Api]** [MUST] It MUST be case-sensitive (e.g. `kb` and `kB` are different units), ASCII string.  
  Case-sensitivity обеспечивается платформой OneScript (оператор = для строк регистрозависим), значит требование case-sensitive выполнено. Однако валидация ASCII не реализована: НормализоватьСтроку возвращает значение как есть, без проверки на не-ASCII символы. Поэтому partial. (`src/Метрики/Классы/ОтелМетр.os:621`)

- ⚠️ **[Metrics Api]** [MUST] Multiple-instrument Callbacks MUST be associated at the time of registration with a declared set of asynchronous instruments from the same `Meter` instance.  
  ЗарегистрироватьОбратныйВызов(Callback, НовыеИнструменты) принимает явный набор инструментов, но не валидирует, что все инструменты принадлежат тому же Метру. (`src/Метрики/Классы/ОтелМетр.os:528`)

- ⚠️ **[Metrics Sdk]** [MUST] If the user does not provide a `name` value, name from the Instrument the View matches MUST be used by default.  
  ОтелПредставление поддерживает поле НовоеИмя, но в коде применения View (ПрименитьПредставлениеКИнструменту) переименование инструмента по View не применяется к выходным данным метрики. (`src/Метрики/Классы/ОтелМетр.os:635`)

- ⚠️ **[Metrics Sdk]** [MUST] If the user does not provide a `description` value, the description from the Instrument a View matches MUST be used by default.  
  Поле НовоеОписание есть в ОтелПредставление, но применение НовоеОписание() в выходные данные метрики не реализовано (используется описание инструмента всегда, если View не задаёт - по умолчанию). (`src/Метрики/Классы/ОтелМетр.os:635`)

- ⚠️ **[Metrics Sdk]** [MUST] This MUST be clearly documented in the API and the reservoir MUST be given the `Attributes` associated with its timeseries point either at construction so that additional sampling performed by the rese...  
  АтрибутыСерии передаются в каждом вызове Предложить(), а не на конструировании резервуара; функционально атрибуты доступны, но архитектура отличается от рекомендованной (at construction). (`src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:38`)

- ⚠️ **[Metrics Sdk]** [MUST] MeterProvider - Meter creation, `ForceFlush` and `Shutdown` MUST be safe to be called concurrently.  
  Закрыт реализован через АтомарноеБулево с CAS (СравнитьИУстановить) - Закрыть идемпотентен и безопасен. Однако ПолучитьМетр (Meter creation) и СброситьБуфер обращаются к коллекциям Метрики/ЧитателиМетрик/Представления без явной БлокировкиРесурса - возможны гонки при параллельном создании метров из ФоновыхЗаданий. (`src/Метрики/Классы/ОтелПровайдерМетрик.os:59`)

- ⚠️ **[Metrics Sdk]** [MUST] MetricExporter - `ForceFlush` and `Shutdown` MUST be safe to be called concurrently.  
  Методы СброситьБуфер и Закрыть в ОтелЭкспортерМетрик существуют как простые делегирующие процедуры, но не имеют явной БлокировкиРесурса для защиты от параллельного вызова - в отличие от ОтелПериодическийЧитательМетрик, где блокировка реализована. (`src/Экспорт/Классы/ОтелЭкспортерМетрик.os:58`)
...
- ⚠️ **[Propagators]** [MUST] A W3C Trace Context propagator MUST parse and validate the `traceparent` and `tracestate` HTTP headers as specified in W3C Trace Context Level 2.  
  Парсинг и валидация traceparent и tracestate реализованы (включая ОтелСостояниеТрассировки.Разобрать с проверкой ключей/значений). Однако явного соответствия именно W3C Trace Context Level 2 (например, обработки специфичных Level 2 полей/ошибок) в коде не отражено — реализация ближе к Level 1. (`src/Пропагация/Классы/ОтелW3CПропагатор.os:81`)
...
- ⚠️ **[Baggage Api]** [SHOULD NOT] The functionality listed above is necessary because API users SHOULD NOT have access to the Context Key used by the Baggage API implementation.  
  Функция КлючBaggage() объявлена как Экспорт — ключ контекста, используемый Baggage API, доступен внешним пользователям. Wrapper-функции (BaggageИзКонтекста/КонтекстСBaggage) есть, но сам ключ не инкапсулирован. (`src/Ядро/Модули/ОтелКонтекст.os:53`)
...
- ⚠️ **[Resource Sdk]** [SHOULD] Note the failure to detect any resource information MUST NOT be considered an error, whereas an error that occurs during an attempt to detect resource information SHOULD be considered an error.  
  Детекторы перехватывают все исключения и логируют их через Лог.Отладка (debug-уровень), что не соответствует требованию рассматривать ошибку детекции как ошибку - корректнее использовать Лог.Ошибка/Предупреждение. (`src/Ядро/Классы/ОтелДетекторРесурсаХоста.os:25`)
...
- ⚠️ **[Resource Sdk]** [SHOULD] In case of any error, e.g. failure during the decoding process, the entire environment variable value SHOULD be discarded and an error SHOULD be reported following the Error Handling principles.  
  При ошибке декодирования отдельной пары парсер просто пропускает её (skip-bad-entry), но НЕ отбрасывает целиком значение OTEL_RESOURCE_ATTRIBUTES, как требует спецификация. (`src/Конфигурация/Модули/ОтелАвтоконфигурация.os:742`)
...
- ❌ **[Resource Sdk]** [SHOULD] In case of any error, e.g. failure during the decoding process, the entire environment variable value SHOULD be discarded and an error SHOULD be reported following the Error Handling principles.  
  В РазобратьПарыКлючЗначение нет логирования ошибок при некорректных парах: невалидные элементы молча пропускаются без вызова Лог.Ошибка/Предупреждение. (-)
...
- ⚠️ **[Trace Api]** [SHOULD NOT] The functionality listed above is necessary because API users SHOULD NOT have access to the Context Key used by the Tracing API implementation.  
  Ключ спана хранится в приватной переменной модуля, но публично экспортируется функция КлючСпана() (строка 44), что даёт пользователям прямой доступ к ключу контекста, нарушая SHOULD NOT. (`src/Ядро/Модули/ОтелКонтекст.os:44`)
...
- ⚠️ **[Trace Api]** [SHOULD] The API SHOULD be documented that instrumentation authors needs to call this API each time they create a new `Span` to ensure they have the most up-to-date response.  
  Документирующий комментарий описывает двухрежимную семантику Включен(), но не содержит явной рекомендации инструментирующим авторам вызывать API перед каждым созданием Span для получения актуального ответа. (`src/Трассировка/Классы/ОтелТрассировщик.os:31`)
...
- ⚠️ **[Trace Api]** [SHOULD NOT] To prevent misuse, implementations SHOULD NOT provide access to a `Span`'s attributes besides its `SpanContext`.  
  Метод Атрибуты() публично экспортирует атрибуты спана наружу - это нарушает рекомендацию SHOULD NOT. Аналогично События() и Линки() экспортированы для процессоров/экспортёров, но Атрибуты() читается и любым внешним кодом. (`src/Трассировка/Классы/ОтелСпан.os:146`)

- ⚠️ **[Trace Api]** [SHOULD] An attempt to set value Unset SHOULD be ignored.  
  Нет явной проверки 'если входное Значение = НеУстановлен то выйти'. Из ERROR попытка установить UNSET игнорируется (стр. 436), но из дефолтного UNSET повторная установка UNSET перезапишет СообщениеСтатуса = "" (`src/Трассировка/Классы/ОтелСпан.os:425`)
...
- ⚠️ **[Trace Api]** [SHOULD] When the status is set to Error by Instrumentation Libraries, the Description SHOULD be documented and predictable.  
  Это требование к документации Instrumentation Libraries, а не к самому SDK. SDK API позволяет передавать Сообщение, но конвенций по документации Description в SDK нет (`src/Трассировка/Классы/ОтелСпан.os:425`)

- ⚠️ **[Trace Api]** [SHOULD] For operations not covered by the semantic conventions, Instrumentation Libraries SHOULD publish their own conventions, including possible values of Description and what they mean.  
  Это процессное требование к авторам Instrumentation Libraries, а не к коду SDK. В данном репозитории Instrumentation Libraries отсутствуют (-)
...
- ❌ **[Trace Api]** [SHOULD NOT] If a new type is required for supporting this operation, it SHOULD NOT be exposed publicly if possible (e.g. by only exposing a function that returns something with the Span interface type).  
  Класс ОтелНоопСпан публично экспонирован и используется напрямую через `Новый ОтелНоопСпан(...)` в пропагаторах и трассировщике. Нет отдельной функции-обёртки, скрывающей конкретный тип. (`src/Трассировка/Классы/ОтелНоопСпан.os:1`)

- ⚠️ **[Trace Api]** [SHOULD] If a new type is required to be publicly exposed, it SHOULD be named `NonRecordingSpan`.  
  Тип называется ОтелНоопСпан (Noop), а не NonRecordingSpan. Семантика близкая, но не точное соответствие требуемому имени. (`src/Трассировка/Классы/ОтелНоопСпан.os:1`)
...
- ⚠️ **[Trace Api]** [SHOULD] Implementations SHOULD record links containing `SpanContext` with empty `TraceId` or `SpanId` (all zeros) as long as either the attribute set or `TraceState` is non-empty.  
  ДобавитьЛинк не отвергает контексты с нулевыми ID, но и не реализует явное условие 'attribute set или TraceState non-empty' - принимает любой контекст без проверок. (`src/Трассировка/Классы/ОтелСпан.os:373`)

- ⚠️ **[Trace Api]** [SHOULD] Link - Links are immutable and SHOULD be safe for concurrent use by default.  
  Линки хранятся как Соответствие внутри массива Линки и не имеют отдельного класса с документацией о неизменяемости/потокобезопасности. Соответствие в OneScript изменяемо. (`src/Трассировка/Классы/ОтелСпан.os:384`)
...
- ⚠️ **[Trace Api]** [SHOULD] If the `Span` in the parent `Context` is already non-recording, it SHOULD be returned directly without instantiating a new `Span`.  
  СоздатьНоопСпанВAPIРежиме всегда создаёт новый ОтелНоопСпан, даже если родитель в контексте уже non-recording - не возвращает существующий объект напрямую. (`src/Трассировка/Классы/ОтелТрассировщик.os:226`)

- ⚠️ **[Trace Sdk]** [SHOULD] SDKs SHOULD return a valid no-op Tracer for these calls, if possible.  
  После Закрыть() возвращается новый ОтелТрассировщик без явной Конфигурация (Включен=Ложь). Tracer.Включен() делегирует в Провайдер.Включен(), который остаётся Истина после Закрыть(). В результате возвращённый трассировщик не является валидным no-op (попытается создавать спаны, хотя процессоры уже закрыты). (`src/Трассировка/Классы/ОтелПровайдерТрассировки.os:82`)

- ⚠️ **[Trace Sdk]** [SHOULD] `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  Метод Закрыть() — Процедура (без возвращаемого значения), ошибки только через исключения, таймаут не отличим от успеха. Есть ЗакрытьАсинхронно (возвращает Обещание), но синхронный API не сообщает результат. (`src/Трассировка/Классы/ОтелПровайдерТрассировки.os:132`)

- ⚠️ **[Trace Sdk]** [SHOULD] `Shutdown` SHOULD complete or abort within some timeout.  
  Provider.Закрыть() не принимает параметр таймаута и не передаёт таймаут в КомпозитныйПроцессор.Закрыть(ТаймаутМс) — вызывается с дефолтным 0 (без ограничения). Механизм прерывания по таймауту на уровне Provider отсутствует. (`src/Трассировка/Классы/ОтелПровайдерТрассировки.os:132`)

- ⚠️ **[Trace Sdk]** [SHOULD] `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  Метод СброситьБуфер() — Процедура, ничего не возвращает; ошибки только через исключения, таймаут не отличим от успеха. Есть СброситьБуферАсинхронно (Обещание), но синхронный API не сообщает результат. (`src/Трассировка/Классы/ОтелПровайдерТрассировки.os:122`)

- ⚠️ **[Trace Sdk]** [SHOULD] `ForceFlush` SHOULD complete or abort within some timeout.  
  Provider.СброситьБуфер() не принимает таймаут. На уровне КомпозитныйПроцессор.СброситьБуфер(ТаймаутМс) поддержка таймаута есть, но Provider вызывает процессоры без таймаута, поэтому общего механизма прерывания на уровне ForceFlush нет. (`src/Трассировка/Классы/ОтелПровайдерТрассировки.os:122`)

- ⚠️ **[Trace Sdk]** [SHOULD NOT] However, Span Exporter SHOULD NOT receive them unless the `Sampled` flag was also set.  
  ПриЗавершении передаёт спан экспортёру без проверки флага Sampled - спаны с решением RECORD_ONLY (IsRecording=true, Sampled=false) всё равно попадают в экспортёр. (`src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:42`)
...
- ⚠️ **[Trace Sdk]** [SHOULD NOT] Span Exporters MUST receive those spans which have `Sampled` flag set to true and they SHOULD NOT receive the ones that do not.  
  Процессоры (Simple/Batch) не фильтруют спаны по флагу Sampled при передаче экспортёру: спаны с RECORD_ONLY попадают в экспортёр. (`src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:42`)
...
- ❌ **[Trace Sdk]** [SHOULD] For root span contexts, the SDK SHOULD set the `Random` flag in the trace flags when it generates TraceIDs that meet the W3C Trace Context Level 2 randomness requirements.  
  Бит Random (0x02) во TraceFlags нигде не устанавливается. ОтелКонтекстСпана.ФлагиТрассировки используется только для бита sampled (0x01); ОтелТрассировщик.ВычислитьФлагиТрассировки и СоздатьКонтекстСпана не выставляют 0x02 для корневых спанов. (-)
...
- ⚠️ **[Trace Sdk]** [SHOULD] For all span contexts, OpenTelemetry samplers SHOULD presume that TraceIDs meet the W3C Trace Context Level 2 randomness requirements, unless an explicit randomness value is present in the `rv` sub-key of the OpenTelemetry TraceState.  
  TraceIdRatioBased сэмплер (СэмплироватьПоДоле) использует первые 8 hex-символов TraceID как источник случайности (presumption of randomness), но не учитывает explicit `rv` sub-key из TraceState - если присутствует, должен использоваться именно он. (`src/Трассировка/Модули/ОтелСэмплер.os:277`)
...
- ❌ **[Trace Sdk]** [SHOULD] If the SDK uses an `IdGenerator` extension point, the SDK SHOULD allow the extension to determine whether the Random flag is set when new IDs are generated.  
  IdGenerator extension point есть (УстановитьГенераторИд), но нет механизма, через который генератор сообщает SDK о том, выставлять ли Random flag (W3C Trace Context Level 2). Trace flags при генерации не учитывают свойство генератора. (-)

- ⚠️ **[Trace Sdk]** [SHOULD] The name of the configuration options SHOULD be `EventCountLimit` and `LinkCountLimit`.  
  Опции названы МаксСобытий (=MaxEvents) и МаксЛинков (=MaxLinks), а не EventCountLimit/LinkCountLimit - перевод неточный, в имени отсутствует семантика 'Count'. (`src/Трассировка/Классы/ОтелЛимитыСпана.os:6`)

- ⚠️ **[Trace Sdk]** [SHOULD] `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  Закрыть реализована как Процедура (void), не возвращает статус успеха/неудачи/таймаута. Таймаут логируется, но caller не может отличить успех от таймаута. (`src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:80`)

- ⚠️ **[Trace Sdk]** [SHOULD] `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  СброситьБуфер реализована как Процедура (void), не возвращает статус. Таймаут логируется, но caller не может отличить успех от таймаута. (`src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:71`)

- ⚠️ **[Trace Sdk]** [SHOULD] `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  СброситьБуфер реализована как Процедура без возвращаемого значения; ошибки/таймауты логируются, но вызывающий не получает явный статус (success/failure/timeout) - поведение реализовано через обходной путь. (`src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:71`)
...
- ⚠️ **[Logs Api]** [SHOULD] The API SHOULD be documented that instrumentation authors needs to call this API each time they emit a LogRecord to ensure they have the most up-to-date response.  
  Документирующий комментарий объясняет назначение Включен (пропуск создания LogRecord), но не содержит явного указания инструментирующим авторам вызывать API каждый раз перед эмитом записи, чтобы получать актуальный ответ. (`src/Логирование/Классы/ОтелЛоггер.os:28`)

- ⚠️ **[Logs Sdk]** [SHOULD] `Shutdown` SHOULD complete or abort within some timeout.  
  Метод Закрыть() провайдера не принимает таймаут и не передаёт его процессорам (используется значение по умолчанию 0 = без ограничения). ЗакрытьАсинхронно возвращает Обещание, но без таймаута. (`src/Логирование/Классы/ОтелПровайдерЛогирования.os:132`)

- ⚠️ **[Logs Sdk]** [SHOULD] `ForceFlush` SHOULD return some ERROR status if there is an error condition;  
  СброситьБуферАсинхронно возвращает Обещание, ошибки могут проявиться через rejected Promise, но синхронный метод СброситьБуфер - процедура без явного статуса ошибки. (`src/Логирование/Классы/ОтелПровайдерЛогирования.os:148`)

- ⚠️ **[Logs Sdk]** [SHOULD] and if there is no error condition, it SHOULD return some NO ERROR status, language implementations MAY decide how to model ERROR and NO ERROR.  
  Только асинхронный вариант возвращает Обещание; синхронный СброситьБуфер - процедура (void), без явного NO ERROR статуса. (`src/Логирование/Классы/ОтелПровайдерЛогирования.os:148`)

- ⚠️ **[Logs Sdk]** [SHOULD] `ForceFlush` SHOULD complete or abort within some timeout.  
  Метод СброситьБуфер провайдера не принимает параметр таймаута и не передаёт его процессорам. Async-вариант также без таймаута. (`src/Логирование/Классы/ОтелПровайдерЛогирования.os:122`)
...
- ❌ **[Logs Sdk]** [SHOULD] To avoid such race conditions, implementations SHOULD recommended to users that a clone of `logRecord` be used for any concurrent processing, such as in a batching processor.  
  В документации API (комментариях ПриПоявлении) нет рекомендации пользователям клонировать ЗаписьЛога для конкурентной обработки в batching-процессоре. (-)
...
- ⚠️ **[Logs Sdk]** [SHOULD] After the call to `Shutdown`, subsequent calls to `OnEmit` are not allowed. SDKs SHOULD ignore these calls gracefully, if possible.  
  ОтелБазовыйПакетныйПроцессор.Обработать проверяет флаг Закрыт и игнорирует вызовы; в ОтелПростойПроцессорЛогов нет проверки состояния после Закрыть(), последующие вызовы ПриПоявлении приведут к ошибке экспортера (исключение поглощается композитом, но не gracefully на уровне процессора). (`src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:43`)

- ⚠️ **[Logs Sdk]** [SHOULD] `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  Закрыть() — процедура без возвращаемого значения; ЗакрытьАсинхронно() возвращает Обещание, но статус успеха/таймаута/ошибки не различается; при таймауте в композите процессор просто пропускается без сигнализации вызывающему. (`src/Логирование/Классы/ОтелПровайдерЛогирования.os:160`)

- ⚠️ **[Logs Sdk]** [SHOULD] `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  СброситьБуфер() — процедура без возврата; СброситьБуферАсинхронно() возвращает Обещание, но не различает статусы успеха/таймаута/ошибки; в композите при таймауте процессор пропускается без явной сигнализации вызывающему. (`src/Логирование/Классы/ОтелПровайдерЛогирования.os:148`)

- ❌ **[Logs Sdk]** [SHOULD] Additional processors defined in this document SHOULD be provided by SDK packages.  
  Дополнительный процессор Event-to-span-event-bridge, описанный в этой же спецификации, в SDK не реализован. (-)

- ⚠️ **[Logs Sdk]** [SHOULD] `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  СброситьБуфер реализован как Процедура без возвращаемого значения; статус успеха/таймаута не возвращается. На уровне провайдера есть СброситьБуферАсинхронно (Обещание), но у самого LogRecordExporter возврата статуса нет. (`src/Экспорт/Классы/ОтелЭкспортерЛогов.os:58`)

- ⚠️ **[Logs Sdk]** [SHOULD] `ForceFlush` SHOULD complete or abort within some timeout.  
  СброситьБуфер у экспортера не принимает параметра таймаута и не имеет механизма прерывания; синхронный экспортер без буфера возвращается сразу, но явного таймаута/прерывания нет. (`src/Экспорт/Классы/ОтелЭкспортерЛогов.os:58`)
...
- ⚠️ **[Metrics Api]** [SHOULD] The API SHOULD be documented in a way to communicate to users that the `name` parameter needs to conform to the instrument name syntax.  
  Документирующий комментарий к СоздатьСчетчик описывает параметр как 'имя счетчика', но не ссылается на правила instrument name syntax. (`src/Метрики/Классы/ОтелМетр.os:40`)

- ⚠️ **[Metrics Api]** [SHOULD NOT] The API SHOULD NOT validate `advisory` parameters.  
  ПроверитьСовет логирует предупреждения при несовпадении типов в advisory. Это мягкая валидация (warn-only), не отбрасывает значения, но формально проверяет структуру. (`src/Метрики/Классы/ОтелМетр.os:796`)

- ⚠️ **[Metrics Api]** [SHOULD] The API SHOULD be documented in a way to communicate to users that the `name` parameter needs to conform to the instrument name syntax.  
  Комментарий описывает 'имя инструмента', но не ссылается на instrument name syntax. (`src/Метрики/Классы/ОтелМетр.os:269`)

- ⚠️ **[Metrics Api]** [SHOULD NOT] The API SHOULD NOT validate `advisory` parameters.  
  ПроверитьСовет логирует предупреждения при несовпадении типов в advisory (warn-only). (`src/Метрики/Классы/ОтелМетр.os:796`)

- ⚠️ **[Metrics Api]** [SHOULD] The API SHOULD provide some way to pass `state` to the callback.  
  Callback получает только объект ОтелНаблюдениеМетрики. Отдельного механизма передачи user state нет; пользователь может использовать замыкание lambda. (`src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:171`)
...
- ⚠️ **[Metrics Api]** [SHOULD] The API SHOULD be documented that instrumentation authors needs to call this API each time they record a measurement to ensure they have the most up-to-date response.  
  Комментарий описывает поведение метода и упоминает MUST/спеку, но не указывает явно, что API нужно вызывать перед каждой записью измерения для актуальности результата. (`src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:248`)

- ⚠️ **[Metrics Api]** [SHOULD NOT] This API SHOULD NOT validate this value, that is left to implementations of the API.  
  Counter.Добавить выполняет валидацию (отрицательные значения молча игнорируются: 'Если Значение < 0 Тогда Возврат'), вместо того чтобы делегировать это SDK. (`src/Метрики/Классы/ОтелСчетчик.os:22`)
...
- ⚠️ **[Metrics Api]** [SHOULD] This API SHOULD be documented in a way to communicate to users that this value is expected to be non-negative.  
  Документирующий комментарий перед Записать описывает параметр Значение, но не упоминает требование неотрицательности (non-negative). (`src/Метрики/Классы/ОтелГистограмма.os:13`)

- ❌ **[Metrics Api]** [SHOULD] The API to register a new Callback SHOULD accept: * A `callback` function* A list (or tuple, etc.) of Instruments used in the `callback` function.  
  Multiple-instrument callbacks являются опциональной фичей (MAY support); SDK не предоставляет API регистрации одного callback на список инструментов — callback задаётся индивидуально на каждый наблюдаемый инструмент через СоздатьНаблюдаемый*/Callback. Отдельного RegisterCallback с (callback, [instruments]) нет. (-)

- ⚠️ **[Metrics Sdk]** [SHOULD] `Shutdown` SHOULD complete or abort within some timeout.  
  Закрыть/ЗакрытьАсинхронно не принимают параметр таймаута и не имеют механизма прерывания операции. Асинхронная обёртка через Обещание есть, но без timeout/abort. (`src/Метрики/Классы/ОтелПровайдерМетрик.os:143`)

- ⚠️ **[Metrics Sdk]** [SHOULD] `ForceFlush` SHOULD return some ERROR status if there is an error condition; and if there is no error condition, it should return some NO ERROR status, language implemen...  
  СброситьБуфер - процедура без возвращаемого статуса; ошибки/успех модулируются только через Обещание в СброситьБуферАсинхронно. Явной модели ERROR/NO ERROR нет. (`src/Метрики/Классы/ОтелПровайдерМетрик.os:167`)

- ⚠️ **[Metrics Sdk]** [SHOULD] `ForceFlush` SHOULD complete or abort within some timeout.  
  СброситьБуфер/СброситьБуферАсинхронно не принимают параметр таймаута и не имеют механизма прерывания операции по истечении времени. (`src/Метрики/Классы/ОтелПровайдерМетрик.os:128`)
...
- ❌ **[Metrics Sdk]** [SHOULD] In order to avoid conflicts, if a `name` is provided the View SHOULD have an instrument selector that selects at most one instrument.  
  Нет проверки, что при заданном имени потока селектор отбирает не более одного инструмента; первый найденный View применяется без проверки конфликта. (-)
...
- ⚠️ **[Metrics Sdk]** [SHOULD] If applying the View results in conflicting metric identities the implementation SHOULD apply the View and emit a warning.  
  Конфликт дескрипторов инструмента (одинаковое имя, разные параметры) логируется через Лог.Предупреждение, но конфликт идентичностей метрик из-за применения нескольких Views не детектируется (НайтиПредставление возвращает только первое совпадение). (`src/Метрики/Классы/ОтелМетр.os:707`)
...
- ❌ **[Metrics Sdk]** [SHOULD] If it is not possible to apply the View without producing semantic errors (e.g. the View sets an asynchronous instrument to use the Explicit bucket histogram aggregation) the implementation SHOULD emit a warning and proceed as if the View did not  
  Нет валидации семантической совместимости View с инструментом (например, Explicit bucket histogram для async-инструмента) и нет соответствующего предупреждения с откатом к поведению без View. (-)
...
- ⚠️ **[Metrics Sdk]** [SHOULD NOT] Arithmetic sum of `Measurement` values in population. This SHOULD NOT be collected when used with instruments that record negative measurements (e.g. `UpDownCounter` or `ObservableGauge`).  
  Сумма всегда собирается без проверки типа инструмента; нет защиты от использования гистограммы с инструментами, регистрирующими отрицательные значения (UpDownCounter/ObservableGauge). (`src/Метрики/Классы/ОтелАгрегаторГистограммы.os:51`)

- ❌ **[Metrics Sdk]** [SHOULD] The implementation SHOULD use a timeout to prevent indefinite callback execution.  
  В ВызватьCallbackИСобрать (ОтелБазовыйНаблюдаемыйИнструмент) и в ВызватьМультиОбратныеВызовы (ОтелМетр) callback вызывается синхронно через Действие.Выполнить без ограничения времени; ошибки ловятся, но таймаут не реализован. (-)
...
- ⚠️ **[Metrics Sdk]** [SHOULD] If there is no matching view, but the `MetricReader` defines a default cardinality limit value based on the instrument an aggregation is created for, that value SHOULD be used.  
  MetricReader предоставляет один общий ЛимитМощности (по умолчанию 2000), не дифференцированный по типу инструмента. Метр применяет его ко всем инструментам через ПрименитьНастройкиЧитателяКМетру. (`src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:151`)
...
- ❌ **[Metrics Sdk]** [SHOULD] Aggregators of asynchronous instruments SHOULD prefer the first-observed attributes in the callback when limiting cardinality, regardless of temporality.  
  В ОтелБазовыйНаблюдаемыйИнструмент.ВызватьCallbackИСобрать/ПреобразоватьЗаписиВТочки нет логики ограничения мощности и нет приоритизации первых наблюдаемых атрибутов; cardinality limit для асинхронных инструментов не применяется. (-)
...
- ⚠️ **[Metrics Sdk]** [SHOULD] The emitted warning SHOULD include information for the user on how to resolve the conflict, if possible.  
  Предупреждение содержит описание различий (вид/единица/описание), но не предлагает конкретного способа разрешения (например, использовать View для переименования). (`src/Метрики/Классы/ОтелМетр.os:727`)
...
- ❌ **[Metrics Sdk]** [SHOULD] If the potential conflict involves multiple `description` properties, setting the `description` through a configured View SHOULD avoid the warning.  
  ПроверитьКонфликтДескриптора сравнивает Описание напрямую и не учитывает, что View мог переопределить description; предупреждение выводится даже если конфликт может быть устранён View. (-)

- ❌ **[Metrics Sdk]** [SHOULD] If the potential conflict involves instruments that can be distinguished by a supported View selector (e.g. name, instrument kind) a renaming View recipe SHOULD be included in the...  
  Текст предупреждения статичен и не содержит рецепта переименования через View. (-)

- ❌ **[Metrics Sdk]** [SHOULD] Otherwise (e.g., use of multiple units), the SDK SHOULD pass through the data by reporting both `Metric` objects and emit a generic warning describing the duplicate instrument re...  
  СоздатьСчетчик/Гистограмму/Датчик возвращает существующий инструмент при совпадении нормализованного имени; нет механизма создания и экспорта обоих Metric-объектов с разными units. (-)
...
- ❌ **[Metrics Sdk]** [SHOULD] When a Meter creates an instrument, it SHOULD validate the instrument name conforms to the instrument name syntax  
  В ОтелМетр.СоздатьСчетчик/СоздатьГистограмму/СоздатьДатчик и др. отсутствует проверка имени инструмента на соответствие синтаксису (длина, допустимые символы, начальный символ). Имя только нормализуется через НРег(Имя). (-)
...
- ❌ **[Metrics Sdk]** [SHOULD] If the instrument name does not conform to this syntax, the Meter SHOULD emit an error notifying the user about the invalid name.  
  Так как валидация имени отсутствует, ошибка о невалидном имени не эмитится. Лог предупреждений вызывается только при конфликте дескрипторов, но не при нарушении синтаксиса имени. (-)
...
- ⚠️ **[Metrics Sdk]** [SHOULD] If an advisory parameter is not valid, the Meter SHOULD emit an error notifying the user and proceed as if the parameter was not provided.  
  ПроверитьСовет логирует Предупреждение о неправильном типе параметра, но не сбрасывает невалидный параметр - дальнейший код (ОпределитьГраницыГистограммы, ПолучитьГраницыИзСовета) всё равно использует исходный Совет, не действуя 'как если бы параметр не был задан'. (`src/Метрики/Классы/ОтелМетр.os:796-815`)

- ⚠️ **[Metrics Sdk]** [SHOULD] The “offer” method SHOULD accept measurements, including:  
  Предложить принимает Значение, АтрибутыИзмерения и КонтекстСпана, но timestamp генерируется внутри (не передаётся параметром), а полный Context (с Baggage) не принимается. (`src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:38`)

- ⚠️ **[Metrics Sdk]** [SHOULD] The `ExemplarReservoir` SHOULD avoid allocations when sampling exemplars.  
  Резервуар выделяет новое Соответствие на каждый сэмплированный экземпляр в СоздатьЭкземпляр; нет пула или переиспользования объектов. (`src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:117`)

- ⚠️ **[Metrics Sdk]** [SHOULD] `Collect` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  СброситьБуфер - процедура без возврата статуса; ошибки экспорта только логируются. Нет способа вызывающему отличить успех/ошибку/таймаут. (`src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:94`)

- ⚠️ **[Metrics Sdk]** [SHOULD] SDKs SHOULD return some failure for these calls, if possible.  
  СброситьБуфер - Процедура без возвращаемого значения; после Закрыть подкласс Экспортер вернёт Ложь, но читатель не пробрасывает результат вызывающей стороне. (`src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:94`)

- ⚠️ **[Metrics Sdk]** [SHOULD] `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  Закрыть - Процедура без возвращаемого значения; таймаут ожидания фонового задания логируется, но статус успех/неудача/таймаут не возвращается вызывающей стороне. (`src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:111`)
...
- ⚠️ **[Metrics Sdk]** [SHOULD] `ForceFlush` SHOULD collect metrics, split into batches if necessary, call `Export(batch)` on each batch and `ForceFlush()` on the configured Push Metric Exporter.  
  СброситьБуфер вызывает СобратьИЭкспортировать (что вызывает Экспортер.Экспортировать), но НЕ вызывает Экспортер.СброситьБуфер() на push-экспортере; разбиение на батчи отсутствует. (`src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:94`)
...
- ❌ **[Metrics Sdk]** [SHOULD] `ForceFlush` MAY skip `Export(batch)` calls if the timeout is already expired, but SHOULD still call `ForceFlush()` on the configured Push Metric Exporter even if the timeout has passed.  
  Нет вызова Экспортер.СброситьБуфер() из СброситьБуфер читателя; механизма таймаута также нет. (-)

- ❌ **[Metrics Sdk]** [SHOULD] `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  СброситьБуфер - Процедура без возвращаемого значения; невозможно отличить успех от неудачи/таймаута. (-)

- ❌ **[Metrics Sdk]** [SHOULD] `ForceFlush` SHOULD return some ERROR status if there is an error condition; and if there is no error condition, it should return some NO ERROR status, language implementations MAY decide how to model...  
  СброситьБуфер не возвращает значение; статус ошибки/успеха недоступен. (-)

- ❌ **[Metrics Sdk]** [SHOULD] `ForceFlush` SHOULD complete or abort within some timeout.  
  СброситьБуфер не принимает таймаут и не прерывает операцию по таймауту. (-)

- ⚠️ **[Metrics Sdk]** [SHOULD] Metric Exporters SHOULD report an error condition for data output by the `MetricReader` with unsupported Aggregation or Aggregation Temporality, as this condition can be corrected by a change of `Metr...  
  СформироватьМетрикуOtlp молча пропускает неизвестные ТипМетрики (нет ветки else с ошибкой); явного сигнала об unsupported Aggregation/Temporality в МетricReader не происходит. (`src/Экспорт/Классы/ОтелЭкспортерМетрик.os:250`)

- ❌ **[Metrics Sdk]** [SHOULD] `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  Экспортер.СброситьБуфер - Процедура без возвращаемого значения; статус succeed/failed/timed out не сообщается. (-)
...
- ❌ **[Metrics Sdk]** [SHOULD] `MetricProducer` implementations SHOULD accept configuration for the `AggregationTemporality` of produced metrics.  
  ИнтерфейсПродюсерМетрик.Произвести(ФильтрМетрик) не принимает конфигурацию AggregationTemporality. (-)
...
- ❌ **[Otlp Exporter]** [SHOULD] However, if they are already implemented, they SHOULD continue to be supported as they were part of a stable release of the specification.  
  Устаревшие переменные OTEL_EXPORTER_OTLP_SPAN_INSECURE и OTEL_EXPORTER_OTLP_METRIC_INSECURE не обрабатываются автоконфигурацией; используются только канонические OTEL_EXPORTER_OTLP_INSECURE/TRACES/LOGS/METRICS_INSECURE. (-)
...
- ❌ **[Otlp Exporter]** [SHOULD] The default protocol SHOULD be `http/protobuf`, unless there are strong reasons for SDKs to select `grpc` as the default.  
  Значение по умолчанию для otel.exporter.otlp.protocol = "http/json" (см. ПараметрИлиУмолчание и ПараметрСигналаИлиОбщий, строки 197 и 610), а не "http/protobuf". (`src/Конфигурация/Модули/ОтелАвтоконфигурация.os:197`)

- ⚠️ **[Otlp Exporter]** [SHOULD] SDKs SHOULD support both `grpc` and `http/protobuf` transports and MUST support at least one of them.  
  grpc реализован полностью (ОтелGrpcТранспорт). Однако "http/protobuf" принимается как значение настройки, но ОтелHttpТранспорт всегда сериализует тело как JSON (Content-Type: application/json), то есть фактической поддержки protobuf-кодирования по HTTP нет. (`src/Конфигурация/Модули/ОтелАвтоконфигурация.os:612`)
...
- ❌ **[Otlp Exporter]** [SHOULD] If no configuration is provided the default transport SHOULD be `http/protobuf` unless SDKs have good reasons to choose `grpc` as the default.  
  Значение по умолчанию для otel.exporter.otlp.protocol = "http/json" (ПараметрСигналаИлиОбщий с дефолтом "http/json"), а не "http/protobuf". (`src/Конфигурация/Модули/ОтелАвтоконфигурация.os:610`)
...
- ❌ **[Otlp Exporter]** [SHOULD] OpenTelemetry protocol exporters SHOULD emit a User-Agent header to at a minimum identify the exporter, the language of its implementation, and the version of the exporter.  
  В ОтелHttpТранспорт.ПриСозданииОбъекта устанавливаются только Content-Type и (при сжатии) Content-Encoding. Заголовок User-Agent не формируется и не отправляется. ОтелGrpcТранспорт также не передаёт user-agent метаданные. (-)

- ❌ **[Otlp Exporter]** [SHOULD] The format of the header SHOULD follow RFC 7231.  
  Заголовок User-Agent не формируется (см. предыдущее требование), поэтому соответствие RFC 7231 не реализовано. (-)

- ❌ **[Otlp Exporter]** [SHOULD] The resulting User-Agent SHOULD include the exporter's default User-Agent string.  
  Нет ни дефолтной строки User-Agent, ни конфигурационной опции для добавления product identifier. (-)
...
- ❌ **[Env Vars]** [SHOULD] If any value other than a true value, case-insensitive string "false", empty, or unset is used, a warning SHOULD be logged to inform users about the fallback to false being applied.  
  Парсинг булевых значений (например, otel.sdk.disabled, insecure) выполняется выражением НРег(Значение) = "true" без отдельной проверки на распознанные значения {true, false, empty, unset} и без логирования предупреждения для нераспознанных значений (например, "yes", "1"). (-)
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 11 | SHOULD | ✅ found | These operations SHOULD only be used to implement automatic scope switching and define higher level APIs by SDK components and OpenTelemetry instrumentation libraries. | `src/Ядро/Модули/ОтелКонтекст.os:246` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | Each name in `Baggage` MUST be associated with exactly one value. | `src/Ядро/Классы/ОтелBaggage.os:155` |  |
...
| 3 | MUST | ✅ found | Language API MUST accept any valid UTF-8 string as baggage value in `Set` and return the same value from `Get`. | `src/Ядро/Классы/ОтелBaggage.os:42` |  |
| 4 | MUST | ✅ found | Language API MUST treat both baggage names and values as case sensitive. | `src/Ядро/Классы/ОтелПостроительBaggage.os:23` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 7 | MUST | ✅ found | To access the value for a name/value pair set by a prior event, the Baggage API MUST provide a function that takes the name as input, and returns a value associated with the given name, or null if t... | `src/Ядро/Классы/ОтелBaggage.os:42` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 8 | MUST NOT | ✅ found | The order of name/value pairs MUST NOT be significant. | `src/Ядро/Классы/ОтелBaggage.os:104` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 9 | MUST | ✅ found | To record the value for a name/value pair, the Baggage API MUST provide a function which takes a name, and a value as input. | `src/Ядро/Классы/ОтелBaggage.os:74` |  |
...
| 12 | SHOULD NOT | ⚠️ partial | The functionality listed above is necessary because API users SHOULD NOT have access to the Context Key used by the Baggage API implementation. | `src/Ядро/Модули/ОтелКонтекст.os:53` | Функция КлючBaggage() объявлена как Экспорт — ключ контекста, используемый Baggage API, доступен внешним пользователям. Wrapper-функции (BaggageИзКонтекста/КонтекстСBaggage) есть, но сам ключ не инкапсулирован. |
...
| 2 | MUST | ✅ found | When associated with a `TracerProvider`, all `Span`s produced by any `Tracer` from the provider MUST be associated with this `Resource`. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:116` |  |
...
| 4 | MUST | ✅ found | This resource MUST be associated with a `TracerProvider` or `MeterProvider` if another resource was not explicitly specified. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:382` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 5 | MUST | ✅ found | The interface MUST provide a way to create a new resource, from `Attributes`. | `src/Ядро/Классы/ОтелПостроительРесурса.os:19` |  |
...
| 14 | SHOULD | ✅ found | Empty Schema URL SHOULD be used if the detector does not populate the resource with any known attributes that have a semantic convention or if the detector does not know what attributes it will populate (e.g. the detector that reads... | `src/Ядро/Классы/ОтелРесурс.os:99` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 16 | MUST | ⚠️ partial | The SDK MUST extract information from the `OTEL_RESOURCE_ATTRIBUTES` environment variable and merge this, as the secondary resource, with any resource information provided by the user, i.e. the user provided resource information has higher priority. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:151` | OTEL_RESOURCE_ATTRIBUTES извлекается и применяется к ресурсу, но логика приоритета 'user-provided resource > env' отсутствует: автоконфигурация просто накладывает env-атрибуты поверх дефолтных, явный merge с пользовательским ресурсом не реализован. |
| 17 | MUST | ✅ found | All attribute values MUST be considered strings. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:756` |  |
...
| 19 | SHOULD | ⚠️ partial | In case of any error, e.g. failure during the decoding process, the entire environment variable value SHOULD be discarded and an error SHOULD be reported following the Error Handling principles. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:742` | При ошибке декодирования отдельной пары парсер просто пропускает её (skip-bad-entry), но НЕ отбрасывает целиком значение OTEL_RESOURCE_ATTRIBUTES, как требует спецификация. |
| 20 | SHOULD | ❌ not_found | In case of any error, e.g. failure during the decoding process, the entire environment variable value SHOULD be discarded and an error SHOULD be reported following the Error Handling principles. | - | В РазобратьПарыКлючЗначение нет логирования ошибок при некорректных парах: невалидные элементы молча пропускаются без вызова Лог.Ошибка/Предупреждение. |
...
| 5 | SHOULD | ✅ found | This name SHOULD uniquely identify the instrumentation scope, such as the instrumentation library (e.g. `io.opentelemetry.contrib.mongodb`), package, module or class name. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:48` |  |
| 6 | MUST | ✅ found | In case an invalid name (null or empty string) is specified, a working Tracer implementation MUST be returned as a fallback rather than returning null or throwing an exception, its `name` property SHOULD be ... | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:74` |  |
| 7 | SHOULD | ✅ found | In case an invalid name (null or empty string) is specified, a working Tracer implementation MUST be returned as a fallback rather than returning null or throwing an exception, its `name` property SHOULD be ... | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:76` |  |
| 8 | SHOULD | ✅ found | In case an invalid name (null or empty string) is specified, a working Tracer implementation MUST be returned as a fallback rather than returning null or throwing an exception, its `name` property SHOULD be ... | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:75` |  |
| 9 | MUST NOT | ✅ found | Implementations MUST NOT require users to repeatedly obtain a `Tracer` again with the same identity to pick up configuration changes. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:86` |  |
...
| 19 | SHOULD | ⚠️ partial | The API SHOULD be documented that instrumentation authors needs to call this API each time they create a new `Span` to ensure they have the most up-to-date response. | `src/Трассировка/Классы/ОтелТрассировщик.os:31` | Документирующий комментарий описывает двухрежимную семантику Включен(), но не содержит явной рекомендации инструментирующим авторам вызывать API перед каждым созданием Span для получения актуального ответа. |
...
| 36 | MUST | ✅ found | All mutating operations MUST return a new `TraceState` with the modifications applied. | `src/Трассировка/Классы/ОтелСостояниеТрассировки.os:111` |  |
| 37 | MUST | ✅ found | `TraceState` MUST at all times be valid according to rules specified in W3C Trace Context specification. | `src/Трассировка/Классы/ОтелСостояниеТрассировки.os:364` |  |
...
| 43 | SHOULD | ✅ found | A `Span`'s start time SHOULD be set to the current time on span creation. | `src/Трассировка/Классы/ОтелСпан.os:632` |  |
| 44 | SHOULD | ✅ found | After the `Span` is created, it SHOULD be possible to change its name, set its `Attribute`s, add `Event`s, and set the `Status`. | `src/Трассировка/Классы/ОтелСпан.os:259` |  |
| 45 | MUST NOT | ✅ found | These MUST NOT be changed after the `Span`'s end time has been set. | `src/Трассировка/Классы/ОтелСпан.os:260` |  |
| 46 | SHOULD NOT | ⚠️ partial | To prevent misuse, implementations SHOULD NOT provide access to a `Span`'s attributes besides its `SpanContext`. | `src/Трассировка/Классы/ОтелСпан.os:146` | Метод Атрибуты() публично экспортирует атрибуты спана наружу - это нарушает рекомендацию SHOULD NOT. Аналогично События() и Линки() экспортированы для процессоров/экспортёров, но Атрибуты() читается и любым внешним кодом. |
| 47 | MUST NOT | ⚠️ partial | However, alternative implementations MUST NOT allow callers to create `Span`s directly. | `src/Трассировка/Классы/ОтелСпан.os:599` | Конструктор класса ОтелСпан публичен (в OneScript иначе нельзя). Тем не менее задокументированный путь создания спана - только через Tracer/SpanBuilder. Архитектурное ограничение платформы. |
| 48 | MUST | ✅ found | All `Span`s MUST be created via a `Tracer`. | `src/Трассировка/Классы/ОтелТрассировщик.os:65` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 49 | MUST NOT | ⚠️ partial | There MUST NOT be any API for creating a `Span` other than with a `Tracer`. | `src/Трассировка/Классы/ОтелТрассировщик.os:27` | Конструктор ОтелСпан публичен (особенность платформы OneScript - все конструкторы публичны). Документированный/идиоматический путь - только через Трассировщик.ПостроительСпана() / НачатьСпан(). |
| 50 | MUST NOT | ✅ found | In languages with implicit `Context` propagation, `Span` creation MUST NOT set the newly created `Span` as the active `Span` in the current `Context` by default, but this functionality MAY be offered ... | `src/Трассировка/Классы/ОтелТрассировщик.os:65` |  |
...
| 53 | MUST | ✅ found | The semantic parent of the Span MUST be determined according to the rules described in Determining the Parent Span from a Context. | `src/Ядро/Модули/ОтелКонтекст.os:141` |  |
| 54 | MUST | ✅ found | The API documentation MUST state that adding attributes at span creation is preferred to calling `SetAttribute` later, as samplers can only consider information already present during span ... | `src/Трассировка/Классы/ОтелПостроительСпана.os:71` |  |
...
| 61 | MUST | ✅ found | Any span that is created MUST also be ended. | `src/Трассировка/Классы/ОтелСпан.os:459` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 65 | SHOULD | ✅ found | After a Span is ended, it SHOULD become non-recording and IsRecording SHOULD always return false. | `src/Трассировка/Классы/ОтелСпан.os:246` |  |
| 66 | SHOULD | ✅ found | After a Span is ended, it SHOULD become non-recording and IsRecording SHOULD always return false. | `src/Трассировка/Классы/ОтелСпан.os:246` |  |
| 67 | SHOULD NOT | ✅ found | IsRecording SHOULD NOT take any parameters. | `src/Трассировка/Классы/ОтелСпан.os:246` |  |
| 68 | SHOULD | ✅ found | This flag SHOULD be used to avoid expensive computations of a Span attributes or events in case when a Span is definitely not recorded. | `src/Трассировка/Классы/ОтелСпан.os:246` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 69 | MUST | ✅ found | A Span MUST have the ability to set Attributes associated with it. | `src/Трассировка/Классы/ОтелСпан.os:275` |  |
| 70 | MUST | ✅ found | The Span interface MUST provide: An API to set a single Attribute where the attribute properties are passed as arguments. | `src/Трассировка/Классы/ОтелСпан.os:275` |  |
| 71 | SHOULD | ✅ found | Setting an attribute with the same key as an existing attribute SHOULD overwrite the existing attribute's value. | `src/Трассировка/Классы/ОтелСпан.os:290` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 72 | MUST | ✅ found | A Span MUST have the ability to add events. | `src/Трассировка/Классы/ОтелСпан.os:305` |  |
| 73 | MUST | ✅ found | The Span interface MUST provide: An API to record a single Event where the Event properties are passed as arguments. | `src/Трассировка/Классы/ОтелСпан.os:305` |  |
| 74 | SHOULD | ✅ found | Events SHOULD preserve the order in which they are recorded. | `src/Трассировка/Классы/ОтелСпан.os:305` | События хранятся в Массиве, добавляются через .Добавить() — порядок сохраняется |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 75 | MUST | ✅ found | A Span MUST have the ability to add Links associated with it after its creation - see Links. | `src/Трассировка/Классы/ОтелСпан.os:373` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 76 | MUST | ✅ found | Description MUST only be used with the Error StatusCode value. | `src/Трассировка/Классы/ОтелСпан.os:441` |  |
| 77 | MUST | ✅ found | The Span interface MUST provide: An API to set the Status. | `src/Трассировка/Классы/ОтелСпан.os:425` |  |
| 78 | SHOULD | ✅ found | An API to set the Status. This SHOULD be called SetStatus. | `src/Трассировка/Классы/ОтелСпан.os:425` |  |
| 79 | MUST | ✅ found | Description MUST be IGNORED for StatusCode Ok & Unset values. | `src/Трассировка/Классы/ОтелСпан.os:441` |  |
| 80 | SHOULD | ✅ found | The status code SHOULD remain unset, except for the following circumstances: | `src/Трассировка/Классы/ОтелСпан.os:425` | Дефолт КодСтатуса = НеУстановлен (Unset), изменяется только явным вызовом УстановитьСтатус |
| 81 | SHOULD | ⚠️ partial | An attempt to set value Unset SHOULD be ignored. | `src/Трассировка/Классы/ОтелСпан.os:425` | Нет явной проверки 'если входное Значение = НеУстановлен то выйти'. Из ERROR попытка установить UNSET игнорируется (стр. 436), но из дефолтного UNSET повторная установка UNSET перезапишет СообщениеСтатуса = "" |
| 82 | SHOULD | ⚠️ partial | When the status is set to Error by Instrumentation Libraries, the Description SHOULD be documented and predictable. | `src/Трассировка/Классы/ОтелСпан.os:425` | Это требование к документации Instrumentation Libraries, а не к самому SDK. SDK API позволяет передавать Сообщение, но конвенций по документации Description в SDK нет |
| 83 | SHOULD | ⚠️ partial | For operations not covered by the semantic conventions, Instrumentation Libraries SHOULD publish their own conventions, including possible values of Description and what they mean. | - | Это процессное требование к авторам Instrumentation Libraries, а не к коду SDK. В данном репозитории Instrumentation Libraries отсутствуют |
| 84 | SHOULD NOT | ➖ n_a | Generally, Instrumentation Libraries SHOULD NOT set the status code to Ok, unless explicitly configured to do so. | - | Требование к Instrumentation Libraries, а не к Trace API SDK. В репозитории нет Instrumentation Libraries |
| 85 | SHOULD | ➖ n_a | Instrumentation Libraries SHOULD leave the status code as Unset unless there is an error, as described above. | - | Требование к Instrumentation Libraries, а не к Trace API SDK. В репозитории нет Instrumentation Libraries |
| 86 | SHOULD | ✅ found | When span status is set to Ok it SHOULD be considered final and any further attempts to change it SHOULD be ignored. | `src/Трассировка/Классы/ОтелСпан.os:431` |  |
| 87 | SHOULD | ✅ found | When span status is set to Ok it SHOULD be considered final and any further attempts to change it SHOULD be ignored. | `src/Трассировка/Классы/ОтелСпан.os:431` |  |
| 88 | SHOULD | ➖ n_a | Analysis tools SHOULD respond to an Ok status by suppressing any errors they would otherwise generate. | - | Требование относится к инструментам анализа (бэкендам), а не к SDK |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 89 | SHOULD | ✅ found | Implementations SHOULD ignore all subsequent calls to End and any other Span methods, i.e. the Span becomes non-recording by being ended | `src/Трассировка/Классы/ОтелСпан.os:459` |  |
| 90 | MUST | ✅ found | However, all API implementations of such methods MUST internally call the End method and be documented to do so. | `src/Трассировка/Классы/ОтелСпан.os:459` | Альтернативные методы завершения отсутствуют — единственный способ завершить спан это Завершить() |
| 91 | MUST NOT | ✅ found | End MUST NOT have any effects on child spans. | `src/Трассировка/Классы/ОтелСпан.os:459` |  |
| 92 | MUST NOT | ✅ found | End MUST NOT inactivate the Span in any Context it is active in. | `src/Трассировка/Классы/ОтелСпан.os:459` |  |
| 93 | MUST | ✅ found | It MUST still be possible to use an ended span as parent via a Context it is contained in. | `src/Трассировка/Классы/ОтелСпан.os:459` |  |
| 94 | MUST | ✅ found | Also, any mechanisms for putting the Span into a Context MUST still work after the Span was ended. | `src/Трассировка/Классы/ОтелСпан.os:459` |  |
| 95 | MUST | ✅ found | (Optional) Timestamp to explicitly set the end timestamp. If omitted, this MUST be treated equivalent to passing the current time. | `src/Трассировка/Классы/ОтелСпан.os:461` |  |
| 96 | MUST NOT | ⚠️ partial | This operation itself MUST NOT perform blocking I/O on the calling thread. | `src/Трассировка/Классы/ОтелСпан.os:466` | Завершить() сам по себе не делает I/O, но синхронно вызывает Процессор.ПриЗавершении. ОтелПростойПроцессорСпанов.ПриЗавершении выполняет экспорт синхронно (потенциальный блокирующий I/O в HTTP-экспортёре). ОтелПакетныйПроцессорСпанов добавляет в очередь без блокировки |
| 97 | SHOULD | ➖ n_a | Any locking used needs be minimized and SHOULD be removed entirely if possible. | - | В OneScript однопоточная модель выполнения в рамках сеанса; явных блокировок (locks/mutex) нет |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 98 | SHOULD | ✅ found | To facilitate recording an exception languages SHOULD provide a `RecordException` method if the language uses exceptions. | `src/Трассировка/Классы/ОтелСпан.os:329` |  |
| 99 | MUST | ✅ found | The method MUST record an exception as an `Event` with the conventions outlined in the exceptions document. | `src/Трассировка/Классы/ОтелСпан.os:358` |  |
| 100 | SHOULD | ✅ found | The minimum required argument SHOULD be no more than only an exception object. | `src/Трассировка/Классы/ОтелСпан.os:329` |  |
| 101 | MUST | ✅ found | If `RecordException` is provided, the method MUST accept an optional parameter to provide any additional event attributes (this SHOULD be done in the same way as for the `AddEvent` method). | `src/Трассировка/Классы/ОтелСпан.os:329` |  |
| 102 | SHOULD | ✅ found | If `RecordException` is provided, the method MUST accept an optional parameter to provide any additional event attributes (this SHOULD be done in the same way as for the `AddEvent` method). | `src/Трассировка/Классы/ОтелСпан.os:352` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 103 | MUST | ✅ found | Start and end time as well as Event's timestamps MUST be recorded at a time of a calling of corresponding API. | `src/Трассировка/Классы/ОтелСпан.os:632` |  |
...
| 105 | SHOULD NOT | ❌ not_found | If a new type is required for supporting this operation, it SHOULD NOT be exposed publicly if possible (e.g. by only exposing a function that returns something with the Span interface type). | `src/Трассировка/Классы/ОтелНоопСпан.os:1` | Класс ОтелНоопСпан публично экспонирован и используется напрямую через `Новый ОтелНоопСпан(...)` в пропагаторах и трассировщике. Нет отдельной функции-обёртки, скрывающей конкретный тип. |
| 106 | SHOULD | ⚠️ partial | If a new type is required to be publicly exposed, it SHOULD be named `NonRecordingSpan`. | `src/Трассировка/Классы/ОтелНоопСпан.os:1` | Тип называется ОтелНоопСпан (Noop), а не NonRecordingSpan. Семантика близкая, но не точное соответствие требуемому имени. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 114 | MUST | ✅ found | A user MUST have the ability to record links to other `SpanContext`s. | `src/Трассировка/Классы/ОтелСпан.os:373` |  |
| 115 | MUST | ✅ found | An API to record a single `Link` where the `Link` properties are passed as arguments. | `src/Трассировка/Классы/ОтелПостроительСпана.os:97` |  |
| 116 | SHOULD | ⚠️ partial | Implementations SHOULD record links containing `SpanContext` with empty `TraceId` or `SpanId` (all zeros) as long as either the attribute set or `TraceState` is non-empty. | `src/Трассировка/Классы/ОтелСпан.os:373` | ДобавитьЛинк не отвергает контексты с нулевыми ID, но и не реализует явное условие 'attribute set или TraceState non-empty' - принимает любой контекст без проверок. |
| 117 | SHOULD | ✅ found | Span SHOULD preserve the order in which `Link`s are set. | `src/Трассировка/Классы/ОтелСпан.os:389` |  |
| 118 | MUST | ✅ found | The API documentation MUST state that adding links at span creation is preferred to calling `AddLink` later, for contexts that are available during span creation, because head sampling decisions can only consider information present during span creation. | `src/Трассировка/Классы/ОтелПостроительСпана.os:86` |  |
...
| 123 | SHOULD | ⚠️ partial | Link - Links are immutable and SHOULD be safe for concurrent use by default. | `src/Трассировка/Классы/ОтелСпан.os:384` | Линки хранятся как Соответствие внутри массива Линки и не имеют отдельного класса с документацией о неизменяемости/потокобезопасности. Соответствие в OneScript изменяемо. |
...
| 125 | SHOULD | ⚠️ partial | If the `Span` in the parent `Context` is already non-recording, it SHOULD be returned directly without instantiating a new `Span`. | `src/Трассировка/Классы/ОтелТрассировщик.os:226` | СоздатьНоопСпанВAPIРежиме всегда создаёт новый ОтелНоопСпан, даже если родитель в контексте уже non-recording - не возвращает существующий объект напрямую. |
| 126 | MUST | ✅ found | If the parent `Context` contains no `Span`, an empty non-recording Span MUST be returned instead (i.e., having a `SpanContext` with all-zero Span and Trace IDs, empty Tracestate, and unsampled TraceFlags). | `src/Трассировка/Классы/ОтелНоопСпан.os:277` |  |
...
| 2 | MUST | ✅ found | If configuration is updated (e.g., adding a `SpanProcessor`), the updated configuration MUST also apply to all already returned `Tracers` (i.e. it MUST NOT matter whether a `Tracer` was obtained ... | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:100` |  |
| 3 | MUST NOT | ✅ found | If configuration is updated (e.g., adding a `SpanProcessor`), the updated configuration MUST also apply to all already returned `Tracers` (i.e. it MUST NOT matter whether a `Tracer` was obtained ... | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:393` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 4 | MUST | ✅ found | `Shutdown` MUST be called only once for each `TracerProvider` instance. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:133` |  |
| 5 | SHOULD | ⚠️ partial | SDKs SHOULD return a valid no-op Tracer for these calls, if possible. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:82` | После Закрыть() возвращается новый ОтелТрассировщик без явной Конфигурация (Включен=Ложь). Tracer.Включен() делегирует в Провайдер.Включен(), который остаётся Истина после Закрыть(). В результате возвращённый трассировщик не является валидным no-op (попытается создавать спаны, хотя процессоры уже закрыты). |
| 6 | SHOULD | ⚠️ partial | `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:132` | Метод Закрыть() — Процедура (без возвращаемого значения), ошибки только через исключения, таймаут не отличим от успеха. Есть ЗакрытьАсинхронно (возвращает Обещание), но синхронный API не сообщает результат. |
| 7 | SHOULD | ⚠️ partial | `Shutdown` SHOULD complete or abort within some timeout. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:132` | Provider.Закрыть() не принимает параметр таймаута и не передаёт таймаут в КомпозитныйПроцессор.Закрыть(ТаймаутМс) — вызывается с дефолтным 0 (без ограничения). Механизм прерывания по таймауту на уровне Provider отсутствует. |
| 8 | MUST | ✅ found | `Shutdown` MUST be implemented at least by invoking `Shutdown` within all internal processors. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:137` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 9 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:122` | Метод СброситьБуфер() — Процедура, ничего не возвращает; ошибки только через исключения, таймаут не отличим от успеха. Есть СброситьБуферАсинхронно (Обещание), но синхронный API не сообщает результат. |
| 10 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:122` | Provider.СброситьБуфер() не принимает таймаут. На уровне КомпозитныйПроцессор.СброситьБуфер(ТаймаутМс) поддержка таймаута есть, но Provider вызывает процессоры без таймаута, поэтому общего механизма прерывания на уровне ForceFlush нет. |
| 11 | MUST | ✅ found | `ForceFlush` MUST invoke `ForceFlush` on all registered `SpanProcessors`. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:124` |  |
...
| 14 | MUST | ⚠️ partial | For backwards compatibility it MUST also be able to access the `InstrumentationLibrary` [deprecated since 1.10.0] having the same name and version values as the `InstrumentationScope`. | `src/Трассировка/Классы/ОтелСпан.os:182` | Spand экспонирует ОбластьИнструментирования (InstrumentationScope), но не имеет отдельного устаревшего геттера InstrumentationLibrary - доступ к данным возможен только через новое имя. |
| 15 | MUST | ✅ found | A function receiving this as argument MUST be able to reliably determine whether the Span has ended (some languages might implement this by having an end timestamp of `null`, others might have an explicit `hasEnded` boole... | `src/Трассировка/Классы/ОтелСпан.os:209` |  |
...
| 17 | MUST | ✅ found | As an exception to the authoritative set of span properties defined in the API spec, implementations MAY choose not to expose (and store) the full parent Context of the Span but they MUST expose at least the full pa... | `src/Трассировка/Классы/ОтелСпан.os:101` |  |
| 18 | MUST | ✅ found | It MUST be possible for functions being called with this to somehow obtain the same `Span` instance and type that the span creation API returned (or will return) to the user (for example, the `Span` could be one of... | `src/Трассировка/Классы/ОтелСпан.os:1` |  |
...
| 20 | SHOULD NOT | ⚠️ partial | However, Span Exporter SHOULD NOT receive them unless the `Sampled` flag was also set. | `src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:42` | ПриЗавершении передаёт спан экспортёру без проверки флага Sampled - спаны с решением RECORD_ONLY (IsRecording=true, Sampled=false) всё равно попадают в экспортёр. |
| 21 | MUST | ✅ found | Span Exporters MUST receive those spans which have `Sampled` flag set to true and they SHOULD NOT receive the ones that do not. | `src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:51` |  |
| 22 | SHOULD NOT | ⚠️ partial | Span Exporters MUST receive those spans which have `Sampled` flag set to true and they SHOULD NOT receive the ones that do not. | `src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:42` | Процессоры (Simple/Batch) не фильтруют спаны по флагу Sampled при передаче экспортёру: спаны с RECORD_ONLY попадают в экспортёр. |
| 23 | MUST NOT | ✅ found | The flag combination `SampledFlag == true` and `IsRecording == false` could cause gaps in the distributed trace, and because of this the OpenTelemetry SDK MUST NOT allow this combination. | `src/Трассировка/Классы/ОтелТрассировщик.os:303` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 24 | MUST | ✅ found | When asked to create a Span, the SDK MUST act as if doing the following in order: If there is a valid parent trace ID, use it. | `src/Трассировка/Классы/ОтелТрассировщик.os:65` |  |
...
| 27 | MUST | ✅ found | `RECORD_AND_SAMPLE` - `IsRecording` will be `true` and the `Sampled` flag MUST be set. | `src/Трассировка/Классы/ОтелТрассировщик.os:305` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 32 | MUST | ❌ not_found | Based on the decision from the wrapped root sampler, `AlwaysRecord` MUST behave as follows: | - | Декоратор AlwaysRecord, преобразующий DROP в RECORD_ONLY от обёрнутого сэмплера, в SDK не реализован. ОтелСэмплер не содержит сущности, оборачивающей другой сэмплер для конвертации решений. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 34 | SHOULD | ❌ not_found | For root span contexts, the SDK SHOULD set the `Random` flag in the trace flags when it generates TraceIDs that meet the W3C Trace Context Level 2 randomness requirements. | - | Бит Random (0x02) во TraceFlags нигде не устанавливается. ОтелКонтекстСпана.ФлагиТрассировки используется только для бита sampled (0x01); ОтелТрассировщик.ВычислитьФлагиТрассировки и СоздатьКонтекстСпана не выставляют 0x02 для корневых спанов. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 36 | SHOULD | ⚠️ partial | For all span contexts, OpenTelemetry samplers SHOULD presume that TraceIDs meet the W3C Trace Context Level 2 randomness requirements, unless an explicit randomness value is present in the `rv` sub-key of the OpenTelemetry TraceState. | `src/Трассировка/Модули/ОтелСэмплер.os:277` | TraceIdRatioBased сэмплер (СэмплироватьПоДоле) использует первые 8 hex-символов TraceID как источник случайности (presumption of randomness), но не учитывает explicit `rv` sub-key из TraceState - если присутствует, должен использоваться именно он. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 37 | SHOULD | ❌ not_found | If the SDK uses an `IdGenerator` extension point, the SDK SHOULD allow the extension to determine whether the Random flag is set when new IDs are generated. | - | IdGenerator extension point есть (УстановитьГенераторИд), но нет механизма, через который генератор сообщает SDK о том, выставлять ли Random flag (W3C Trace Context Level 2). Trace flags при генерации не учитывают свойство генератора. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 38 | MUST | ✅ found | Span attributes MUST adhere to the common rules of attribute limits. | `src/Трассировка/Классы/ОтелЛимитыСпана.os:165` |  |
...
| 40 | SHOULD | ⚠️ partial | The name of the configuration options SHOULD be `EventCountLimit` and `LinkCountLimit`. | `src/Трассировка/Классы/ОтелЛимитыСпана.os:6` | Опции названы МаксСобытий (=MaxEvents) и МаксЛинков (=MaxLinks), а не EventCountLimit/LinkCountLimit - перевод неточный, в имени отсутствует семантика 'Count'. |
...
| 42 | SHOULD | ✅ found | There SHOULD be a message printed in the SDK's log to indicate to the user that an attribute, event, or link was discarded due to such a limit. | `src/Трассировка/Классы/ОтелСпан.os:482` |  |
| 43 | MUST | ✅ found | To prevent excessive logging, the message MUST be printed at most once per span (i.e., not per discarded attribute, event, or link). | `src/Трассировка/Классы/ОтелСпан.os:481` |  |
...
| 46 | MUST | ✅ found | The SDK MAY provide this functionality by allowing custom implementations of an interface like the Java example below (name of the interface MAY be `IdGenerator`, name of the methods MUST be consistent with SpanContext), which provides extension points for two methods, one to generate a `SpanId` and one for `TraceId`. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:260` |  |
| 47 | MUST NOT | ✅ found | Additional `IdGenerator` implementing vendor-specific protocols such as AWS X-Ray trace ID generator MUST NOT be maintained or distributed as part of the Core OpenTelemetry repositories. | - |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 48 | MUST | ✅ found | SDK MUST allow to end each pipeline with individual exporter. | `src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:104` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 50 | MUST | ✅ found | The `SpanProcessor` interface MUST declare the following methods: OnStart, OnEnd, Shutdown, ForceFlush. | `src/Трассировка/Классы/ИнтерфейсПроцессорСпанов.os:11` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 52 | SHOULD | ✅ found | It SHOULD be possible to keep a reference to this span object and updates to the span SHOULD be reflected in it. | `src/Трассировка/Классы/ОтелКомпозитныйПроцессорСпанов.os:21` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 54 | MUST | ✅ found | This method MUST be called synchronously within the `Span.End()` API, therefore it should not block or throw an exception. | `src/Трассировка/Классы/ОтелСпан.os:471` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 55 | SHOULD | ✅ found | `Shutdown` SHOULD be called only once for each `SpanProcessor` instance. | `src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:78` |  |
| 56 | SHOULD | ✅ found | After the call to `Shutdown`, subsequent calls to `OnStart`, `OnEnd`, or `ForceFlush` are not allowed. SDKs SHOULD ignore these calls gracefully, if possible. | `src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:43` |  |
| 57 | SHOULD | ⚠️ partial | `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:80` | Закрыть реализована как Процедура (void), не возвращает статус успеха/неудачи/таймаута. Таймаут логируется, но caller не может отличить успех от таймаута. |
| 58 | MUST | ✅ found | `Shutdown` MUST include the effects of `ForceFlush`. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:83` |  |
| 59 | SHOULD | ✅ found | `Shutdown` SHOULD complete or abort within some timeout. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:80` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 60 | SHOULD | ✅ found | This is a hint to ensure that any tasks associated with `Spans` for which the `SpanProcessor` had already received events prior to the call to `ForceFlush` SHOULD be completed as soon as possible, preferably before retu... | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:71` |  |
| 61 | SHOULD | ✅ found | In particular, if any `SpanProcessor` has any associated exporter, it SHOULD try to call the exporter's `Export` with all spans for which this was not already done and then invoke `ForceFlush` on it. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:148` |  |
| 62 | MUST | ✅ found | The built-in SpanProcessors MUST do so. | `src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:66` |  |
| 63 | MUST | ✅ found | If a timeout is specified (see below), the SpanProcessor MUST prioritize honoring the timeout over finishing all calls. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:128` |  |
| 64 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:71` | СброситьБуфер реализована как Процедура (void), не возвращает статус. Таймаут логируется, но caller не может отличить успех от таймаута. |
| 65 | SHOULD | ✅ found | `ForceFlush` SHOULD only be called in cases where it is absolutely necessary, such as when using some FaaS providers that may suspend the process after an invocation, but before the `SpanProcessor` exports the completed ... | `src/Трассировка/Классы/ИнтерфейсПроцессорСпанов.os:37` |  |
| 66 | SHOULD | ✅ found | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:71` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 69 | MUST | ✅ found | The processor MUST synchronize calls to `Span Exporter`'s `Export` to make sure that they are not invoked concurrently. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:155` |  |
| 70 | SHOULD | ✅ found | The processor SHOULD export a batch when any of the following happens AND the previous export call has returned: | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:42` |  |
...
| 75 | SHOULD NOT | ✅ found | The default SDK's Span Processors SHOULD NOT implement retry logic, as the required logic is likely to depend heavily on the specific protocol and backend the spans are being sent to. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:148` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 76 | SHOULD | ✅ found | This is a hint to ensure that the export of any `Spans` the exporter has received prior to the call to `ForceFlush` SHOULD be completed as soon as possible, preferably before returning from this method. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:71` |  |
| 77 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:71` | СброситьБуфер реализована как Процедура без возвращаемого значения; ошибки/таймауты логируются, но вызывающий не получает явный статус (success/failure/timeout) - поведение реализовано через обходной путь. |
| 78 | SHOULD | ✅ found | `ForceFlush` SHOULD only be called in cases where it is absolutely necessary, such as when using some FaaS providers that may suspend the process after an invocation, but before the exporter ex... | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:71` |  |
| 79 | SHOULD | ✅ found | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:126` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 80 | MUST | ✅ found | Tracer Provider - Tracer creation, `ForceFlush` and `Shutdown` MUST be safe to be called concurrently. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:7` |  |
| 81 | MUST | ✅ found | Sampler - `ShouldSample` and `GetDescription` MUST be safe to be called concurrently. | `src/Трассировка/Модули/ОтелСэмплер.os:141` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 3 | MUST | ✅ found | This API MUST accept the following instrumentation scope parameters: | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:56` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 7 | MUST | ✅ found | The API MUST accept the following parameters: | `src/Логирование/Классы/ОтелЛоггер.os:94` |  |
...
| 9 | MUST | ✅ found | When implicit Context is supported, then this parameter SHOULD be optional and if unspecified then MUST use current Context. | `src/Логирование/Классы/ОтелЛоггер.os:101` |  |
| 10 | SHOULD | ➖ n_a | When only explicit Context is supported, this parameter SHOULD be required. | - | Альтернативная ветка спецификации: SDK поддерживает неявный контекст (ОтелКонтекст.Текущий()), поэтому требование 'only explicit Context' не применимо. |
...
| 16 | SHOULD | ⚠️ partial | The API SHOULD be documented that instrumentation authors needs to call this API each time they emit a LogRecord to ensure they have the most up-to-date response. | `src/Логирование/Классы/ОтелЛоггер.os:28` | Документирующий комментарий объясняет назначение Включен (пропуск создания LogRecord), но не содержит явного указания инструментирующим авторам вызывать API каждый раз перед эмитом записи, чтобы получать актуальный ответ. |
...
| 6 | MUST | ✅ found | If configuration is updated (e.g., adding a `LogRecordProcessor`), the updated configuration MUST also apply to all already returned `Logger`s (i.e. it MUST NOT matter whether a `Logger` was obtained... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:171` |  |
| 7 | MUST | ✅ found | If configuration is updated (e.g., adding a `LogRecordProcessor`), the updated configuration MUST also apply to all already returned `Logger`s (i.e. it MUST NOT matter whether a `Logger` was obtained... | `src/Логирование/Классы/ОтелКомпозитныйПроцессорЛогов.os:1` |  |
...
| 11 | SHOULD | ⚠️ partial | `Shutdown` SHOULD complete or abort within some timeout. | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:132` | Метод Закрыть() провайдера не принимает таймаут и не передаёт его процессорам (используется значение по умолчанию 0 = без ограничения). ЗакрытьАсинхронно возвращает Обещание, но без таймаута. |
...
| 14 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD return some ERROR status if there is an error condition; | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:148` | СброситьБуферАсинхронно возвращает Обещание, ошибки могут проявиться через rejected Promise, но синхронный метод СброситьБуфер - процедура без явного статуса ошибки. |
| 15 | SHOULD | ⚠️ partial | and if there is no error condition, it SHOULD return some NO ERROR status, language implementations MAY decide how to model ERROR and NO ERROR. | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:148` | Только асинхронный вариант возвращает Обещание; синхронный СброситьБуфер - процедура (void), без явного NO ERROR статуса. |
| 16 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:122` | Метод СброситьБуфер провайдера не принимает параметр таймаута и не передаёт его процессорам. Async-вариант также без таймаута. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 22 | MUST | ✅ found | A function receiving this as an argument MUST additionally be able to modify the following information added to the LogRecord: Timestamp, ObservedTimestamp, SeverityText, SeverityNumber, Bod... | `src/Логирование/Классы/ОтелЗаписьЛога.os:179` |  |
...
| 24 | MUST | ✅ found | If the SDK implements attribute limits it MUST provide a way to change these limits, via a configuration to the LoggerProvider, by allowing users to configure individual limits like in the J... | `src/Логирование/Классы/ОтелПостроительПровайдераЛогирования.os:50` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 28 | MUST | ✅ found | The SDK MUST allow each pipeline to end with an individual exporter. | `src/Логирование/Классы/ОтелПростойПроцессорЛогов.os:20` |  |
...
| 32 | SHOULD | ❌ not_found | To avoid such race conditions, implementations SHOULD recommended to users that a clone of `logRecord` be used for any concurrent processing, such as in a batching processor. | - | В документации API (комментариях ПриПоявлении) нет рекомендации пользователям клонировать ЗаписьЛога для конкурентной обработки в batching-процессоре. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 33 | MUST NOT | ✅ found | Any modifications to parameters inside `Enabled` MUST NOT be propagated to the caller. | `src/Логирование/Классы/ОтелЛоггер.os:77` |  |
...
| 35 | SHOULD | ⚠️ partial | After the call to `Shutdown`, subsequent calls to `OnEmit` are not allowed. SDKs SHOULD ignore these calls gracefully, if possible. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:43` | ОтелБазовыйПакетныйПроцессор.Обработать проверяет флаг Закрыт и игнорирует вызовы; в ОтелПростойПроцессорЛогов нет проверки состояния после Закрыть(), последующие вызовы ПриПоявлении приведут к ошибке экспортера (исключение поглощается композитом, но не gracefully на уровне процессора). |
| 36 | SHOULD | ⚠️ partial | `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:160` | Закрыть() — процедура без возвращаемого значения; ЗакрытьАсинхронно() возвращает Обещание, но статус успеха/таймаута/ошибки не различается; при таймауте в композите процессор просто пропускается без сигнализации вызывающему. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 39 | SHOULD | ✅ found | This is a hint to ensure that any tasks associated with `LogRecord`s for which the `LogRecordProcessor` had already received events prior to the call to `ForceFlush` SHOULD be completed as soon as ... | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:71` |  |
...
| 42 | MUST | ✅ found | If a timeout is specified (see below), the `LogRecordProcessor` MUST prioritize honoring the timeout over finishing all calls. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:126` |  |
| 43 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:148` | СброситьБуфер() — процедура без возврата; СброситьБуферАсинхронно() возвращает Обещание, но не различает статусы успеха/таймаута/ошибки; в композите при таймауте процессор пропускается без явной сигнализации вызывающему. |
| 44 | SHOULD | ✅ found | `ForceFlush` SHOULD only be called in cases where it is absolutely necessary, such as when using some FaaS providers that may suspend the process after an invocation, but before the `LogRecordProces... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:122` |  |
| 45 | SHOULD | ✅ found | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:126` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 46 | MUST | ✅ found | The standard OpenTelemetry SDK MUST implement both simple and batch processors, as described below. | `src/Логирование/Классы/ОтелПростойПроцессорЛогов.os:18` |  |
...
| 48 | SHOULD | ❌ not_found | Additional processors defined in this document SHOULD be provided by SDK packages. | - | Дополнительный процессор Event-to-span-event-bridge, описанный в этой же спецификации, в SDK не реализован. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 49 | MUST | ✅ found | The processor MUST synchronize calls to `LogRecordExporter`'s `Export` to make sure that they are not invoked concurrently. | `src/Логирование/Классы/ОтелПростойПроцессорЛогов.os:22` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 50 | MUST | ✅ found | The processor MUST synchronize calls to `LogRecordExporter`'s `Export` to make sure that they are not invoked concurrently. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:155` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 53 | MUST NOT | ✅ found | `Export` MUST NOT block indefinitely, there MUST be a reasonable upper limit after which the call must time out with an error result (`Failure`). | `src/Экспорт/Классы/ОтелЭкспортерЛогов.os:43` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 56 | SHOULD | ✅ found | This is a hint to ensure that the export of any `ReadableLogRecords` the exporter has received prior to the call to `ForceFlush` SHOULD be completed as soon as possible, preferably before returnin... | `src/Экспорт/Классы/ОтелЭкспортерЛогов.os:58` |  |
| 57 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Экспорт/Классы/ОтелЭкспортерЛогов.os:58` | СброситьБуфер реализован как Процедура без возвращаемого значения; статус успеха/таймаута не возвращается. На уровне провайдера есть СброситьБуферАсинхронно (Обещание), но у самого LogRecordExporter возврата статуса нет. |
| 58 | SHOULD | ✅ found | `ForceFlush` SHOULD only be called in cases where it is absolutely necessary, such as when using some FaaS providers that may suspend the process after an invocation, but before the exporter expor... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:122` |  |
| 59 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Экспорт/Классы/ОтелЭкспортерЛогов.os:58` | СброситьБуфер у экспортера не принимает параметра таймаута и не имеет механизма прерывания; синхронный экспортер без буфера возвращается сразу, но явного таймаута/прерывания нет. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 60 | SHOULD | ✅ found | Shutdown SHOULD be called only once for each `LogRecordExporter` instance. | `src/Экспорт/Классы/ОтелЭкспортерЛогов.os:64` |  |
...
| 62 | SHOULD NOT | ✅ found | `Shutdown` SHOULD NOT block indefinitely (e.g. if it attempts to flush the data and the destination is unavailable). | `src/Экспорт/Классы/ОтелЭкспортерЛогов.os:64` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 63 | MUST | ✅ found | LoggerProvider - Logger creation, `ForceFlush` and `Shutdown` MUST be safe to be called concurrently. | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:14` |  |
| 64 | MUST | ✅ found | Logger - all methods MUST be safe to be called concurrently. | `src/Логирование/Классы/ОтелЛоггер.os:1` |  |
...
| 6 | MUST | ✅ found | Therefore, this API MUST be structured to accept a variable number of attributes, including none. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:62` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 9 | SHOULD | ✅ found | Language-level features such as the distinction between integer and floating point numbers SHOULD be considered as identifying. | `src/Метрики/Классы/ОтелБазовыйАгрегатор.os:8` |  |
...
| 11 | MUST | ⚠️ partial | It MUST be case-sensitive (e.g. `kb` and `kB` are different units), ASCII string. | `src/Метрики/Классы/ОтелМетр.os:621` | Case-sensitivity обеспечивается платформой OneScript (оператор = для строк регистрозависим), значит требование case-sensitive выполнено. Однако валидация ASCII не реализована: НормализоватьСтроку возвращает значение как есть, без проверки на не-ASCII символы. Поэтому partial. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 12 | MUST | ✅ found | The API MUST treat it as an opaque string. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:65` |  |
...
| 17 | MUST | ✅ found | If it is not possible to structurally enforce this obligation, the API MUST be documented in a way to communicate to users that this parameter is needed. | `src/Метрики/Классы/ОтелМетр.os:40` |  |
| 18 | SHOULD | ⚠️ partial | The API SHOULD be documented in a way to communicate to users that the `name` parameter needs to conform to the instrument name syntax. | `src/Метрики/Классы/ОтелМетр.os:40` | Документирующий комментарий к СоздатьСчетчик описывает параметр как 'имя счетчика', но не ссылается на правила instrument name syntax. |
...
| 26 | SHOULD NOT | ⚠️ partial | The API SHOULD NOT validate `advisory` parameters. | `src/Метрики/Классы/ОтелМетр.os:796` | ПроверитьСовет логирует предупреждения при несовпадении типов в advisory. Это мягкая валидация (warn-only), не отбрасывает значения, но формально проверяет структуру. |
...
| 29 | MUST | ✅ found | If it is not possible to structurally enforce this obligation, the API MUST be documented in a way to communicate to users that this parameter is needed. | `src/Метрики/Классы/ОтелМетр.os:269` |  |
| 30 | SHOULD | ⚠️ partial | The API SHOULD be documented in a way to communicate to users that the `name` parameter needs to conform to the instrument name syntax. | `src/Метрики/Классы/ОтелМетр.os:269` | Комментарий описывает 'имя инструмента', но не ссылается на instrument name syntax. |
...
| 38 | SHOULD NOT | ⚠️ partial | The API SHOULD NOT validate `advisory` parameters. | `src/Метрики/Классы/ОтелМетр.os:796` | ПроверитьСовет логирует предупреждения при несовпадении типов в advisory (warn-only). |
...
| 40 | MUST | ✅ found | The API MUST support creation of asynchronous instruments by passing zero or more `callback` functions to be permanently registered to the newly created instrument. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:153` |  |
...
| 42 | MUST | ✅ found | Where the API supports registration of `callback` functions after asynchronous instrumentation creation, the user MUST be able to undo registration of the specific callback after its registration by some means. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:69` |  |
...
| 44 | MUST | ✅ found | Callback functions MUST be documented as follows for the end user: | `src/Метрики/Классы/ОтелМетр.os:269` |  |
...
| 50 | MUST | ⚠️ partial | Multiple-instrument Callbacks MUST be associated at the time of registration with a declared set of asynchronous instruments from the same `Meter` instance. | `src/Метрики/Классы/ОтелМетр.os:528` | ЗарегистрироватьОбратныйВызов(Callback, НовыеИнструменты) принимает явный набор инструментов, но не валидирует, что все инструменты принадлежат тому же Метру. |
| 51 | MUST | ✅ found | The API MUST treat observations from a single Callback as logically taking place at a single instant, such that when recorded, observations from a single callback MUST be reported with... | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:188` |  |
| 52 | MUST | ✅ found | The API MUST treat observations from a single Callback as logically taking place at a single instant, such that when recorded, observations from a single callback MUST be reported with... | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:188` |  |
| 53 | SHOULD | ⚠️ partial | The API SHOULD provide some way to pass `state` to the callback. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:171` | Callback получает только объект ОтелНаблюдениеМетрики. Отдельного механизма передачи user state нет; пользователь может использовать замыкание lambda. |
...
| 58 | SHOULD | ⚠️ partial | The API SHOULD be documented that instrumentation authors needs to call this API each time they record a measurement to ensure they have the most up-to-date response. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:248` | Комментарий описывает поведение метода и упоминает MUST/спеку, но не указывает явно, что API нужно вызывать перед каждой записью измерения для актуальности результата. |
...
| 65 | SHOULD NOT | ⚠️ partial | This API SHOULD NOT validate this value, that is left to implementations of the API. | `src/Метрики/Классы/ОтелСчетчик.os:22` | Counter.Добавить выполняет валидацию (отрицательные значения молча игнорируются: 'Если Значение < 0 Тогда Возврат'), вместо того чтобы делегировать это SDK. |
...
| 71 | SHOULD | ✅ found | The API SHOULD provide some way to pass `state` to the callback. | `src/Метрики/Классы/ОтелМетр.os:291` |  |
...
| 77 | SHOULD | ⚠️ partial | This API SHOULD be documented in a way to communicate to users that this value is expected to be non-negative. | `src/Метрики/Классы/ОтелГистограмма.os:13` | Документирующий комментарий перед Записать описывает параметр Значение, но не упоминает требование неотрицательности (non-negative). |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 95 | SHOULD | ❌ not_found | The API to register a new Callback SHOULD accept: * A `callback` function* A list (or tuple, etc.) of Instruments used in the `callback` function. | - | Multiple-instrument callbacks являются опциональной фичей (MAY support); SDK не предоставляет API регистрации одного callback на список инструментов — callback задаётся индивидуально на каждый наблюдаемый инструмент через СоздатьНаблюдаемый*/Callback. Отдельного RegisterCallback с (callback, [instruments]) нет. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 96 | SHOULD | ✅ found | All the metrics components SHOULD allow new APIs to be added to existing components without introducing breaking changes. | `src/Метрики/Классы/ОтелМетр.os:202` |  |
| 97 | SHOULD | ✅ found | All the metrics APIs SHOULD allow optional parameter(s) to be added to existing APIs without introducing breaking changes, if possible. | `src/Метрики/Классы/ОтелМетр.os:202` |  |
...
| 6 | MUST | ✅ found | If configuration is updated (e.g., adding a `MetricReader`), the updated configuration MUST also apply to all already returned `Meters` (i.e. it MUST NOT matter whether ... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:223` |  |
| 7 | MUST NOT | ✅ found | If configuration is updated (e.g., adding a `MetricReader`), the updated configuration MUST also apply to all already returned `Meters` (i.e. it MUST NOT matter whether ... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:84` |  |
...
| 11 | SHOULD | ⚠️ partial | `Shutdown` SHOULD complete or abort within some timeout. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:143` | Закрыть/ЗакрытьАсинхронно не принимают параметр таймаута и не имеют механизма прерывания операции. Асинхронная обёртка через Обещание есть, но без timeout/abort. |
...
| 15 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD return some ERROR status if there is an error condition; and if there is no error condition, it should return some NO ERROR status, language implemen... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:167` | СброситьБуфер - процедура без возвращаемого статуса; ошибки/успех модулируются только через Обещание в СброситьБуферАсинхронно. Явной модели ERROR/NO ERROR нет. |
| 16 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:128` | СброситьБуфер/СброситьБуферАсинхронно не принимают параметр таймаута и не имеют механизма прерывания операции по истечении времени. |
...
| 29 | MUST NOT | ➖ n_a | Therefore, the instrument selection criteria can be structured to accept the criteria, but MUST NOT obligate a user to provide them. | - | Дополнительные (не из списка спецификации) критерии селектором не поддерживаются - SDK принимает только базовые: имя, тип, имя/версия/адрес схемы метра, единица. Требование применимо только если SDK поддерживает дополнительные критерии (MAY). |
...
| 32 | SHOULD | ❌ not_found | In order to avoid conflicts, if a `name` is provided the View SHOULD have an instrument selector that selects at most one instrument. | - | Нет проверки, что при заданном имени потока селектор отбирает не более одного инструмента; первый найденный View применяется без проверки конфликта. |
...
| 34 | MUST | ⚠️ partial | If the user does not provide a `name` value, name from the Instrument the View matches MUST be used by default. | `src/Метрики/Классы/ОтелМетр.os:635` | ОтелПредставление поддерживает поле НовоеИмя, но в коде применения View (ПрименитьПредставлениеКИнструменту) переименование инструмента по View не применяется к выходным данным метрики. |
...
| 37 | MUST | ⚠️ partial | If the user does not provide a `description` value, the description from the Instrument a View matches MUST be used by default. | `src/Метрики/Классы/ОтелМетр.os:635` | Поле НовоеОписание есть в ОтелПредставление, но применение НовоеОписание() в выходные данные метрики не реализовано (используется описание инструмента всегда, если View не задаёт - по умолчанию). |
...
| 42 | MUST | ✅ found | If the `Attributes` advisory parameter is absent, all attributes MUST be kept. | `src/Метрики/Классы/ОтелМетр.os:657` |  |
...
| 51 | MUST | ✅ found | If the user does not provide an `aggregation_cardinality_limit` value, the `MeterProvider` MUST apply the default aggregation cardinality limit the `MetricReader` is configured with. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:151` |  |
...
| 53 | MUST | ✅ found | Instrument advisory parameters, if any, MUST be honored. | `src/Метрики/Классы/ОтелМетр.os:683` |  |
| 54 | SHOULD | ⚠️ partial | If applying the View results in conflicting metric identities the implementation SHOULD apply the View and emit a warning. | `src/Метрики/Классы/ОтелМетр.os:707` | Конфликт дескрипторов инструмента (одинаковое имя, разные параметры) логируется через Лог.Предупреждение, но конфликт идентичностей метрик из-за применения нескольких Views не детектируется (НайтиПредставление возвращает только первое совпадение). |
| 55 | SHOULD | ❌ not_found | If it is not possible to apply the View without producing semantic errors (e.g. the View sets an asynchronous instrument to use the Explicit bucket histogram aggregation) the implementation SHOULD emit a warning and proceed as if the View did not | - | Нет валидации семантической совместимости View с инструментом (например, Explicit bucket histogram для async-инструмента) и нет соответствующего предупреждения с откатом к поведению без View. |
...
| 57 | SHOULD | ✅ found | If the Instrument could not match with any of the registered `View`(s), the SDK SHOULD enable the instrument using the default aggregation and temporality. | `src/Метрики/Классы/ОтелМетр.os:67` |  |
...
| 59 | SHOULD | ✅ found | The SDK SHOULD provide the following `Aggregation`: | `src/Метрики/Модули/ОтелАгрегация.os:79` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 60 | SHOULD NOT | ⚠️ partial | Arithmetic sum of `Measurement` values in population. This SHOULD NOT be collected when used with instruments that record negative measurements (e.g. `UpDownCounter` or `ObservableGauge`). | `src/Метрики/Классы/ОтелАгрегаторГистограммы.os:51` | Сумма всегда собирается без проверки типа инструмента; нет защиты от использования гистограммы с инструментами, регистрирующими отрицательные значения (UpDownCounter/ObservableGauge). |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 66 | MUST | ✅ found | Callback functions MUST be invoked for the specific `MetricReader` performing collection, such that observations made or produced by executing callbacks only apply to the intended `MetricReader` during collection. | `src/Метрики/Классы/ОтелМетр.os:536` |  |
| 67 | SHOULD | ✅ found | The implementation SHOULD disregard the use of asynchronous instrument APIs outside of registered callbacks. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:168` |  |
| 68 | SHOULD | ❌ not_found | The implementation SHOULD use a timeout to prevent indefinite callback execution. | - | В ВызватьCallbackИСобрать (ОтелБазовыйНаблюдаемыйИнструмент) и в ВызватьМультиОбратныеВызовы (ОтелМетр) callback вызывается синхронно через Действие.Выполнить без ограничения времени; ошибки ловятся, но таймаут не реализован. |
| 69 | MUST | ✅ found | The implementation MUST complete the execution of all callbacks for a given instrument before starting a subsequent round of collection. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:99` |  |
| 70 | SHOULD NOT | ✅ found | The implementation SHOULD NOT produce aggregated metric data for a previously-observed attribute set which is not observed during a successful callback. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:112` |  |
...
| 74 | SHOULD | ⚠️ partial | If there is no matching view, but the `MetricReader` defines a default cardinality limit value based on the instrument an aggregation is created for, that value SHOULD be used. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:151` | MetricReader предоставляет один общий ЛимитМощности (по умолчанию 2000), не дифференцированный по типу инструмента. Метр применяет его ко всем инструментам через ПрименитьНастройкиЧитателяКМетру. |
| 75 | SHOULD | ✅ found | If none of the previous values are defined, the default value of 2000 SHOULD be used. | `src/Метрики/Классы/ОтелМетр.os:613` |  |
...
| 79 | MUST | ✅ found | Regardless of aggregation temporality, the SDK MUST ensure that every Measurement is reflected in exactly one Aggregator, which is either an Aggregator associated with the corre... | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:108` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 81 | SHOULD | ❌ not_found | Aggregators of asynchronous instruments SHOULD prefer the first-observed attributes in the callback when limiting cardinality, regardless of temporality. | - | В ОтелБазовыйНаблюдаемыйИнструмент.ВызватьCallbackИСобрать/ПреобразоватьЗаписиВТочки нет логики ограничения мощности и нет приоритизации первых наблюдаемых атрибутов; cardinality limit для асинхронных инструментов не применяется. |
...
| 84 | SHOULD | ⚠️ partial | The emitted warning SHOULD include information for the user on how to resolve the conflict, if possible. | `src/Метрики/Классы/ОтелМетр.os:727` | Предупреждение содержит описание различий (вид/единица/описание), но не предлагает конкретного способа разрешения (например, использовать View для переименования). |
| 85 | SHOULD | ❌ not_found | If the potential conflict involves multiple `description` properties, setting the `description` through a configured View SHOULD avoid the warning. | - | ПроверитьКонфликтДескриптора сравнивает Описание напрямую и не учитывает, что View мог переопределить description; предупреждение выводится даже если конфликт может быть устранён View. |
| 86 | SHOULD | ❌ not_found | If the potential conflict involves instruments that can be distinguished by a supported View selector (e.g. name, instrument kind) a renaming View recipe SHOULD be included in the... | - | Текст предупреждения статичен и не содержит рецепта переименования через View. |
| 87 | SHOULD | ❌ not_found | Otherwise (e.g., use of multiple units), the SDK SHOULD pass through the data by reporting both `Metric` objects and emit a generic warning describing the duplicate instrument re... | - | СоздатьСчетчик/Гистограмму/Датчик возвращает существующий инструмент при совпадении нормализованного имени; нет механизма создания и экспорта обоих Metric-объектов с разными units. |
| 88 | MUST | ✅ found | the SDK MUST aggregate data from identical Instruments | `src/Метрики/Классы/ОтелМетр.os:56` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 89 | MUST | ✅ found | When this happens, the Meter MUST return an instrument using the first-seen instrument name and log an appropriate error as described above. | `src/Метрики/Классы/ОтелМетр.os:56-60` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 90 | SHOULD | ❌ not_found | When a Meter creates an instrument, it SHOULD validate the instrument name conforms to the instrument name syntax | - | В ОтелМетр.СоздатьСчетчик/СоздатьГистограмму/СоздатьДатчик и др. отсутствует проверка имени инструмента на соответствие синтаксису (длина, допустимые символы, начальный символ). Имя только нормализуется через НРег(Имя). |
| 91 | SHOULD | ❌ not_found | If the instrument name does not conform to this syntax, the Meter SHOULD emit an error notifying the user about the invalid name. | - | Так как валидация имени отсутствует, ошибка о невалидном имени не эмитится. Лог предупреждений вызывается только при конфликте дескрипторов, но не при нарушении синтаксиса имени. |
...
| 93 | MUST | ✅ found | If a unit is not provided or the unit is null, the Meter MUST treat it the same as an empty unit string. | `src/Метрики/Классы/ОтелМетр.os:621-626` |  |
...
| 95 | MUST | ✅ found | If a description is not provided or the description is null, the Meter MUST treat it the same as an empty description string. | `src/Метрики/Классы/ОтелМетр.os:621-626` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 96 | SHOULD | ✅ found | When a Meter creates an instrument, it SHOULD validate the instrument advisory parameters. | `src/Метрики/Классы/ОтелМетр.os:796-815` |  |
| 97 | SHOULD | ⚠️ partial | If an advisory parameter is not valid, the Meter SHOULD emit an error notifying the user and proceed as if the parameter was not provided. | `src/Метрики/Классы/ОтелМетр.os:796-815` | ПроверитьСовет логирует Предупреждение о неправильном типе параметра, но не сбрасывает невалидный параметр - дальнейший код (ОпределитьГраницыГистограммы, ПолучитьГраницыИзСовета) всё равно использует исходный Совет, не действуя 'как если бы параметр не был задан'. |
| 98 | MUST | ✅ found | If multiple identical Instruments are created with different advisory parameters, the Meter MUST return an instrument using the first-seen advisory parameters and log an appropriate error as described in duplicate instrument registrations. | `src/Метрики/Классы/ОтелМетр.os:707-733` |  |
| 99 | MUST | ✅ found | If both a View and advisory parameters specify the same aspect of the Stream configuration, the setting defined by the View MUST take precedence over the advisory parameters. | `src/Метрики/Классы/ОтелМетр.os:683-695` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 100 | MUST | ✅ found | If no View matches, or if a matching View selects the default aggregation, the `ExplicitBucketBoundaries` advisory parameter MUST be used. | `src/Метрики/Классы/ОтелМетр.os:683-695` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 101 | MUST | ✅ found | A Metric SDK MUST provide a mechanism to sample `Exemplar`s from measurements via the `ExemplarFilter` and `ExemplarReservoir` hooks. | `src/Метрики/Модули/ОтелФильтрЭкземпляров.os:14-66; src/Метрики/Классы/ОтелРезервуарЭкземпляров.os` |  |
| 102 | SHOULD | ✅ found | `Exemplar` sampling SHOULD be turned on by default. | `src/Метрики/Классы/ОтелМетр.os:608-611` |  |
| 103 | MUST NOT | ✅ found | If `Exemplar` sampling is off, the SDK MUST NOT have overhead related to exemplar sampling. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:413-417` |  |
| 104 | MUST | ✅ found | A Metric SDK MUST allow exemplar sampling to leverage the configuration of metric aggregation. | `src/Метрики/Классы/ОтелВыровненныйРезервуарГистограммы.os:115-120; src/Метрики/Классы/ОтелФабрикаВыровненныхРезервуаровГистограммы.os:21` |  |
| 105 | SHOULD | ✅ found | A Metric SDK SHOULD provide configuration for Exemplar sampling, specifically: `ExemplarFilter`: filter which measurements can become exemplars. | `src/Метрики/Классы/ОтелПостроительПровайдераМетрик.os:74-76; src/Метрики/Классы/ОтелПредставление.os` |  |
...
| 108 | SHOULD | ✅ found | The default value SHOULD be `TraceBased`. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:312` |  |
...
| 110 | MUST | ✅ found | An OpenTelemetry SDK MUST support the following filters: | `src/Метрики/Модули/ОтелФильтрЭкземпляров.os:13` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 111 | MUST | ✅ found | The `ExemplarReservoir` interface MUST provide a method to offer measurements to the reservoir and another to collect accumulated Exemplars. | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:38` |  |
| 112 | MUST | ✅ found | A new `ExemplarReservoir` MUST be created for every known timeseries data point, as determined by aggregation and view configuration. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:432` |  |
| 113 | SHOULD | ⚠️ partial | The “offer” method SHOULD accept measurements, including: | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:38` | Предложить принимает Значение, АтрибутыИзмерения и КонтекстСпана, но timestamp генерируется внутри (не передаётся параметром), а полный Context (с Baggage) не принимается. |
| 114 | SHOULD | ✅ found | The “offer” method SHOULD have the ability to pull associated trace and span information without needing to record full context. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:413` |  |
| 115 | MUST | ✅ found | This MUST be clearly documented in the API and the reservoir MUST be given the `Attributes` associated with its timeseries point either at construction so that additional sampling performed by the rese... | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:30` |  |
| 116 | MUST | ⚠️ partial | This MUST be clearly documented in the API and the reservoir MUST be given the `Attributes` associated with its timeseries point either at construction so that additional sampling performed by the rese... | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:38` | АтрибутыСерии передаются в каждом вызове Предложить(), а не на конструировании резервуара; функционально атрибуты доступны, но архитектура отличается от рекомендованной (at construction). |
| 117 | MUST | ✅ found | The “collect” method MUST return accumulated `Exemplar`s. | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:62` |  |
| 118 | SHOULD | ✅ found | In other words, Exemplars reported against a metric data point SHOULD have occurred within the start/stop timestamps of that point. | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:122` |  |
| 119 | MUST | ✅ found | `Exemplar`s MUST retain any attributes available in the measurement that are not preserved by aggregation or view configuration for the associated timeseries. | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:140` |  |
| 120 | SHOULD | ⚠️ partial | The `ExemplarReservoir` SHOULD avoid allocations when sampling exemplars. | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:117` | Резервуар выделяет новое Соответствие на каждый сэмплированный экземпляр в СоздатьЭкземпляр; нет пула или переиспользования объектов. |
...
| 122 | SHOULD | ✅ found | Explicit bucket histogram aggregation with more than 1 bucket SHOULD use `AlignedHistogramBucketExemplarReservoir`. | `src/Метрики/Классы/ОтелМетр.os:138` |  |
| 123 | SHOULD | ✅ found | Base2 Exponential Histogram Aggregation SHOULD use a `SimpleFixedSizeExemplarReservoir` with a reservoir equal to the smaller of the maximum number of buckets configured on the aggregation or twenty (e... | `src/Метрики/Классы/ОтелМетр.os:179` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 125 | MUST | ✅ found | This reservoir MUST use a uniformly-weighted sampling algorithm based on the number of samples the reservoir has seen so far to determine if the offered measurements should be sampled. | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:97` |  |
| 126 | SHOULD | ✅ found | Any stateful portion of sampling computation SHOULD be reset every collection cycle. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:163` |  |
| 127 | SHOULD | ✅ found | Otherwise, a default size of `1` SHOULD be used. | `src/Метрики/Классы/ОтелФабрикаПростыхРезервуаров.os:43` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 128 | MUST | ✅ found | This Exemplar reservoir MUST take a configuration parameter that is the configuration of a Histogram. | `src/Метрики/Классы/ОтелВыровненныйРезервуарГистограммы.os:178` |  |
| 129 | MUST | ✅ found | This implementation MUST store at most one measurement that falls within a histogram bucket, and SHOULD use a uniformly-weighted sampling algorithm based on the number of measurements the bucket has se... | `src/Метрики/Классы/ОтелВыровненныйРезервуарГистограммы.os:74` |  |
| 130 | SHOULD | ✅ found | This implementation MUST store at most one measurement that falls within a histogram bucket, and SHOULD use a uniformly-weighted sampling algorithm based on the number of measurements the bucket has se... | `src/Метрики/Классы/ОтелВыровненныйРезервуарГистограммы.os:55` |  |
...
| 133 | MUST | ✅ found | This extension MUST be configurable on a metric View, although individual reservoirs MUST still be instantiated per metric-timeseries (see Exemplar Reservoir - Paragraph 2). | `src/Метрики/Классы/ОтелПредставление.os:128` |  |
| 134 | MUST | ✅ found | This extension MUST be configurable on a metric View, although individual reservoirs MUST still be instantiated per metric-timeseries (see Exemplar Reservoir - Paragraph 2). | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:432` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 135 | SHOULD | ⚠️ partial | `Collect` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:94` | СброситьБуфер - процедура без возврата статуса; ошибки экспорта только логируются. Нет способа вызывающему отличить успех/ошибку/таймаут. |
...
| 138 | SHOULD | ⚠️ partial | SDKs SHOULD return some failure for these calls, if possible. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:94` | СброситьБуфер - Процедура без возвращаемого значения; после Закрыть подкласс Экспортер вернёт Ложь, но читатель не пробрасывает результат вызывающей стороне. |
| 139 | SHOULD | ⚠️ partial | `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:111` | Закрыть - Процедура без возвращаемого значения; таймаут ожидания фонового задания логируется, но статус успех/неудача/таймаут не возвращается вызывающей стороне. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 141 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD collect metrics, split into batches if necessary, call `Export(batch)` on each batch and `ForceFlush()` on the configured Push Metric Exporter. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:94` | СброситьБуфер вызывает СобратьИЭкспортировать (что вызывает Экспортер.Экспортировать), но НЕ вызывает Экспортер.СброситьБуфер() на push-экспортере; разбиение на батчи отсутствует. |
| 142 | SHOULD | ❌ not_found | `ForceFlush` MAY skip `Export(batch)` calls if the timeout is already expired, but SHOULD still call `ForceFlush()` on the configured Push Metric Exporter even if the timeout has passed. | - | Нет вызова Экспортер.СброситьБуфер() из СброситьБуфер читателя; механизма таймаута также нет. |
| 143 | SHOULD | ❌ not_found | `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | - | СброситьБуфер - Процедура без возвращаемого значения; невозможно отличить успех от неудачи/таймаута. |
| 144 | SHOULD | ❌ not_found | `ForceFlush` SHOULD return some ERROR status if there is an error condition; and if there is no error condition, it should return some NO ERROR status, language implementations MAY decide how to model... | - | СброситьБуфер не возвращает значение; статус ошибки/успеха недоступен. |
| 145 | SHOULD | ❌ not_found | `ForceFlush` SHOULD complete or abort within some timeout. | - | СброситьБуфер не принимает таймаут и не прерывает операцию по таймауту. |
...
| 147 | SHOULD | ⚠️ partial | Metric Exporters SHOULD report an error condition for data output by the `MetricReader` with unsupported Aggregation or Aggregation Temporality, as this condition can be corrected by a change of `Metr... | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:250` | СформироватьМетрикуOtlp молча пропускает неизвестные ТипМетрики (нет ветки else с ошибкой); явного сигнала об unsupported Aggregation/Temporality в МетricReader не происходит. |
...
| 151 | MUST | ✅ found | `Export` MUST NOT block indefinitely, there MUST be a reasonable upper limit after which the call must time out with an error result (Failure). | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:49` |  |
...
| 153 | SHOULD | ✅ found | This is a hint to ensure that the export of any `Metrics` the exporter has received prior to the call to `ForceFlush` SHOULD be completed as soon as possible, preferably before returning from this met... | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:58` |  |
| 154 | SHOULD | ❌ not_found | `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | - | Экспортер.СброситьБуфер - Процедура без возвращаемого значения; статус succeed/failed/timed out не сообщается. |
| 155 | SHOULD | ✅ found | `ForceFlush` SHOULD only be called in cases where it is absolutely necessary, such as when using some FaaS providers that may suspend the process after an invocation, but before the exporter exports t... | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:58` |  |
| 156 | SHOULD | ✅ found | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:58` |  |
| 157 | SHOULD | ✅ found | Shutdown SHOULD be called only once for each `MetricExporter` instance. | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:64` |  |
...
| 160 | SHOULD | ❌ not_found | `MetricProducer` implementations SHOULD accept configuration for the `AggregationTemporality` of produced metrics. | - | ИнтерфейсПродюсерМетрик.Произвести(ФильтрМетрик) не принимает конфигурацию AggregationTemporality. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 163 | MUST | ✅ found | The SDK MUST provide configuration according to the SDK environment variables specification. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:431` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 164 | MUST | ✅ found | The SDK MUST handle numerical limits in a graceful way according to Error handling in OpenTelemetry. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:125` |  |
| 165 | MUST | ➖ n_a | If the SDK receives float/double values from Instruments, it MUST handle all the possible values. | - | OneScript использует System.Decimal (28 значащих цифр), а не IEEE 754 float/double. NaN, Infinity, отрицательный ноль платформенно невозможны - арифметические операции выбрасывают исключение, которое уже перехватывается в Попытка/Исключение в Записать (ОтелБазовыйСинхронныйИнструмент.os:125-130). Требование об IEEE 754 не применимо к платформе. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 168 | MUST | ⚠️ partial | MeterProvider - Meter creation, `ForceFlush` and `Shutdown` MUST be safe to be called concurrently. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:59` | Закрыт реализован через АтомарноеБулево с CAS (СравнитьИУстановить) - Закрыть идемпотентен и безопасен. Однако ПолучитьМетр (Meter creation) и СброситьБуфер обращаются к коллекциям Метрики/ЧитателиМетрик/Представления без явной БлокировкиРесурса - возможны гонки при параллельном создании метров из ФоновыхЗаданий. |
...
| 171 | MUST | ⚠️ partial | MetricExporter - `ForceFlush` and `Shutdown` MUST be safe to be called concurrently. | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:58` | Методы СброситьБуфер и Закрыть в ОтелЭкспортерМетрик существуют как простые делегирующие процедуры, но не имеют явной БлокировкиРесурса для защиты от параллельного вызова - в отличие от ОтелПериодическийЧитательМетрик, где блокировка реализована. |
...
| 2 | MUST | ✅ found | Each configuration option MUST be overridable by a signal specific option. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:909` |  |
| 3 | MUST | ✅ found | The implementation MUST honor the following URL components: scheme (`http` or `https`), host, port, path | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:776` |  |
...
| 5 | SHOULD | ✅ found | The option SHOULD accept any form allowed by the underlying gRPC client implementation. | `src/Экспорт/Классы/ОтелGrpcТранспорт.os:184` |  |
| 6 | MUST | ✅ found | Additionally, the option MUST accept a URL with a scheme of either `http` or `https`. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:785` |  |
| 7 | SHOULD | ✅ found | If the gRPC client implementation does not support an endpoint with a scheme of `http` or `https` then the endpoint SHOULD be transformed to the most sensible format for that implementation. | `src/Экспорт/Классы/ОтелGrpcТранспорт.os:184` |  |
| 8 | MUST | ✅ found | Protocol: The transport protocol. Options MUST be one of: `grpc`, `http/protobuf`, `http/json`. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:612` |  |
| 9 | SHOULD | ✅ found | SDKs SHOULD default endpoint variables to use `http` scheme unless they have good reasons to choose `https` scheme for the default (e.g., for backward compatibility reasons in a stable SDK release). | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:200` |  |
| 10 | SHOULD | ❌ not_found | However, if they are already implemented, they SHOULD continue to be supported as they were part of a stable release of the specification. | - | Устаревшие переменные OTEL_EXPORTER_OTLP_SPAN_INSECURE и OTEL_EXPORTER_OTLP_METRIC_INSECURE не обрабатываются автоконфигурацией; используются только канонические OTEL_EXPORTER_OTLP_INSECURE/TRACES/LOGS/METRICS_INSECURE. |
| 11 | SHOULD | ❌ not_found | The default protocol SHOULD be `http/protobuf`, unless there are strong reasons for SDKs to select `grpc` as the default. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:197` | Значение по умолчанию для otel.exporter.otlp.protocol = "http/json" (см. ПараметрИлиУмолчание и ПараметрСигналаИлиОбщий, строки 197 и 610), а не "http/protobuf". |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 12 | MUST | ✅ found | Based on the environment variables above, the OTLP/HTTP exporter MUST construct URLs for each signal as follow: | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:651` |  |
...
| 14 | MUST | ✅ found | The only exception is that if an URL contains no path part, the root path `/` MUST be used (see Example 2). | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:928` |  |
| 15 | MUST NOT | ✅ found | An SDK MUST NOT modify the URL in ways other than specified above. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:649` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 16 | SHOULD | ⚠️ partial | SDKs SHOULD support both `grpc` and `http/protobuf` transports and MUST support at least one of them. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:612` | grpc реализован полностью (ОтелGrpcТранспорт). Однако "http/protobuf" принимается как значение настройки, но ОтелHttpТранспорт всегда сериализует тело как JSON (Content-Type: application/json), то есть фактической поддержки protobuf-кодирования по HTTP нет. |
| 17 | MUST | ✅ found | SDKs SHOULD support both `grpc` and `http/protobuf` transports and MUST support at least one of them. | `src/Экспорт/Классы/ОтелGrpcТранспорт.os:48` |  |
| 18 | SHOULD | ✅ found | If they support only one, it SHOULD be `http/protobuf`. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:612` |  |
| 19 | SHOULD | ❌ not_found | If no configuration is provided the default transport SHOULD be `http/protobuf` unless SDKs have good reasons to choose `grpc` as the default. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:610` | Значение по умолчанию для otel.exporter.otlp.protocol = "http/json" (ПараметрСигналаИлиОбщий с дефолтом "http/json"), а не "http/protobuf". |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 20 | MUST | ✅ found | All attribute values MUST be considered strings. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:742` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 23 | SHOULD | ❌ not_found | OpenTelemetry protocol exporters SHOULD emit a User-Agent header to at a minimum identify the exporter, the language of its implementation, and the version of the exporter. | - | В ОтелHttpТранспорт.ПриСозданииОбъекта устанавливаются только Content-Type и (при сжатии) Content-Encoding. Заголовок User-Agent не формируется и не отправляется. ОтелGrpcТранспорт также не передаёт user-agent метаданные. |
| 24 | SHOULD | ❌ not_found | The format of the header SHOULD follow RFC 7231. | - | Заголовок User-Agent не формируется (см. предыдущее требование), поэтому соответствие RFC 7231 не реализовано. |
| 25 | SHOULD | ❌ not_found | The resulting User-Agent SHOULD include the exporter's default User-Agent string. | - | Нет ни дефолтной строки User-Agent, ни конфигурационной опции для добавления product identifier. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 4 | MUST NOT | ✅ found | If a value can not be parsed from the carrier, for a cross-cutting concern, the implementation MUST NOT throw an exception and MUST NOT store a new value in the `Context`, in order to preserve any previously... | `src/Пропагация/Классы/ОтелW3CПропагатор.os:81` |  |
| 5 | MUST NOT | ✅ found | If a value can not be parsed from the carrier, for a cross-cutting concern, the implementation MUST NOT throw an exception and MUST NOT store a new value in the `Context`, in order to preserve any previously... | `src/Пропагация/Классы/ОтелW3CПропагатор.os:89` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 6 | MUST | ✅ found | In order to increase compatibility, the key-value pairs MUST only consist of US-ASCII characters that make up valid HTTP header fields as per RFC 9110. | `src/Пропагация/Классы/ОтелW3CПропагатор.os:63` |  |
| 7 | MUST | ✅ found | `Getter` and `Setter` MUST be stateless and allowed to be saved as constants, in order to effectively avoid runtime allocations. | `src/Пропагация/Классы/ОтелСеттерТекстовойКарты.os:26` |  |
...
| 9 | MUST | ✅ found | The implementation SHOULD preserve casing (e.g. it should not transform `Content-Type` to `content-type`) if the used protocol is case insensitive, otherwise it MUST preserve casing. | `src/Пропагация/Классы/ОтелСеттерТекстовойКарты.os:18` |  |
...
| 22 | SHOULD | ✅ found | If pre-configured, `Propagator`s SHOULD default to a composite `Propagator` containing the W3C Trace Context Propagator and the Baggage `Propagator` specified in the Baggage API. | `src/Ядро/Модули/ОтелГлобальный.os:121` | OneScript не относится к платформам с pre-configured пропагаторами (как ASP.NET); условие 'If pre-configured' не выполняется, поэтому требование вакуумно соблюдается. Пропагаторы W3C Trace Context и W3C Baggage реализованы и доступны для явной композиции через ОтелПостроительSdk.УстановитьПропагаторы. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 30 | MUST | ⚠️ partial | A W3C Trace Context propagator MUST parse and validate the `traceparent` and `tracestate` HTTP headers as specified in W3C Trace Context Level 2. | `src/Пропагация/Классы/ОтелW3CПропагатор.os:81` | Парсинг и валидация traceparent и tracestate реализованы (включая ОтелСостояниеТрассировки.Разобрать с проверкой ключей/значений). Однако явного соответствия именно W3C Trace Context Level 2 (например, обработки специфичных Level 2 полей/ошибок) в коде не отражено — реализация ближе к Level 1. |
...
| 34 | MUST | ✅ found | MUST preserve a debug trace flag, if received, and propagate it with subsequent requests. | `src/Пропагация/Классы/ОтелB3Пропагатор.os:154` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ✅ found | If they do, they SHOULD use the names and value parsing behavior specified in this document. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:91` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 4 | MUST | ✅ found | The SDK MUST interpret an empty value of an environment variable the same way as when the variable is unset. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:869` |  |
...
| 7 | MUST | ✅ found | Any value not explicitly defined here as a true value, including unset and empty values, MUST be interpreted as false. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:852` |  |
| 8 | SHOULD | ❌ not_found | If any value other than a true value, case-insensitive string "false", empty, or unset is used, a warning SHOULD be logged to inform users about the fallback to false being applied. | - | Парсинг булевых значений (например, otel.sdk.disabled, insecure) выполняется выражением НРег(Значение) = "true" без отдельной проверки на распознанные значения {true, false, empty, unset} и без логирования предупреждения для нераспознанных значений (например, "yes", "1"). |
...
| 10 | MUST NOT | ✅ found | Renaming or changing the default value MUST NOT happen without a major version upgrade. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:852` |  |
...
| 13 | SHOULD | ✅ found | For variables accepting a numeric value, if the user provides a value the implementation cannot parse, the implementation SHOULD generate a warning and gracefully ignore the setting, i.e., treat them as not set. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:886` |  |
...
| 15 | MUST | ✅ found | For sources accepting an enum value, if the user provides a value the implementation does not recognize, the implementation MUST generate a warning and gracefully ignore the setting. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:292` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 20 | SHOULD | ✅ found | Implementations SHOULD only offer environment variables for the types of attributes, for which that SDK implements truncation mechanism. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:546` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ➖ n_a | Resource detectors SHOULD have a unique name for reference in configuration. | - | Условная фича Resource Detector Naming не реализована: классы детекторов (ОтелДетекторРесурсаХоста, ОтелДетекторРесурсаПроцесса, ОтелДетекторРесурсаПроцессора) не имеют поля/метода Имя и не регистрируются по имени в декларативной конфигурации. |
| 2 | SHOULD | ➖ n_a | Names SHOULD be snake case and consist of lowercase alphanumeric and `_` characters, which ensures they conform to declarative configuration property name requirements. | - | Условная фича Resource Detector Naming не реализована: у детекторов нет имён, к которым применимо требование snake_case. |
| 3 | SHOULD | ➖ n_a | Resource detector names SHOULD reflect the root namespace of attributes they populate. | - | Условная фича Resource Detector Naming не реализована: у детекторов нет имён, которые могли бы отражать корневой namespace атрибутов. |
| 4 | SHOULD | ➖ n_a | Resource detectors which populate attributes from multiple root namespaces SHOULD choose a name which appropriately conveys their purpose. | - | Условная фича Resource Detector Naming не реализована: имена детекторам не присваиваются. |
| 5 | SHOULD | ➖ n_a | An SDK which identifies multiple resource detectors with the same name SHOULD report an error. | - | Условная фича Resource Detector Naming не реализована: SDK не идентифицирует детекторы по имени, поэтому проверка коллизий имён неприменима. |
| 6 | SHOULD | ➖ n_a | In order to limit collisions, resource detectors SHOULD document their name in a manner which is easily discoverable. | - | Условная фича Resource Detector Naming не реализована: имена детекторам не присваиваются и не документируются. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | The function MUST accept the following parameter: `tracer_scope`: The `InstrumentationScope` of the `Tracer`. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:338` |  |
| 2 | MUST | ✅ found | The function MUST return the relevant `TracerConfig`, or some signal indicating that the default TracerConfig should be used. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:333` |  |
...
| 2 | MUST | ✅ found | If the `TracerProvider` supports updating the TracerConfigurator, then upon update the `Tracer` MUST be updated to behave according to the new `TracerConfig`. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:344` |  |
...
| 4 | MUST | ✅ found | However, the changes MUST be eventually visible. | `src/Трассировка/Классы/ОтелТрассировщик.os:206` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ⚠️ partial | `Enabled` MUST return `false` when either: there are no registered `SpanProcessors`, Status: Development - `Tracer` is disabled (`TracerConfig.enabled` is `false`). | `src/Трассировка/Классы/ОтелТрассировщик.os:47` | Метод Включен() возвращает Ложь, если Конфигурация.Включен()=Ложь (TracerConfig.enabled=false) либо Провайдер.Включен()=Ложь, но НЕ проверяет, что список зарегистрированных SpanProcessors пуст - провайдер может иметь нулевые процессоры и всё равно возвращать Истина. |
| 2 | SHOULD | ✅ found | Otherwise, it SHOULD return `true`. | `src/Трассировка/Классы/ОтелТрассировщик.os:51` |  |
...
| 6 | MUST | ✅ found | implementations MUST use a deterministic hash of the `TraceId` when computing the sampling decision. | `src/Трассировка/Модули/ОтелСэмплер.os:290` |  |
...
| 8 | SHOULD | ❌ not_found | When this sampler observes a non-empty parent span context, meaning when it is used not as a root sampler, the SDK SHOULD emit a warning such as: | - | Логика TraceIdRatioBased в ОтелСэмплер не различает корневой/дочерний контексты и не эмитит предупреждение при использовании в качестве дочернего сэмплера. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ❌ not_found | The `ProbabilitySampler` sampler MUST ignore the parent `SampledFlag`. | - | Сэмплер ProbabilitySampler в проекте не реализован. ОтелСэмплер содержит только ВсегдаВключен/ВсегдаВыключен/ПоДолеТрассировок/НаОсновеРодителя; нет отдельного ProbabilitySampler с алгоритмом R>=T на 56-битной случайности по W3C Trace Context Level 2. |
| 2 | SHOULD | ❌ not_found | When (R >= T), the OpenTelemetry TraceState SHOULD be modified to include the key-value `th:T` for rejection threshold value (T), as specified for the OpenTelemetry TraceState `th` sub-key. | - | ProbabilitySampler не реализован; автоматическая запись `th:T` в TraceState при решении сэмплирования отсутствует. ОтелСостояниеТрассировки поддерживает поле `th` как формат, но сэмплер его не устанавливает. |
| 3 | SHOULD | ❌ not_found | When a ProbabilitySampler Sampler makes a decision for a non-root Span using TraceID randomness when the Trace random flag was not set, the SDK SHOULD issue a warning statement in its log with... | - | ProbabilitySampler не реализован, флаг Trace random (0x02) также не используется; механизм выдачи warning при отсутствии random flag отсутствует. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST NOT | ❌ not_found | ComposableSamplers MUST NOT modify the parameters passed to delegate GetSamplingIntent methods, as they are considered read-only state. | - | CompositeSampler/ComposableSampler с методом GetSamplingIntent в SDK не реализован; соответственно требование о неизменяемости параметров делегирования не применимо к коду — сущности нет. |
| 2 | MUST NOT | ❌ not_found | ComposableSamplers MUST NOT modify the OpenTelemetry TraceState (i.e., the `ot` sub-key of TraceState). | - | ComposableSampler не реализован; механизм запрета модификации `ot` sub-key TraceState через композитные сэмплеры отсутствует, так как сами сэмплеры отсутствуют. |
| 3 | SHOULD | ❌ not_found | The calling CompositeSampler SHOULD update the threshold of the outgoing TraceState (unless `!threshold_reliable`) and that the explicit randomness values MUST not be modified. | - | CompositeSampler не реализован — обновление threshold в исходящем TraceState через композитную обёртку отсутствует. |
| 4 | MUST | ❌ not_found | The calling CompositeSampler SHOULD update the threshold of the outgoing TraceState (unless `!threshold_reliable`) and that the explicit randomness values MUST not be modified. | - | CompositeSampler не реализован; гарантия неизменяемости explicit randomness (`rv`) при работе композитного сэмплера отсутствует. |
| 5 | SHOULD | ❌ not_found | For the zero case a `ComposableAlwaysOff` instance SHOULD be returned instead. | - | ComposableProbability/ComposableAlwaysOff не реализованы; кейс ratio=0 в составной модели отсутствует. (В ОтелСэмплер.СэмплироватьПоДоле при Доля<=0 возвращается DROP — это другая, не композитная ветка реализации.) |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ❌ not_found | Custom implementations of the `IdGenerator` SHOULD identify themselves appropriately when all generated TraceID values meet the W3C Trace Context Level 2 randomness requirements, so that the Trace `random` flag will be set in the associated Trace contexts. | - | Нет marker-интерфейса или иного способа для IdGenerator идентифицировать себя как генератор криптослучайных TraceID; Random flag в trace flags не выставляется на основании свойства IdGenerator. |
...
| 4 | MUST | ⚠️ partial | The SDK MUST guarantee that the span can no longer be modified by any other thread before invoking `OnEnding` of the first `SpanProcessor`. | `src/Трассировка/Классы/ОтелСпан.os:459` | Явного механизма thread-safe заморозки спана перед ПередЗавершением нет; OneScript обычно однопоточный в пределах скрипта, но SDK не использует блокировок/барьеров для гарантии этого требования при многопоточном использовании через ФоновыеЗадания. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ❌ not_found | The ergonomic API SHOULD make it more convenient to emit event records following the event semantics. | - | Эргономичный API для эмита event records (например, EmitEvent / EventBuilder) не реализован. SDK предоставляет только базовый Logger.Записать(ЗаписьЛога, Контекст) и УстановитьИмяСобытия на ОтелЗаписьЛога; отдельного удобного API для событийной семантики нет. |
| 2 | SHOULD | ❌ not_found | The design of the ergonomic API SHOULD be idiomatic for its language. | - | Эргономичный API не реализован, поэтому идиоматичность дизайна для OneScript оценить нельзя. |
...
| 4 | MUST | ✅ found | In the case where an invalid `name` (null or empty string) is specified, a working `Logger` MUST be returned as a fallback rather than returning null or throwing an exception, its `name` SHOULD keep the o... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:61` |  |
| 5 | SHOULD | ⚠️ partial | In the case where an invalid `name` (null or empty string) is specified, a working `Logger` MUST be returned as a fallback rather than returning null or throwing an exception, its `name` SHOULD keep the o... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:63` | При ИмяБиблиотеки = Неопределено код заменяет на пустую строку (Неопределено → ''), не сохраняя исходное значение Неопределено. Для пустой строки оригинальное значение сохраняется. |
| 6 | SHOULD | ✅ found | In the case where an invalid `name` (null or empty string) is specified, a working `Logger` MUST be returned as a fallback rather than returning null or throwing an exception, its `name` SHOULD keep the o... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:62` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | Logger MUST behave according to the LoggerConfig computed during logger creation. | `src/Логирование/Классы/ОтелЛоггер.os:60` |  |
...
| 7 | MUST | ✅ found | If trace_based is true, log records associated with unsampled traces MUST be dropped by the Logger. | `src/Логирование/Классы/ОтелЛоггер.os:186` |  |
| 8 | MUST | ✅ found | However, the changes MUST be eventually visible. | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:244` |  |
...
| 5 | MUST | ✅ found | Before processing a log record, the implementation MUST apply the filtering rules defined by the LoggerConfig (in case Enabled was not called prior to emitting the record). | `src/Логирование/Классы/ОтелЛоггер.os:112` |  |
...
| 7 | MUST | ✅ found | Trace-based: If trace_based is true, and if the log record has a SpanId and the TraceFlags SAMPLED flag is unset, the log record MUST be dropped. | `src/Логирование/Классы/ОтелЛоггер.os:205` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | Enabled MUST return false when either: there are no registered LogRecordProcessors, Logger is disabled, severity is less than minimum_severity, trace_based and unsampled, or all process... | `src/Логирование/Классы/ОтелЛоггер.os:49` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ❌ not_found | This processor SHOULD be provided by SDK. | - | Процессор Event-to-span-event-bridge не реализован в src/Логирование/. |
| 2 | MUST | ❌ not_found | The processor MUST bridge a `LogRecord` to a span event if and only if all of the following conditions are met: | - | Процессор-мост Event→span event отсутствует в SDK. |
| 3 | MUST | ❌ not_found | If any of these conditions is not met, the processor MUST do nothing. | - | Процессор-мост Event→span event отсутствует в SDK. |
| 4 | MUST | ❌ not_found | When a `LogRecord` is bridged, the processor MUST add exactly one span event with the following mapping: | - | Процессор-мост Event→span event отсутствует в SDK. |
| 5 | MUST | ❌ not_found | the span event name MUST be the `LogRecord`'s Event Name. | - | Процессор-мост Event→span event отсутствует в SDK. |
| 6 | MUST | ❌ not_found | if the `LogRecord` has a Timestamp set, it MUST be used as the span event timestamp. | - | Процессор-мост Event→span event отсутствует в SDK. |
| 7 | MUST | ❌ not_found | Otherwise, if the `LogRecord` has an ObservedTimestamp set, it MUST be used as the span event timestamp. | - | Процессор-мост Event→span event отсутствует в SDK. |
| 8 | MUST | ❌ not_found | all `LogRecord` Attributes MUST be copied to the span event as span event attributes. | - | Процессор-мост Event→span event отсутствует в SDK. |
| 9 | MUST NOT | ❌ not_found | Note that bridging a `LogRecord` to a span event MUST NOT prevent that `LogRecord` from continuing through the normal log processing pipeline. | - | Процессор-мост Event→span event отсутствует в SDK. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ✅ found | It SHOULD only be possible to create `Meter` instances through a `MeterProvider` (see API). | `src/Метрики/Классы/ОтелПровайдерМетрик.os:44` |  |
...
| 5 | SHOULD | ⚠️ partial | In the case where an invalid `name` (null or empty string) is specified, a working Meter MUST be returned as a fallback rather than returning null or throwing an excepti... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:66` | При передаче Неопределено имя нормализуется в пустую строку (стр. 67), исходное Неопределено не сохраняется. Пустая строка сохраняется. Полное соответствие требованию не достигнуто. |
| 6 | SHOULD | ✅ found | In the case where an invalid `name` (null or empty string) is specified, a working Meter MUST be returned as a fallback rather than returning null or throwing an excepti... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:65` |  |
| 7 | MUST | ✅ found | Status: Development - The `MeterProvider` MUST compute the relevant MeterConfig using the configured MeterConfigurator, and create a `Meter` whose behavior conforms to t... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:260` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | The function MUST accept the following parameter: | `src/Метрики/Классы/ОтелПровайдерМетрик.os:264` |  |
| 2 | MUST | ✅ found | The function MUST return the relevant `MeterConfig`, or some signal indicating that the default MeterConfig should be used. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:265` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | For delta aggregations, the start timestamp MUST equal the previous collection interval’s timestamp, or the creation time of the instrument if this is the first collection interval for the instrument. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:171` |  |
...
| 3 | MUST | ✅ found | Cumulative timeseries MUST use a consistent start timestamp for all collection intervals. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:170` |  |
| 4 | SHOULD | ⚠️ partial | For synchronous instruments, the start timestamp SHOULD be the time of the first measurement for the series. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:321` | ВремяСтарта устанавливается в момент создания инструмента (ПриСозданииОбъекта), а не в момент первого измерения серии; для разных серий внутри одного инструмента используется единое ВремяСтарта. |
| 5 | SHOULD | ⚠️ partial | For asynchronous instrument, the start timestamp SHOULD be: | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:188` | В ПреобразоватьЗаписиВТочки startTimeUnixNano выставляется равным ВремяСейчас (время сбора), а не времени создания инструмента и не времени предыдущего интервала сбора. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | Distinct meters MUST be treated as separate namespaces for the purposes of detecting duplicate instrument registrations. | `src/Метрики/Классы/ОтелМетр.os:606` |  |
...
| 2 | MUST | ⚠️ partial | If a `Meter` is disabled, it MUST behave equivalently to No-op Meter. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:88` | Записать проверяет только инструмент-уровневый Включен, не МетрВключен. Хотя Включен(Атрибуты) корректно возвращает совмещённое состояние (МетрВключен AND Включен), при выключенном Meter измерения всё равно записываются в агрегаторы (нет проверки МетрВключен на горячем пути Записать). |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ⚠️ partial | The synchronous instrument `Enabled` MUST return `false` when either: ... The MeterConfig of the `Meter` used to create the instrument has parameter `enabled=false`. ... All resolved views for the instrument are configured with the Drop Aggregation. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:259-261` | Включен() проверяет МетрВключен (MeterConfig.enabled=false → Ложь) и собственный флаг Включен. Однако случай 'все View сконфигурированы с Drop Aggregation' не приводит автоматически к false: ОтелАгрегаторDrop назначается агрегатором, но Включен() не учитывает агрегатор инструмента. |
| 2 | SHOULD | ✅ found | Otherwise, it SHOULD return `true`. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:259-261` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ✅ found | To construct a `MetricReader` when setting up an SDK, at least the following SHOULD be provided: | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:398` |  |
| 2 | SHOULD | ⚠️ partial | This function SHOULD be obtained from the `exporter`. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:419` | Селектор агрегации задаётся внутренней инициализацией ИнициализироватьСелекторАгрегации, не получается из экспортёра. По умолчанию SDK использует фиксированный набор. |
| 3 | SHOULD | ✅ found | If not configured, the default aggregation SHOULD be used. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:173` |  |
...
| 9 | MUST | ⚠️ partial | The `MetricReader` MUST ensure that data points from OpenTelemetry instruments are output in the configured aggregation temporality for each instrument kind. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:163` | Для синхронных инструментов конверсия Delta→Cumulative реализована через сохранение аккумуляторов при Кумулятивной. Для асинхронных инструментов конверсии Cumulative→Delta нет: ВнешниеНаблюдения сбрасываются в каждом цикле без вычитания предыдущего значения. |
| 10 | MUST | ✅ found | For synchronous instruments with Cumulative aggregation temporality, MetricReader.Collect MUST receive data points exposed in previous collections regardless of whether new measurements have been recor... | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:170` |  |
...
| 12 | MUST | ⚠️ partial | For asynchronous instruments with Delta or Cumulative aggregation temporality, MetricReader.Collect MUST only receive data points with measurements recorded since the previous collection. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:114` | Для асинхронных инструментов ВнешниеНаблюдения очищаются после сбора, но Cumulative-вариант не вычитает предыдущие значения - всегда выдаются последние наблюдения без поддержки разных временностей. |
| 13 | MUST | ✅ found | For instruments with Cumulative aggregation temporality, successive data points received by successive calls to MetricReader.Collect MUST repeat the same starting timestamps (e.g. `(T0, T1], (T0, T2], ... | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:170` |  |
| 14 | MUST | ✅ found | For instruments with Delta aggregation temporality, successive data points received by successive calls to MetricReader.Collect MUST advance the starting timestamp (e.g. `(T0, T1], (T1, T2], (T2, T3]`). | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:174` |  |
...
| 16 | MUST | ✅ found | The SDK MUST support multiple `MetricReader` instances to be registered on the same `MeterProvider`, and the MetricReader.Collect invocation on one `MetricReader` instance SHOULD NOT introduce side-eff... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:120` |  |
| 17 | SHOULD NOT | ⚠️ partial | The SDK MUST support multiple `MetricReader` instances to be registered on the same `MeterProvider`, and the MetricReader.Collect invocation on one `MetricReader` instance SHOULD NOT introduce side-eff... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:128` | СброситьБуферБезОчистки используется для всех читателей кроме последнего, последний выполняет ОчиститьТочкиДанных. Это уменьшает побочные эффекты, но аккумуляторы всё ещё общие на инструмент: первый Collect всё равно влияет на агрегацию, видимую другими читателями (нет per-reader снапшотов). |
...
| 19 | SHOULD | ✅ found | The SDK SHOULD provide a way to allow `MetricReader` to respond to MeterProvider.ForceFlush and MeterProvider.Shutdown. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:128` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ❌ not_found | Status: Development - When `maxExportBatchSize` is configured, the reader MUST ensure no batch provided to `Export` exceeds the `maxExportBatchSize` by splitting the batch of metric data points into smaller bat... | - | Параметр maxExportBatchSize не реализован в ОтелПериодическийЧитательМетрик; разбиение пакетов отсутствует. |
| 2 | MUST | ❌ not_found | The initial batch of metric data MUST be split into as many "full" batches of size `maxExportBatchSize` as possible – even if this splits up data points that belong to the same metric into different bat... | - | maxExportBatchSize не поддерживается, разбиение на full batches не реализовано. |
| 3 | MUST | ✅ found | The reader MUST ensure all metric data points from a single `Collect()` are provided to `Export` before metric data points from a subsequent `Collect()` so that metric points are sent in-order. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:209` |  |
| 4 | MUST NOT | ✅ found | The reader MUST NOT combine metrics from different `Collect()` calls into the same batch provided to `Export`. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:223` |  |
| 5 | MUST | ✅ found | The reader MUST synchronize calls to `MetricExporter`'s `Export` to make sure that they are not invoked concurrently. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:242` |  |
...
| 3 | SHOULD | ❌ not_found | If the batch of Metric Points includes resource information, `Produce` SHOULD require a resource as a parameter. | - | Произвести(ФильтрМетрик) не принимает Resource в качестве параметра. |
| 4 | SHOULD | ⚠️ partial | `Produce` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Метрики/Классы/ИнтерфейсПродюсерМетрик.os:13` | Произвести возвращает Массив без отдельного поля статуса; неудача может быть сообщена только через исключение, таймаут от успеха неотличим. |
| 5 | SHOULD | ⚠️ partial | `Produce` SHOULD include a single InstrumentationScope which identifies the `MetricProducer`. | `src/Метрики/Классы/ИнтерфейсПродюсерМетрик.os:13` | Интерфейс не предписывает и не валидирует включение единственного InstrumentationScope; зависит от реализации продюсера. |
