...
| Найдено требований (Stable universal) | 701 |
| ✅ Реализовано (found) | 629 (89.7%) |
| ⚠️ Частично (partial) | 52 (7.4%) |
| ❌ Не реализовано (not_found) | 20 (2.9%) |
| ➖ Неприменимо (n_a) | 6 |
| **MUST/MUST NOT found** | 419/425 (98.6%) |
| **SHOULD/SHOULD NOT found** | 210/276 (76.1%) |
...
| Trace Api | 116 | 7 | 0 | 3 | 123 | 94.3% |
| Trace Sdk | 73 | 8 | 2 | 0 | 83 | 88.0% |
| Logs Api | 19 | 2 | 0 | 0 | 21 | 90.5% |
| Logs Sdk | 60 | 4 | 1 | 0 | 65 | 92.3% |
| Metrics Api | 89 | 10 | 0 | 1 | 99 | 89.9% |
| Metrics Sdk | 144 | 17 | 9 | 1 | 170 | 84.7% |
| Otlp Exporter | 19 | 0 | 5 | 1 | 24 | 79.2% |
...
- ⚠️ **[Resource Sdk]** [MUST] The SDK MUST extract information from the `OTEL_RESOURCE_ATTRIBUTES` environment variable and merge this, as the secondary resource, with any resource information provided by the user, i.e. the user p...  
  OTEL_RESOURCE_ATTRIBUTES читается и применяется только в составе дефолтной инициализации ресурса (ЗаполнитьАтрибутыПоУмолчанию). Если пользователь передаёт собственный Ресурс через ОтелПостроительПровайдераТрассировки/Метрик.УстановитьРесурс(СвойРесурс), env переменная не сливается с ним. По спеке env должен быть secondary относительно user-provided, но всегда применяться. (`src/Ядро/Классы/ОтелРесурс.os:138`)
...
- ⚠️ **[Logs Api]** [MUST] When implicit Context is supported, then this parameter SHOULD be optional and if unspecified then MUST use current Context.  
  Параметр Контекст опциональный (Неопределено по умолчанию), но в методе Включен текущий контекст разрешается только в ветке TraceBased (через ТрассировкаНеСэмплирована); в общем случае Неопределено передаётся в Процессор.Включен без явного резолва ОтелКонтекст.Текущий(). (`src/Логирование/Классы/ОтелЛоггер.os:49`)
...
- ⚠️ **[Metrics Sdk]** [MUST] The implementation MUST maintain reasonable minimum and maximum scale parameters that the automatic scale parameter will not exceed.  
  Минимальная шкала захардкожена (-10), максимальная ограничена параметром НачальнаяШкала (default 20), но MaxScale как отдельный конфигурируемый параметр спецификации не предусмотрен. (`src/Метрики/Классы/ОтелАгрегаторЭкспоненциальнойГистограммы.os:218`)

- ⚠️ **[Metrics Sdk]** [MUST] Callback functions MUST be invoked for the specific `MetricReader` performing collection, such that observations made or produced by executing callbacks only apply to the intended `MetricReader` during c...  
  Перед вызовом мульти-callback инструменты очищаются, но изоляция per-MetricReader не строгая: если несколько reader-ов конкурентно собирают метрики, наблюдения могут пересекаться. (`src/Метрики/Классы/ОтелМетр.os:565`)

- ⚠️ **[Metrics Sdk]** [MUST] This MUST be clearly documented in the API and the reservoir MUST be given the `Attributes` associated with its timeseries point either at construction so that additional sampling performed by the re...  
  Документирующий комментарий метода Предложить упоминает АтрибутыИзмерения и АтрибутыСерии, но не описывает явно семантику фильтрованного подмножества атрибутов и их расхождения с timeseries-набором. (`src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:30`)

- ⚠️ **[Metrics Sdk]** [MUST] This MUST be clearly documented in the API and the reservoir MUST be given the `Attributes` associated with its timeseries point either at construction so that additional sampling performed by the re...  
  АтрибутыСерии передаются в каждый вызов Предложить(), а не сохраняются в резервуаре при конструировании; функционально эквивалентно, но архитектура отличается от описанной в спеке. (`src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:41`)
...
- ⚠️ **[Baggage Api]** [SHOULD NOT] The functionality listed above is necessary because API users SHOULD NOT have access to the Context Key used by the Baggage API implementation.  
  Функция КлючBaggage() помечена как Экспорт, то есть ключ контекста доступен пользователям API. Это нарушает требование скрытия ключа. (`src/Ядро/Модули/ОтелКонтекст.os:53`)
...
- ⚠️ **[Resource Sdk]** [SHOULD] Note the failure to detect any resource information MUST NOT be considered an error, whereas an error that occurs during an attempt to detect resource information SHOULD be considered an error.  
  Ошибки детектирования логируются на уровне Отладка (Debug), а не Ошибка/Предупреждение — по спеке такие ошибки SHOULD быть error-level. (`src/Ядро/Классы/ОтелДетекторРесурсаХоста.os:25`)

- ⚠️ **[Resource Sdk]** [SHOULD] Empty Schema URL SHOULD be used if the detector does not populate the resource with any known attributes that have a semantic convention or if the detector does not know what attributes it will populate (e.g. the detector...  
  Все встроенные детекторы (host, process, processor) безусловно устанавливают непустой Schema URL '1.29.0'. ПрименитьАтрибутыИзОкружения (env-источник) не задаёт Schema URL — это корректно. Однако правило 'если детектор не знает какие атрибуты он наполнит' для env-чтения не выделено в отдельный детектор. (`src/Ядро/Классы/ОтелДетекторРесурсаХоста.os:18`)
...
- ❌ **[Resource Sdk]** [SHOULD] In case of any error, e.g. failure during the decoding process, the entire environment variable value SHOULD be discarded and an error SHOULD be reported following the Error Handling principles.  
  Декодирование (РаскодироватьСтроку) не обёрнуто в Попытка/Исключение. При ошибке декодирования исключение распространится наверх, а не приведёт к отбрасыванию всей переменной с продолжением работы. (-)

- ❌ **[Resource Sdk]** [SHOULD] In case of any error, e.g. failure during the decoding process, the entire environment variable value SHOULD be discarded and an error SHOULD be reported following the Error Handling principles.  
  Нет логики report-an-error для ошибок парсинга OTEL_RESOURCE_ATTRIBUTES (нет Лог.Ошибка / обработчика ошибок). (-)
...
- ⚠️ **[Trace Api]** [SHOULD] The API SHOULD be documented that instrumentation authors needs to call this API each time they create a new `Span` to ensure they have the most up-to-date response.  
  Метод Включен() имеет документирующий комментарий с описанием двухрежимной семантики, но в нём явно не указано требование вызывать API каждый раз при создании нового Span для получения актуального значения. (`src/Трассировка/Классы/ОтелТрассировщик.os:31`)
...
- ⚠️ **[Trace Api]** [SHOULD NOT] To prevent misuse, implementations SHOULD NOT provide access to a `Span`'s attributes besides its `SpanContext`.  
  Класс ОтелСпан экспортирует методы Атрибуты(), События(), Линки() и др., предоставляя прямой доступ к атрибутам спана из публичного API. Не выделен отдельный read-only интерфейс (ReadableSpan/SpanData) для экспортеров, как рекомендует спецификация. (`src/Трассировка/Классы/ОтелСпан.os:146`)

- ⚠️ **[Trace Api]** [SHOULD] An attempt to set value Unset SHOULD be ignored.  
  Установка Unset явно не игнорируется отдельной проверкой; работает корректно только потому, что Unset->Unset идемпотентно, а Error->Unset и Ok->Unset уже блокируются финальностью Ok и проверкой Error->НеУстановлен. Нет общего ранее-возврата при попытке установить Unset. (`src/Трассировка/Классы/ОтелСпан.os:450`)
...
- ⚠️ **[Trace Api]** [SHOULD NOT] If a new type is required for supporting this operation, it SHOULD NOT be exposed publicly if possible (e.g. by only exposing a function that returns something with the Span interface type).  
  Класс ОтелНоопСпан публичен и инстанцируется напрямую через `Новый ОтелНоопСпан(...)` (например в ОтелB3Пропагатор.os:214, ОтелW3CПропагатор.os:135, ОтелТрассировщик.os:228). OneScript не поддерживает internal/package-private классы, нет фабричной функции, скрывающей тип. (`src/Трассировка/Классы/ОтелНоопСпан.os:272`)

- ⚠️ **[Trace Api]** [SHOULD] If a new type is required to be publicly exposed, it SHOULD be named `NonRecordingSpan`.  
  Класс назван ОтелНоопСпан (NoopSpan), а не NonRecordingSpan. Семантика близка (no-op = non-recording), но спека явно требует имя NonRecordingSpan. (`src/Трассировка/Классы/ОтелНоопСпан.os:272`)

- ⚠️ **[Trace Api]** [SHOULD] Link - Links are immutable and SHOULD be safe for concurrent use by default.  
  Link реализован inline как `Новый Соответствие()` (не отдельный класс), нет документирующего комментария об иммутабельности и потокобезопасности. Соответствие в OneScript не является иммутабельным. (`src/Трассировка/Классы/ОтелСпан.os:409`)
...
- ⚠️ **[Trace Api]** [SHOULD] If the `Span` in the parent `Context` is already non-recording, it SHOULD be returned directly without instantiating a new `Span`.  
  СоздатьНоопСпанВAPIРежиме всегда инстанцирует новый ОтелНоопСпан, даже если родительский Span уже non-recording. Нет проверки `Если Родитель является ОтелНоопСпан Тогда Возврат Родитель`. (`src/Трассировка/Классы/ОтелТрассировщик.os:226`)
...
- ⚠️ **[Trace Sdk]** [SHOULD] After the call to `Shutdown`, subsequent attempts to get a `Tracer` are not allowed. SDKs SHOULD return a valid no-op Tracer for these calls, if possible.  
  После Закрыть() возвращается новый ОтелТрассировщик без Конфигурации; его Включен() делегирует Провайдер.Включен(), который остаётся Истина (ВключенПровайдер не сбрасывается при Закрыть), поэтому возвращаемый Tracer не является истинным no-op — он по-прежнему попытается создавать спаны через уже закрытые процессоры. (`src/Трассировка/Классы/ОтелПровайдерТрассировки.os:82`)

- ⚠️ **[Trace Sdk]** [SHOULD] `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  Закрыть — Процедура (void), не возвращает статус; ошибки внутренних процессоров проглатываются (Лог.Предупреждение в композитном процессоре). Таймаут неотличим от успеха. ЗакрытьАсинхронно возвращает Обещание, но Обещание разрешается без статуса операции. (`src/Трассировка/Классы/ОтелПровайдерТрассировки.os:140`)

- ⚠️ **[Trace Sdk]** [SHOULD] `Shutdown` SHOULD complete or abort within some timeout.  
  Метод Закрыть() на TracerProvider не принимает параметр таймаута; внутренний ОтелКомпозитныйПроцессорСпанов.Закрыть(ТаймаутМс) поддерживает таймаут, но провайдер вызывает Процессор.Закрыть() без передачи таймаута (значение по умолчанию 0 = без ограничения). (`src/Трассировка/Классы/ОтелПровайдерТрассировки.os:140`)

- ⚠️ **[Trace Sdk]** [SHOULD NOT] However, Span Exporter SHOULD NOT receive them unless the Sampled flag was also set.  
  ОтелПростойПроцессорСпанов и ОтелПакетныйПроцессорСпанов передают экспортеру все завершённые спаны (включая RECORD_ONLY с неустановленным флагом Sampled); фильтрации по флагу Sampled перед вызовом Экспортер.Экспортировать() нет. (`src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:42`)
...
- ⚠️ **[Trace Sdk]** [SHOULD NOT] Span Exporters MUST receive those spans which have Sampled flag set to true and they SHOULD NOT receive the ones that do not.  
  Процессоры спанов передают экспортёру и спаны с RECORD_ONLY (Sampled=false), нет отдельного шага фильтрации перед экспортом. (`src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:42`)
...
- ❌ **[Trace Sdk]** [SHOULD] For root span contexts, the SDK SHOULD set the `Random` flag in the trace flags when it generates TraceIDs that meet the W3C Trace Context Level 2 randomness requirements.  
  ВычислитьФлагиТрассировки в ОтелТрассировщик.os:303 устанавливает только бит 0 (sampled=1) для корневых спанов; бит 1 (маска 0x02 — Random) никогда не выставляется, несмотря на использование UUID-идентификаторов с достаточной случайностью. (-)
...
- ⚠️ **[Trace Sdk]** [SHOULD] For all span contexts, OpenTelemetry samplers SHOULD presume that TraceIDs meet the W3C Trace Context Level 2 randomness requirements, unless an explicit randomness value is present in the `rv` sub-key of the OpenTelemetry TraceState.  
  Сэмплер TraceIdRatioBased презюмирует случайность, используя TraceId для решения. Однако нет логики проверки sub-key 'rv' в TraceState - explicit randomness не используется при наличии. (`src/Трассировка/Модули/ОтелСэмплер.os:158`)
...
- ❌ **[Trace Sdk]** [SHOULD] If the SDK uses an `IdGenerator` extension point, the SDK SHOULD allow the extension to determine whether the Random flag is set when new IDs are generated.  
  Интерфейс IdGenerator (УстановитьГенераторИд) поддерживает только генерацию ID, но не предоставляет механизма для расширения сообщить SDK об установке Random flag (бит 1) в TraceFlags. Флаг Random обрабатывается только в W3C-пропагаторе при extract. (-)

- ⚠️ **[Trace Sdk]** [SHOULD] The name of the configuration options SHOULD be `EventCountLimit` and `LinkCountLimit`.  
  Поля называются МаксСобытий (MaxEvents) и МаксЛинков (MaxLinks), что семантически близко, но не точно соответствует EventCountLimit/LinkCountLimit (отсутствует часть 'Count'). (`src/Трассировка/Классы/ОтелЛимитыСпана.os:34`)

- ⚠️ **[Trace Sdk]** [SHOULD] `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  Закрыть - процедура (void), не возвращает результат. Невозможно отличить успех от таймаута или ошибки на стороне вызывающего. (`src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:83`)
...
- ⚠️ **[Logs Api]** [SHOULD] The API SHOULD be documented that instrumentation authors needs to call this API each time they emit a LogRecord to ensure they have the most up-to-date response.  
  Документирующий комментарий есть и поясняет назначение API (пропустить создание LogRecord), но не указывает явно, что значение динамическое и инструментирующий код должен вызывать API каждый раз перед эмиссией для актуальности результата. (`src/Логирование/Классы/ОтелЛоггер.os:28`)

- ⚠️ **[Logs Sdk]** [SHOULD] `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  Закрыть() - процедура без возвращаемого значения; ЗакрытьАсинхронно() возвращает Обещание, но статус успех/ошибка/таймаут на уровне провайдера не различается (синхронная версия не сообщает таймаут). (`src/Логирование/Классы/ОтелПровайдерЛогирования.os:139`)

- ⚠️ **[Logs Sdk]** [SHOULD] `Shutdown` SHOULD complete or abort within some timeout.  
  ОтелПровайдерЛогирования.Закрыть() не принимает параметр таймаута и вызывает Процессор.Закрыть() без таймаута. КомпозитныйПроцессорЛогов.Закрыть(ТаймаутМс) поддерживает таймаут, но провайдер его не пробрасывает. (`src/Логирование/Классы/ОтелПровайдерЛогирования.os:139`)
...
- ⚠️ **[Logs Sdk]** [SHOULD NOT] This method is called synchronously on the thread that emitted the `LogRecord`, therefore it SHOULD NOT block or throw exceptions.  
  ПриПоявлении выполняется синхронно. ОтелКомпозитныйПроцессорЛогов.ПриПоявлении ловит исключения процессоров, но сам ОтелПростойПроцессорЛогов.ПриПоявлении при ошибке экспорта пробрасывает исключение наружу (ВызватьИсключение). Гарантии 'не блокирует/не бросает' для индивидуального процессора нет. (`src/Логирование/Классы/ОтелПростойПроцессорЛогов.os:23`)
...
- ❌ **[Logs Sdk]** [SHOULD] To avoid such race conditions, implementations SHOULD recommended to users that a clone of `logRecord` be used for any concurrent processing, such as in a batching processor.  
  Ни в комментариях ОтелПакетныйПроцессорЛогов / ИнтерфейсПроцессорЛогов, ни в документации проекта нет рекомендации пользователям клонировать ЗаписьЛога перед параллельной обработкой. (-)

- ⚠️ **[Logs Sdk]** [SHOULD] `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  Закрыть реализован как Процедура (void) и не возвращает статус успеха/ошибки/таймаута. Асинхронный вариант ЗакрытьАсинхронно возвращает Обещание, но также без статуса операции. (`src/Логирование/Классы/ОтелПростойПроцессорЛогов.os:79`)
...
- ⚠️ **[Metrics Api]** [SHOULD] The API SHOULD be documented in a way to communicate to users that the `name` parameter needs to conform to the instrument name syntax.  
  Doc-комментарий метода СоздатьСчетчик описывает имя как 'имя счетчика', но не упоминает требования к синтаксису instrument name. (`src/Метрики/Классы/ОтелМетр.os:42`)

- ⚠️ **[Metrics Api]** [SHOULD NOT] The API SHOULD NOT validate the `unit`.  
  УстановитьЕдиницаИзмерения проверяет, что строка содержит только печатные ASCII-символы; не-ASCII значения сбрасываются в пустую строку с предупреждением — это валидация, противоречащая SHOULD NOT. (`src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:277`)

- ⚠️ **[Metrics Api]** [SHOULD NOT] The API SHOULD NOT validate `advisory` parameters.  
  ПроверитьСовет валидирует тип Совет (должна быть Структура) и типы Границы/Ключи (должны быть Массивами), логируя предупреждения — это валидация, противоречащая SHOULD NOT. (`src/Метрики/Классы/ОтелМетр.os:853`)

- ⚠️ **[Metrics Api]** [SHOULD] The API SHOULD be documented in a way to communicate to users that the `name` parameter needs to conform to the instrument name syntax.  
  Doc-комментарий метода СоздатьНаблюдаемыйСчетчик описывает имя как 'имя инструмента', но не упоминает требования к синтаксису instrument name. (`src/Метрики/Классы/ОтелМетр.os:268`)

- ⚠️ **[Metrics Api]** [SHOULD NOT] The API SHOULD NOT validate the `unit`.  
  УстановитьЕдиницаИзмерения проверяет ASCII-печатность и сбрасывает не-ASCII в пустую строку с предупреждением — это валидация, противоречащая SHOULD NOT. (`src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:142`)

- ⚠️ **[Metrics Api]** [SHOULD NOT] The API SHOULD NOT validate `advisory` parameters.  
  ПроверитьСовет валидирует типы Совет/Границы/Ключи и логирует предупреждения — это валидация, противоречащая SHOULD NOT. (`src/Метрики/Классы/ОтелМетр.os:853`)
...
- ⚠️ **[Metrics Api]** [SHOULD] The API SHOULD be documented that instrumentation authors needs to call this API each time they record a measurement to ensure they have the most up-to-date response.  
  Doc-комментарий описывает семантику Включен(), но не содержит явного указания вызывать API перед каждой записью измерения для актуальности ответа. (`src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:248`)

- ⚠️ **[Metrics Api]** [SHOULD NOT] This API SHOULD NOT validate this value, that is left to implementations of the API.  
  Метод Добавить проверяет 'Если Значение < 0 Тогда Возврат' — отрицательные значения тихо отбрасываются на уровне API; это форма валидации, противоречащая SHOULD NOT. (`src/Метрики/Классы/ОтелСчетчик.os:22`)

- ⚠️ **[Metrics Api]** [SHOULD] The API SHOULD provide some way to pass `state` to the callback.  
  Явного параметра state у callback нет; передача состояния возможна только через захват замыкания лямбды (#Использовать lambdas). Это допустимый идиоматический подход по спецификации, но как отдельная фича API — не реализован. (`src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:65`)
...
- ⚠️ **[Metrics Api]** [SHOULD] This API SHOULD be documented in a way to communicate to users that this value is expected to be non-negative.  
  Документирующий комментарий процедуры Записать описывает параметр Значение как «измеренное значение», но не упоминает, что значение должно быть неотрицательным. (`src/Метрики/Классы/ОтелГистограмма.os:13`)

- ⚠️ **[Metrics Sdk]** [SHOULD] `Shutdown` SHOULD complete or abort within some timeout.  
  Метод Закрыть() не принимает параметр таймаута. Есть асинхронная обертка ЗакрытьАсинхронно(), но без явного механизма прерывания по таймауту в самом MeterProvider. (`src/Метрики/Классы/ОтелПровайдерМетрик.os:180`)

- ⚠️ **[Metrics Sdk]** [SHOULD] If it is not possible to apply the View without producing semantic errors (e.g. the View sets an asynchronous instrument to use the Explicit bucket histogram aggregation) the implementation SHOULD emit a warni...  
  Нет специальной проверки несовместимости: при попытке назначить histogram-агрегацию async-инструменту через View агрегатор просто заменяется без предупреждения. (`src/Метрики/Классы/ОтелМетр.os:936`)
...
- ⚠️ **[Metrics Sdk]** [SHOULD NOT] Arithmetic sum of `Measurement` values in population. This SHOULD NOT be collected when used with instruments that record negative measurements (e.g. `UpDownCounter` or `ObservableGauge`).  
  Sum собирается безусловно. Нет проверки типа инструмента и пропуска sum при UpDownCounter/ObservableGauge. (`src/Метрики/Классы/ОтелАгрегаторГистограммы.os:65`)

- ❌ **[Metrics Sdk]** [SHOULD] The implementation SHOULD use a timeout to prevent indefinite callback execution.  
  Callback'и async-инструментов выполняются синхронно без таймаута: ВызватьМультиОбратныеВызовы и ВызватьCallbackИСобрать просто вызывают .Выполнить() без ограничения времени. (-)
...
- ⚠️ **[Metrics Sdk]** [SHOULD] If there is no matching view, but the `MetricReader` defines a default cardinality limit value based on the instrument an aggregation is created for, that value SHOULD be used.  
  MetricReader.ЛимитМощности() применяется ко всем инструментам Meter единым числом; нет дифференциации лимита по типу инструмента (per-instrument-type default cardinality). (`src/Метрики/Классы/ОтелПровайдерМетрик.os:290`)
...
- ❌ **[Metrics Sdk]** [SHOULD] Aggregators of asynchronous instruments SHOULD prefer the first-observed attributes in the callback when limiting cardinality, regardless of temporality.  
  В ОтелБазовыйНаблюдаемыйИнструмент.ВызватьCallbackИСобрать() лимит мощности для асинхронных инструментов не реализован: все наблюдения преобразуются в точки данных без cardinality-ограничения и без предпочтения first-observed атрибутов. (-)
...
- ⚠️ **[Metrics Sdk]** [SHOULD] The emitted warning SHOULD include information for the user on how to resolve the conflict, if possible.  
  Текст предупреждения сообщает имя, вид и единицу измерения конфликта, но не содержит конкретных рекомендаций (renaming View, изменение description) по разрешению конфликта. (`src/Метрики/Классы/ОтелМетр.os:784`)
...
- ❌ **[Metrics Sdk]** [SHOULD] If the potential conflict involves multiple `description` properties, setting the `description` through a configured View SHOULD avoid the warning.  
  ПроверитьКонфликтДескриптора сравнивает Описание непосредственно с зарегистрированным дескриптором без учёта View с заданным новым description; нет логики, подавляющей предупреждение, если View переопределяет description. (-)
...
- ❌ **[Metrics Sdk]** [SHOULD] If the potential conflict involves instruments that can be distinguished by a supported View selector (e.g. name, instrument kind) a renaming View recipe SHOULD be included in the warning.  
  Текст Лог.Предупреждение в ПроверитьКонфликтДескриптора не содержит примера/recipe конфигурации View для разрешения конфликта. (-)
...
- ❌ **[Metrics Sdk]** [SHOULD] Otherwise (e.g., use of multiple units), the SDK SHOULD pass through the data by reporting both `Metric` objects and emit a generic warning describing the duplicate instrument registration.  
  При повторной регистрации возвращается ранее зарегистрированный инструмент (Возврат Существующий), второй инструмент с другой единицей измерения не создаётся и не экспортируется отдельной метрикой. (-)
...
- ❌ **[Metrics Sdk]** [SHOULD] When a Meter creates an instrument, it SHOULD validate the instrument name conforms to the instrument name syntax.  
  ОтелМетр.СоздатьСчетчик/СоздатьГистограмму/etc. не выполняют валидацию имени инструмента (нет проверки регулярным выражением [a-zA-Z][a-zA-Z0-9_./-]{0,254}). Имя принимается как есть. (-)
...
- ❌ **[Metrics Sdk]** [SHOULD] If the instrument name does not conform to this syntax, the Meter SHOULD emit an error notifying the user about the invalid name.  
  Поскольку валидация имени отсутствует, ошибка о невалидном имени не эмитируется. (-)

- ⚠️ **[Metrics Sdk]** [SHOULD] The `ExemplarReservoir` SHOULD avoid allocations when sampling exemplars.  
  Метод СоздатьЭкземпляр аллоцирует новое Соответствие на каждое принятое измерение и пересчитывает отфильтрованные атрибуты; явных оптимизаций по предотвращению аллокаций нет. (`src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:121`)

- ⚠️ **[Metrics Sdk]** [SHOULD] SDKs SHOULD return some failure for these calls, if possible.  
  После Закрыть() флаг Закрыт=Истина, но СброситьБуфер()/СобратьИЭкспортировать() не проверяют этот флаг и могут отработать повторно без возврата признака отказа. (`src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:106`)

- ⚠️ **[Metrics Sdk]** [SHOULD] `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  Закрыть() — Процедура (void), не возвращает статус успеха/ошибки/таймаута. (`src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:126`)
...
- ⚠️ **[Metrics Sdk]** [SHOULD] `ForceFlush` SHOULD collect metrics, split into batches if necessary, call `Export(batch)` on each batch and `ForceFlush()` on the configured Push Metric Exporter.  
  СброситьБуфер() читателя собирает и вызывает Экспортировать(), но НЕ вызывает СброситьБуфер() (ForceFlush) на экспортёре после экспорта. (`src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:106`)
...
- ⚠️ **[Metrics Sdk]** [SHOULD] `ForceFlush` MAY skip `Export(batch)` calls if the timeout is already expired, but SHOULD still call `ForceFlush()` on the configured Push Metric Exporter even if the timeout has passed.  
  Не реализована логика таймаута и отдельного вызова ForceFlush() на экспортёре по истечении таймаута. (`src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:106`)

- ⚠️ **[Metrics Sdk]** [SHOULD] `ForceFlush` SHOULD complete or abort within some timeout.  
  Параметр ТаймаутМс присутствует, но помечен как UnusedParameters — таймаут на уровне reader не enforced (не прерывает выполнение по истечении). (`src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:106`)
...
- ❌ **[Metrics Sdk]** [SHOULD] Metric Exporters SHOULD report an error condition for data output by the `MetricReader` with unsupported Aggregation or Aggregation Temporality, as this condition can be corrected by a change of `Met...  
  ОтелЭкспортерМетрик не проверяет совместимость агрегации/временности данных и не сигнализирует об ошибке для неподдерживаемых типов. (-)

- ⚠️ **[Metrics Sdk]** [SHOULD] `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out.  
  СброситьБуфер() возвращает ОтелРезультатыЭкспорта.Успех() безусловно (no-op), статус таймаута/ошибки не дифференцируется. (`src/Экспорт/Классы/ОтелЭкспортерМетрик.os:66`)

- ⚠️ **[Metrics Sdk]** [SHOULD] `ForceFlush` SHOULD complete or abort within some timeout.  
  Параметр ТаймаутМс есть, но помечен UnusedParameters — таймаут не учитывается; реализация — синхронный no-op (тривиально завершается). (`src/Экспорт/Классы/ОтелЭкспортерМетрик.os:66`)
...
- ❌ **[Metrics Sdk]** [SHOULD] `MetricProducer` implementations SHOULD accept configuration for the `AggregationTemporality` of produced metrics.  
  Интерфейс ИнтерфейсПродюсерМетрик.Произвести(ФильтрМетрик) не принимает и нет конфигурации AggregationTemporality. (-)
...
- ❌ **[Otlp Exporter]** [SHOULD] The default protocol SHOULD be http/protobuf, unless there are strong reasons for SDKs to select grpc as the default.  
  Дефолт протокола в коде - 'http/json' (см. ПротоколHttpJson = "http/json" и default параметра 'protocol'), а спека требует 'http/protobuf'. Хотя http/protobuf поддерживается в списке допустимых, по умолчанию выбран http/json. (`src/Конфигурация/Модули/ОтелАвтоконфигурация.os:630`)

- ❌ **[Otlp Exporter]** [SHOULD] If no configuration is provided the default transport SHOULD be http/protobuf unless SDKs have good reasons to choose grpc as the default (e.g. for backward compatibility reasons when grpc was already the de...  
  Default protocol установлен в 'http/json' (ПараметрСигналаИлиОбщий(... 'protocol', ПротоколHttpJson)), а не 'http/protobuf' как требует спецификация. (`src/Конфигурация/Модули/ОтелАвтоконфигурация.os:630`)
...
- ❌ **[Otlp Exporter]** [SHOULD] OpenTelemetry protocol exporters SHOULD emit a User-Agent header to at a minimum identify the exporter, the language of its implementation, and the version of the exporter.  
  В коде HTTP/gRPC транспортов (ОтелHttpТранспорт, ОтелGrpcТранспорт) не задаётся заголовок User-Agent. Поиск 'User-Agent' / 'UserAgent' по src/ не дал совпадений. (-)

- ❌ **[Otlp Exporter]** [SHOULD] The format of the header SHOULD follow RFC 7231.  
  User-Agent заголовок не формируется, формат RFC 7231 не применяется. (-)

- ❌ **[Otlp Exporter]** [SHOULD] The resulting User-Agent SHOULD include the exporter's default User-Agent string.  
  Default User-Agent строки нет, поэтому при добавлении product identifier (тоже не реализовано) включать нечего. (-)
...
- ❌ **[Env Vars]** [SHOULD] If any value other than a true value, case-insensitive string "false", empty, or unset is used, a warning SHOULD be logged to inform users about the fallback to false being applied.  
  Boolean-парсинг (НРег(Значение) = "true") молча трактует любое не-"true" значение как false без логирования предупреждения для значений, не совпадающих с "false"/empty/unset. (-)
...
| 8 | MUST | ✅ found | The API MUST return the value in the `Context` for the specified key. | `src/Ядро/Модули/ОтелКонтекст.os:113` |  |
...
| 2 | MUST | ✅ found | When associated with a `TracerProvider`, all `Span`s produced by any `Tracer` from the provider MUST be associated with this `Resource`. | `src/Трассировка/Классы/ОтелТрассировщик.os:108` |  |
...
| 14 | SHOULD | ⚠️ partial | Empty Schema URL SHOULD be used if the detector does not populate the resource with any known attributes that have a semantic convention or if the detector does not know what attributes it will populate (e.g. the detector... | `src/Ядро/Классы/ОтелДетекторРесурсаХоста.os:18` | Все встроенные детекторы (host, process, processor) безусловно устанавливают непустой Schema URL '1.29.0'. ПрименитьАтрибутыИзОкружения (env-источник) не задаёт Schema URL — это корректно. Однако правило 'если детектор не знает какие атрибуты он наполнит' для env-чтения не выделено в отдельный детектор. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 16 | MUST | ⚠️ partial | The SDK MUST extract information from the `OTEL_RESOURCE_ATTRIBUTES` environment variable and merge this, as the secondary resource, with any resource information provided by the user, i.e. the user p... | `src/Ядро/Классы/ОтелРесурс.os:138` | OTEL_RESOURCE_ATTRIBUTES читается и применяется только в составе дефолтной инициализации ресурса (ЗаполнитьАтрибутыПоУмолчанию). Если пользователь передаёт собственный Ресурс через ОтелПостроительПровайдераТрассировки/Метрик.УстановитьРесурс(СвойРесурс), env переменная не сливается с ним. По спеке env должен быть secondary относительно user-provided, но всегда применяться. |
| 17 | MUST | ✅ found | All attribute values MUST be considered strings. | `src/Ядро/Классы/ОтелРесурс.os:149` |  |
| 18 | MUST | ✅ found | The `,` and `=` characters in keys and values MUST be percent encoded. | `src/Ядро/Классы/ОтелРесурс.os:146` |  |
| 19 | SHOULD | ❌ not_found | In case of any error, e.g. failure during the decoding process, the entire environment variable value SHOULD be discarded and an error SHOULD be reported following the Error Handling principles. | - | Декодирование (РаскодироватьСтроку) не обёрнуто в Попытка/Исключение. При ошибке декодирования исключение распространится наверх, а не приведёт к отбрасыванию всей переменной с продолжением работы. |
| 20 | SHOULD | ❌ not_found | In case of any error, e.g. failure during the decoding process, the entire environment variable value SHOULD be discarded and an error SHOULD be reported following the Error Handling principles. | - | Нет логики report-an-error для ошибок парсинга OTEL_RESOURCE_ATTRIBUTES (нет Лог.Ошибка / обработчика ошибок). |
...
| 6 | MUST | ✅ found | In case an invalid name (null or empty string) is specified, a working Tracer implementation MUST be returned as a fallback rather than returning null or throwing an exception. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:74` |  |
| 7 | SHOULD | ✅ found | its `name` property SHOULD be set to an empty string. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:76` |  |
| 8 | SHOULD | ✅ found | and a message reporting that the specified value is invalid SHOULD be logged. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:75` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 10 | MUST | ✅ found | The API MUST provide the following functionality to interact with a `Context` instance: Extract the `Span` from a `Context` instance, Combine the `Span` with a `Context` instance, creating a new `Context` ins... | `src/Ядро/Модули/ОтелКонтекст.os:141` |  |
| 11 | SHOULD NOT | ✅ found | The functionality listed above is necessary because API users SHOULD NOT have access to the Context Key used by the Tracing API implementation. | `src/Ядро/Модули/ОтелКонтекст.os:141` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 14 | MUST | ✅ found | The `Tracer` MUST provide functions to: Create a new `Span` | `src/Трассировка/Классы/ОтелТрассировщик.os:27` |  |
...
| 18 | MUST | ✅ found | This API MUST return a language idiomatic boolean type. | `src/Трассировка/Классы/ОтелТрассировщик.os:45` |  |
| 19 | SHOULD | ⚠️ partial | The API SHOULD be documented that instrumentation authors needs to call this API each time they create a new `Span` to ensure they have the most up-to-date response. | `src/Трассировка/Классы/ОтелТрассировщик.os:31` | Метод Включен() имеет документирующий комментарий с описанием двухрежимной семантики, но в нём явно не указано требование вызывать API каждый раз при создании нового Span для получения актуального значения. |
...
| 23 | SHOULD NOT | ✅ found | This functionality MUST be fully implemented in the API, and SHOULD NOT be overridable. | `src/Трассировка/Классы/ОтелКонтекстСпана.os:1` | OneScript-классы не поддерживают наследование/переопределение пользовательским кодом, поэтому функциональность SpanContext не может быть переопределена. |
...
| 25 | MUST | ✅ found | Hex - returns the lowercase hex encoded `TraceId` (result MUST be a 32-hex-character lowercase string) or `SpanId` (result MUST be a 16-hex-character lowercase string). | `src/Трассировка/Классы/ОтелКонтекстСпана.os:176` |  |
...
| 43 | SHOULD | ✅ found | A `Span`'s start time SHOULD be set to the current time on span creation. | `src/Трассировка/Классы/ОтелСпан.os:680` |  |
| 44 | SHOULD | ✅ found | After the `Span` is created, it SHOULD be possible to change its name, set its `Attribute`s, add `Event`s, and set the `Status`. | `src/Трассировка/Классы/ОтелСпан.os:272` |  |
...
| 46 | SHOULD NOT | ⚠️ partial | To prevent misuse, implementations SHOULD NOT provide access to a `Span`'s attributes besides its `SpanContext`. | `src/Трассировка/Классы/ОтелСпан.os:146` | Класс ОтелСпан экспортирует методы Атрибуты(), События(), Линки() и др., предоставляя прямой доступ к атрибутам спана из публичного API. Не выделен отдельный read-only интерфейс (ReadableSpan/SpanData) для экспортеров, как рекомендует спецификация. |
| 47 | MUST NOT | ➖ n_a | However, alternative implementations MUST NOT allow callers to create `Span`s directly. | `src/Трассировка/Классы/ОтелСпан.os:622` | OneScript не поддерживает приватные конструкторы (ПриСозданииОбъекта всегда публичен). Ограничение задокументировано в комментарии класса ОтелСпан и в docs/spec-compliance.md. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 49 | MUST NOT | ➖ n_a | There MUST NOT be any API for creating a `Span` other than with a `Tracer`. | `src/Трассировка/Классы/ОтелСпан.os:622` | OneScript не поддерживает приватные конструкторы; ПриСозданииОбъекта класса ОтелСпан всегда публичен. Ограничение задокументировано в комментарии класса и в docs/spec-compliance.md; пользовательский код должен создавать спаны только через ОтелТрассировщик. |
...
| 56 | MUST NOT | ✅ found | If API is called at a moment of a Span logical start, API user MUST NOT explicitly set this argument. | `src/Трассировка/Классы/ОтелПостроительСпана.os:109` |  |
...
| 60 | MUST | ✅ found | Also, the child span MUST inherit all `TraceState` values of its parent by default. | `src/Трассировка/Классы/ОтелТрассировщик.os:325` |  |
| 61 | MUST | ✅ found | Any span that is created MUST also be ended. | `src/Трассировка/Классы/ОтелСпан.os:484` |  |
...
| 80 | SHOULD | ✅ found | The status code SHOULD remain unset, except for the following circumstances: | `src/Трассировка/Классы/ОтелСпан.os:450` |  |
| 81 | SHOULD | ⚠️ partial | An attempt to set value Unset SHOULD be ignored. | `src/Трассировка/Классы/ОтелСпан.os:450` | Установка Unset явно не игнорируется отдельной проверкой; работает корректно только потому, что Unset->Unset идемпотентно, а Error->Unset и Ok->Unset уже блокируются финальностью Ok и проверкой Error->НеУстановлен. Нет общего ранее-возврата при попытке установить Unset. |
...
| 83 | SHOULD | ✅ found | For operations not covered by the semantic conventions, Instrumentation Libraries SHOULD publish their own conventions, including possible values of Description and what they mean. | `src/Трассировка/Классы/ОтелСпан.os:450` |  |
| 84 | SHOULD NOT | ✅ found | Generally, Instrumentation Libraries SHOULD NOT set the status code to Ok, unless explicitly configured to do so. | `src/Трассировка/Классы/ОтелСпан.os:450` |  |
| 85 | SHOULD | ✅ found | Instrumentation Libraries SHOULD leave the status code as Unset unless there is an error, as described above. | `src/Трассировка/Классы/ОтелСпан.os:450` |  |
...
| 88 | SHOULD | ➖ n_a | Analysis tools SHOULD respond to an Ok status by suppressing any errors they would otherwise generate. | - | Требование адресовано инструментам анализа/визуализации (downstream-системы), а не SDK/API. Вне области реализации OpenTelemetry SDK для OneScript. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 89 | SHOULD | ✅ found | Implementations SHOULD ignore all subsequent calls to End and any other Span methods, i.e. the Span becomes non-recording by being ended (there might be exceptions when Tracer is streaming events and ha... | `src/Трассировка/Классы/ОтелСпан.os:484` |  |
...
| 94 | MUST | ✅ found | Also, any mechanisms for putting the Span into a Context MUST still work after the Span was ended. | `src/Трассировка/Классы/ОтелСпан.os:435` |  |
...
| 102 | SHOULD | ✅ found | If `RecordException` is provided, the method MUST accept an optional parameter to provide any additional event attributes (this SHOULD be done in the same way as for the `AddEvent` method). | `src/Трассировка/Классы/ОтелСпан.os:365` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 103 | MUST | ✅ found | Start and end time as well as Event's timestamps MUST be recorded at a time of a calling of corresponding API. | `src/Трассировка/Классы/ОтелСпан.os:680` |  |
...
| 105 | SHOULD NOT | ⚠️ partial | If a new type is required for supporting this operation, it SHOULD NOT be exposed publicly if possible (e.g. by only exposing a function that returns something with the Span interface type). | `src/Трассировка/Классы/ОтелНоопСпан.os:272` | Класс ОтелНоопСпан публичен и инстанцируется напрямую через `Новый ОтелНоопСпан(...)` (например в ОтелB3Пропагатор.os:214, ОтелW3CПропагатор.os:135, ОтелТрассировщик.os:228). OneScript не поддерживает internal/package-private классы, нет фабричной функции, скрывающей тип. |
| 106 | SHOULD | ⚠️ partial | If a new type is required to be publicly exposed, it SHOULD be named `NonRecordingSpan`. | `src/Трассировка/Классы/ОтелНоопСпан.os:272` | Класс назван ОтелНоопСпан (NoopSpan), а не NonRecordingSpan. Семантика близка (no-op = non-recording), но спека явно требует имя NonRecordingSpan. |
...
| 115 | MUST | ✅ found | The API MUST provide: An API to record a single `Link` where the `Link` properties are passed as arguments. | `src/Трассировка/Классы/ОтелСпан.os:386` |  |
...
| 118 | MUST | ✅ found | The API documentation MUST state that adding links at span creation is preferred to calling `AddLink` later, for contexts that are available during span creation, because head sampling decisions can only consider information present d... | `src/Трассировка/Классы/ОтелПостроительСпана.os:86` |  |
...
| 123 | SHOULD | ⚠️ partial | Link - Links are immutable and SHOULD be safe for concurrent use by default. | `src/Трассировка/Классы/ОтелСпан.os:409` | Link реализован inline как `Новый Соответствие()` (не отдельный класс), нет документирующего комментария об иммутабельности и потокобезопасности. Соответствие в OneScript не является иммутабельным. |
...
| 125 | SHOULD | ⚠️ partial | If the `Span` in the parent `Context` is already non-recording, it SHOULD be returned directly without instantiating a new `Span`. | `src/Трассировка/Классы/ОтелТрассировщик.os:226` | СоздатьНоопСпанВAPIРежиме всегда инстанцирует новый ОтелНоопСпан, даже если родительский Span уже non-recording. Нет проверки `Если Родитель является ОтелНоопСпан Тогда Возврат Родитель`. |
...
| 2 | MUST | ✅ found | If configuration is updated (e.g., adding a `SpanProcessor`), the updated configuration MUST also apply to all already returned `Tracers` | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:100` |  |
...
| 5 | SHOULD | ⚠️ partial | After the call to `Shutdown`, subsequent attempts to get a `Tracer` are not allowed. SDKs SHOULD return a valid no-op Tracer for these calls, if possible. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:82` | После Закрыть() возвращается новый ОтелТрассировщик без Конфигурации; его Включен() делегирует Провайдер.Включен(), который остаётся Истина (ВключенПровайдер не сбрасывается при Закрыть), поэтому возвращаемый Tracer не является истинным no-op — он по-прежнему попытается создавать спаны через уже закрытые процессоры. |
| 6 | SHOULD | ⚠️ partial | `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:140` | Закрыть — Процедура (void), не возвращает статус; ошибки внутренних процессоров проглатываются (Лог.Предупреждение в композитном процессоре). Таймаут неотличим от успеха. ЗакрытьАсинхронно возвращает Обещание, но Обещание разрешается без статуса операции. |
| 7 | SHOULD | ⚠️ partial | `Shutdown` SHOULD complete or abort within some timeout. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:140` | Метод Закрыть() на TracerProvider не принимает параметр таймаута; внутренний ОтелКомпозитныйПроцессорСпанов.Закрыть(ТаймаутМс) поддерживает таймаут, но провайдер вызывает Процессор.Закрыть() без передачи таймаута (значение по умолчанию 0 = без ограничения). |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 12 | MUST | ✅ found | Readable span: A function receiving this as argument MUST be able to access all information that was added to the span, as listed in the API spec for Span. | `src/Трассировка/Классы/ОтелСпан.os:146` |  |
...
| 15 | MUST | ✅ found | A function receiving this as argument MUST be able to reliably determine whether the Span has ended (some languages might implement this by having an end timestamp of null, others might have an explicit hasEnded boolean). | `src/Трассировка/Классы/ОтелСпан.os:222` |  |
...
| 20 | SHOULD NOT | ⚠️ partial | However, Span Exporter SHOULD NOT receive them unless the Sampled flag was also set. | `src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:42` | ОтелПростойПроцессорСпанов и ОтелПакетныйПроцессорСпанов передают экспортеру все завершённые спаны (включая RECORD_ONLY с неустановленным флагом Sampled); фильтрации по флагу Sampled перед вызовом Экспортер.Экспортировать() нет. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 24 | MUST | ✅ found | When asked to create a Span, the SDK MUST act as if doing the following in order: If there is a valid parent trace ID, use it. Otherwise generate a new trace ID ... Query the Sampler's ShouldSample method. | `src/Трассировка/Классы/ОтелТрассировщик.os:65` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 25 | MUST | ✅ found | TraceId of the Span to be created. If the parent SpanContext contains a valid TraceId, they MUST always match. | `src/Трассировка/Классы/ОтелТрассировщик.os:77` |  |
...
| 28 | SHOULD | ✅ found | If the sampler returns an empty Tracestate here, the Tracestate will be cleared, so samplers SHOULD normally return the passed-in Tracestate if they do not intend to change it. | `src/Трассировка/Модули/ОтелСэмплер.os:158` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 31 | MUST | ✅ found | Description MUST be `AlwaysOffSampler`. | `src/Трассировка/Модули/ОтелСэмплер.os:122` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 32 | MUST | ✅ found | Based on the decision from the wrapped root sampler, `AlwaysRecord` MUST behave as follows: | `src/Трассировка/Модули/ОтелСэмплер.os:284` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 33 | SHOULD | ✅ found | For root span contexts, the SDK SHOULD implement the TraceID randomness requirements of the W3C Trace Context Level 2 Candidate Recommendation when generating TraceID values. | `src/Ядро/Модули/ОтелУтилиты.os:82` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 34 | SHOULD | ❌ not_found | For root span contexts, the SDK SHOULD set the `Random` flag in the trace flags when it generates TraceIDs that meet the W3C Trace Context Level 2 randomness requirements. | - | ВычислитьФлагиТрассировки в ОтелТрассировщик.os:303 устанавливает только бит 0 (sampled=1) для корневых спанов; бит 1 (маска 0x02 — Random) никогда не выставляется, несмотря на использование UUID-идентификаторов с достаточной случайностью. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 37 | SHOULD | ❌ not_found | If the SDK uses an `IdGenerator` extension point, the SDK SHOULD allow the extension to determine whether the Random flag is set when new IDs are generated. | - | Интерфейс IdGenerator (УстановитьГенераторИд) поддерживает только генерацию ID, но не предоставляет механизма для расширения сообщить SDK об установке Random flag (бит 1) в TraceFlags. Флаг Random обрабатывается только в W3C-пропагаторе при extract. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 38 | MUST | ✅ found | Span attributes MUST adhere to the common rules of attribute limits. | `src/Трассировка/Классы/ОтелЛимитыСпана.os:53` |  |
...
| 40 | SHOULD | ⚠️ partial | The name of the configuration options SHOULD be `EventCountLimit` and `LinkCountLimit`. | `src/Трассировка/Классы/ОтелЛимитыСпана.os:34` | Поля называются МаксСобытий (MaxEvents) и МаксЛинков (MaxLinks), что семантически близко, но не точно соответствует EventCountLimit/LinkCountLimit (отсутствует часть 'Count'). |
| 41 | SHOULD | ✅ found | The options MAY be bundled in a class, which then SHOULD be called `SpanLimits`. | `src/Трассировка/Классы/ОтелЛимитыСпана.os:169` |  |
| 42 | SHOULD | ✅ found | There SHOULD be a message printed in the SDK's log to indicate to the user that an attribute, event, or link was discarded due to such a limit. | `src/Трассировка/Классы/ОтелСпан.os:507` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 44 | MUST | ✅ found | The SDK MUST by default randomly generate both the `TraceId` and the `SpanId`. | `src/Ядро/Модули/ОтелУтилиты.os:82` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 48 | MUST | ✅ found | SDK MUST allow to end each pipeline with individual exporter. | `src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:6` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 54 | MUST | ✅ found | This method MUST be called synchronously within the `Span.End()` API, therefore it should not block or throw an exception. | `src/Трассировка/Классы/ОтелСпан.os:496` |  |
...
| 57 | SHOULD | ⚠️ partial | `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:83` | Закрыть - процедура (void), не возвращает результат. Невозможно отличить успех от таймаута или ошибки на стороне вызывающего. |
| 58 | MUST | ✅ found | `Shutdown` MUST include the effects of `ForceFlush`. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:86` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 60 | SHOULD | ✅ found | This is a hint to ensure that any tasks associated with `Spans` for which the `SpanProcessor` had already received events prior to the call to `ForceFlush` SHOULD be completed as soon as possible, preferably ... | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:74` |  |
| 61 | SHOULD | ✅ found | In particular, if any `SpanProcessor` has any associated exporter, it SHOULD try to call the exporter's `Export` with all spans for which this was not already done and then invoke `ForceFlush` on it. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:155` |  |
| 62 | MUST | ✅ found | The built-in SpanProcessors MUST do so. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:155` |  |
| 63 | MUST | ✅ found | If a timeout is specified (see below), the SpanProcessor MUST prioritize honoring the timeout over finishing all calls. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:133` |  |
...
| 65 | SHOULD | ✅ found | `ForceFlush` SHOULD only be called in cases where it is absolutely necessary, such as when using some FaaS providers that may suspend the process after an invocation, but before the `SpanProcessor` exports th... | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:74` |  |
| 66 | SHOULD | ✅ found | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:133` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 68 | MUST | ✅ found | The processor MUST synchronize calls to `Span Exporter`'s `Export` to make sure that they are not invoked concurrently. | `src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:48` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 69 | MUST | ✅ found | The processor MUST synchronize calls to `Span Exporter`'s `Export` to make sure that they are not invoked concurrently. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:160` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 72 | MUST | ✅ found | The exporter MUST support three functions: Export, Shutdown, and ForceFlush. | `src/Экспорт/Классы/ОтелЭкспортерСпанов.os:32` |  |
...
| 75 | SHOULD NOT | ✅ found | The default SDK's Span Processors SHOULD NOT implement retry logic, as the required logic is likely to depend heavily on the specific protocol and backend the spans are being sent to. | `src/Трассировка/Классы/ОтелПростойПроцессорСпанов.os:48` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 76 | SHOULD | ✅ found | This is a hint to ensure that the export of any `Spans` the exporter has received prior to the call to `ForceFlush` SHOULD be completed as soon as possible, preferably before returning from this method. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:75` |  |
| 77 | SHOULD | ✅ found | `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:75` |  |
| 78 | SHOULD | ✅ found | `ForceFlush` SHOULD only be called in cases where it is absolutely necessary, such as when using some FaaS providers that may suspend the process after an invocation, but before the exporter exports the completed spans. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:69` | Это рекомендация для вызывающего кода (когда вызывать ForceFlush), а не функциональное требование к реализации; метод СброситьБуфер документирован комментарием о принудительной отправке |
| 79 | SHOULD | ✅ found | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:130` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 80 | MUST | ✅ found | Tracer Provider - Tracer creation, `ForceFlush` and `Shutdown` MUST be safe to be called concurrently. | `src/Трассировка/Классы/ОтелПровайдерТрассировки.os:128` |  |
...
| 82 | MUST | ✅ found | Span processor - all methods MUST be safe to be called concurrently. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:42` |  |
| 83 | MUST | ✅ found | Span Exporter - `ForceFlush` and `Shutdown` MUST be safe to be called concurrently. | `src/Экспорт/Классы/ОтелЭкспортерСпанов.os:14` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ✅ found | Thus, the API SHOULD provide a way to set/register and access a global default `LoggerProvider`. | `src/Ядро/Модули/ОтелГлобальный.os:87` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 2 | MUST | ✅ found | The `LoggerProvider` MUST provide the following functions: | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:56` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 5 | MUST | ✅ found | The `Logger` MUST provide a function to: | `src/Логирование/Классы/ОтелЛоггер.os:94` |  |
| 6 | SHOULD | ✅ found | The `Logger` SHOULD provide functions to: | `src/Логирование/Классы/ОтелЛоггер.os:49` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 7 | MUST | ✅ found | The API MUST accept the following parameters: | `src/Логирование/Классы/ОтелЗаписьЛога.os:1` |  |
...
| 14 | MUST | ⚠️ partial | When implicit Context is supported, then this parameter SHOULD be optional and if unspecified then MUST use current Context. | `src/Логирование/Классы/ОтелЛоггер.os:49` | Параметр Контекст опциональный (Неопределено по умолчанию), но в методе Включен текущий контекст разрешается только в ветке TraceBased (через ТрассировкаНеСэмплирована); в общем случае Неопределено передаётся в Процессор.Включен без явного резолва ОтелКонтекст.Текущий(). |
| 15 | MUST | ✅ found | This API MUST return a language idiomatic boolean type. | `src/Логирование/Классы/ОтелЛоггер.os:47` |  |
| 16 | SHOULD | ⚠️ partial | The API SHOULD be documented that instrumentation authors needs to call this API each time they emit a LogRecord to ensure they have the most up-to-date response. | `src/Логирование/Классы/ОтелЛоггер.os:28` | Документирующий комментарий есть и поясняет назначение API (пропустить создание LogRecord), но не указывает явно, что значение динамическое и инструментирующий код должен вызывать API каждый раз перед эмиссией для актуальности результата. |
...
| 6 | MUST | ✅ found | If configuration is updated (e.g., adding a `LogRecordProcessor`), the updated configuration MUST also apply to all already returned `Logger`s (i.e. it MUST NOT matter ... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:294` |  |
| 7 | MUST | ✅ found | If configuration is updated (e.g., adding a `LogRecordProcessor`), the updated configuration MUST also apply to all already returned `Logger`s (i.e. it MUST NOT matter ... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:178` |  |
...
| 10 | SHOULD | ⚠️ partial | `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:139` | Закрыть() - процедура без возвращаемого значения; ЗакрытьАсинхронно() возвращает Обещание, но статус успех/ошибка/таймаут на уровне провайдера не различается (синхронная версия не сообщает таймаут). |
| 11 | SHOULD | ⚠️ partial | `Shutdown` SHOULD complete or abort within some timeout. | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:139` | ОтелПровайдерЛогирования.Закрыть() не принимает параметр таймаута и вызывает Процессор.Закрыть() без таймаута. КомпозитныйПроцессорЛогов.Закрыть(ТаймаутМс) поддерживает таймаут, но провайдер его не пробрасывает. |
...
| 14 | SHOULD | ✅ found | `ForceFlush` SHOULD return some ERROR status if there is an error condition; and if there is no error condition, it SHOULD return some NO ERROR status, language impleme... | `src/Логирование/Классы/ОтелКомпозитныйПроцессорЛогов.os:80` |  |
| 15 | SHOULD | ✅ found | `ForceFlush` SHOULD return some ERROR status if there is an error condition; and if there is no error condition, it SHOULD return some NO ERROR status, language impleme... | `src/Логирование/Классы/ОтелКомпозитныйПроцессорЛогов.os:72` |  |
| 16 | SHOULD | ✅ found | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Логирование/Классы/ОтелКомпозитныйПроцессорЛогов.os:70` |  |
| 17 | MUST | ✅ found | `ForceFlush` MUST invoke `ForceFlush` on all registered LogRecordProcessors. | `src/Логирование/Классы/ОтелКомпозитныйПроцессорЛогов.os:73` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 22 | MUST | ✅ found | A function receiving this as an argument MUST additionally be able to modify the following information added to the LogRecord: Timestamp, ObservedTimestamp, SeverityText... | `src/Логирование/Классы/ОтелЗаписьЛога.os:179` |  |
...
| 24 | MUST | ✅ found | If the SDK implements attribute limits it MUST provide a way to change these limits, via a configuration to the LoggerProvider, by allowing users to configure individual... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:276` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 28 | MUST | ✅ found | The SDK MUST allow each pipeline to end with an individual exporter. | `src/Логирование/Классы/ОтелПростойПроцессорЛогов.os:25` |  |
| 29 | MUST | ✅ found | The SDK MUST allow users to implement and configure custom processors and decorate built-in processors for advanced scenarios such as enriching with attributes. | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:99` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 30 | SHOULD NOT | ⚠️ partial | This method is called synchronously on the thread that emitted the `LogRecord`, therefore it SHOULD NOT block or throw exceptions. | `src/Логирование/Классы/ОтелПростойПроцессорЛогов.os:23` | ПриПоявлении выполняется синхронно. ОтелКомпозитныйПроцессорЛогов.ПриПоявлении ловит исключения процессоров, но сам ОтелПростойПроцессорЛогов.ПриПоявлении при ошибке экспорта пробрасывает исключение наружу (ВызватьИсключение). Гарантии 'не блокирует/не бросает' для индивидуального процессора нет. |
...
| 32 | SHOULD | ❌ not_found | To avoid such race conditions, implementations SHOULD recommended to users that a clone of `logRecord` be used for any concurrent processing, such as in a batching processor. | - | Ни в комментариях ОтелПакетныйПроцессорЛогов / ИнтерфейсПроцессорЛогов, ни в документации проекта нет рекомендации пользователям клонировать ЗаписьЛога перед параллельной обработкой. |
...
| 36 | SHOULD | ⚠️ partial | `Shutdown` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Логирование/Классы/ОтелПростойПроцессорЛогов.os:79` | Закрыть реализован как Процедура (void) и не возвращает статус успеха/ошибки/таймаута. Асинхронный вариант ЗакрытьАсинхронно возвращает Обещание, но также без статуса операции. |
| 37 | MUST | ✅ found | `Shutdown` MUST include the effects of `ForceFlush`. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:84` |  |
| 38 | SHOULD | ✅ found | `Shutdown` SHOULD complete or abort within some timeout. | `src/Логирование/Классы/ОтелКомпозитныйПроцессорЛогов.os:99` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 39 | SHOULD | ✅ found | This is a hint to ensure that any tasks associated with `LogRecord`s for which the `LogRecordProcessor` had already received events prior to the call to `ForceFlush` SHOULD be completed as soon as p... | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:78` |  |
| 40 | SHOULD | ✅ found | In particular, if any `LogRecordProcessor` has any associated exporter, it SHOULD try to call the exporter’s `Export` with all `LogRecord`s for which this was not already done and then invoke `ForceFlush` on it. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:155` |  |
| 41 | MUST | ✅ found | The built-in LogRecordProcessors MUST do so. | `src/Логирование/Классы/ОтелПростойПроцессорЛогов.os:67` |  |
| 42 | MUST | ✅ found | If a timeout is specified (see below), the `LogRecordProcessor` MUST prioritize honoring the timeout over finishing all calls. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:131` |  |
| 43 | SHOULD | ✅ found | `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Логирование/Классы/ОтелКомпозитныйПроцессорЛогов.os:74` |  |
| 44 | SHOULD | ✅ found | `ForceFlush` SHOULD only be called in cases where it is absolutely necessary, such as when using some FaaS providers that may suspend the process after an invocation, but before the `LogRecordProces... | - |  |
| 45 | SHOULD | ✅ found | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Логирование/Классы/ОтелКомпозитныйПроцессорЛогов.os:78` |  |
...
| 48 | SHOULD | ✅ found | Additional processors defined in this document SHOULD be provided by SDK packages. | - |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 49 | MUST | ✅ found | The processor MUST synchronize calls to `LogRecordExporter`’s `Export` to make sure that they are not invoked concurrently. | `src/Логирование/Классы/ОтелПростойПроцессорЛогов.os:31` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 50 | MUST | ✅ found | The processor MUST synchronize calls to `LogRecordExporter`’s `Export` to make sure that they are not invoked concurrently. | `src/Экспорт/Классы/ОтелБазовыйПакетныйПроцессор.os:160` |  |
...
| 55 | SHOULD NOT | ✅ found | The default SDK's `LogRecordProcessors` SHOULD NOT implement retry logic, as the required logic is likely to depend heavily on the specific protocol and backend the logs are being sent to. | `src/Логирование/Классы/ОтелПростойПроцессорЛогов.os:25` |  |
...
| 58 | SHOULD | ✅ found | `ForceFlush` SHOULD only be called in cases where it is absolutely necessary, such as when using some FaaS providers that may suspend the process after an invocation, but before the exporter exports the `ReadlableLogRecords`. | `src/Экспорт/Классы/ОтелЭкспортерЛогов.os:55` |  |
...
| 4 | MUST NOT | ✅ found | Therefore, this API needs to be structured to accept a `version`, but MUST NOT obligate a user to provide one. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:75` |  |
| 5 | MUST | ✅ found | Therefore, this API needs to be structured to accept a `schema_url`, but MUST NOT obligate a user to provide one. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:77` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 9 | SHOULD | ➖ n_a | Language-level features such as the distinction between integer and floating point numbers SHOULD be considered as identifying. | - | OneScript: тип Число = System.Decimal, нет различия между integer и floating-point на уровне языка - различение невозможно платформенно. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 10 | SHOULD | ✅ found | The API SHOULD treat it as an opaque string. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:277` |  |
| 11 | MUST | ✅ found | It MUST be case-sensitive (e.g. `kb` and `kB` are different units), ASCII string. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:277` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 12 | MUST | ✅ found | The API MUST treat it as an opaque string. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:292` |  |
...
| 18 | SHOULD | ⚠️ partial | The API SHOULD be documented in a way to communicate to users that the `name` parameter needs to conform to the instrument name syntax. | `src/Метрики/Классы/ОтелМетр.os:42` | Doc-комментарий метода СоздатьСчетчик описывает имя как 'имя счетчика', но не упоминает требования к синтаксису instrument name. |
...
| 21 | MUST | ✅ found | Meaning, the API MUST accept a case-sensitive string that supports ASCII character encoding and can hold at least 63 characters. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:277` |  |
| 22 | SHOULD NOT | ⚠️ partial | The API SHOULD NOT validate the `unit`. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:277` | УстановитьЕдиницаИзмерения проверяет, что строка содержит только печатные ASCII-символы; не-ASCII значения сбрасываются в пустую строку с предупреждением — это валидация, противоречащая SHOULD NOT. |
...
| 24 | MUST | ✅ found | Meaning, the API MUST accept a string that supports at least BMP (Unicode Plane 0) encoded characters and hold at least 1023 characters. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:292` |  |
...
| 26 | SHOULD NOT | ⚠️ partial | The API SHOULD NOT validate `advisory` parameters. | `src/Метрики/Классы/ОтелМетр.os:853` | ПроверитьСовет валидирует тип Совет (должна быть Структура) и типы Границы/Ключи (должны быть Массивами), логируя предупреждения — это валидация, противоречащая SHOULD NOT. |
| 27 | MUST | ✅ found | The API to construct asynchronous instruments MUST accept the following parameters: | `src/Метрики/Классы/ОтелМетр.os:295` |  |
| 28 | SHOULD | ✅ found | If possible, the API SHOULD be structured so a user is obligated to provide this parameter. | `src/Метрики/Классы/ОтелМетр.os:295` |  |
| 29 | MUST | ✅ found | If it is not possible to structurally enforce this obligation, the API MUST be documented in a way to communicate to users that this parameter is needed. | `src/Метрики/Классы/ОтелМетр.os:268` |  |
| 30 | SHOULD | ⚠️ partial | The API SHOULD be documented in a way to communicate to users that the `name` parameter needs to conform to the instrument name syntax. | `src/Метрики/Классы/ОтелМетр.os:268` | Doc-комментарий метода СоздатьНаблюдаемыйСчетчик описывает имя как 'имя инструмента', но не упоминает требования к синтаксису instrument name. |
| 31 | SHOULD NOT | ✅ found | The API SHOULD NOT validate the `name`, that is left to implementations of the API. | `src/Метрики/Классы/ОтелМетр.os:295` |  |
| 32 | MUST NOT | ✅ found | Therefore, this API needs to be structured to accept a `unit`, but MUST NOT obligate a user to provide one. | `src/Метрики/Классы/ОтелМетр.os:295` |  |
| 33 | MUST | ✅ found | Meaning, the API MUST accept a case-sensitive string that supports ASCII character encoding and can hold at least 63 characters. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:142` |  |
| 34 | SHOULD NOT | ⚠️ partial | The API SHOULD NOT validate the `unit`. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:142` | УстановитьЕдиницаИзмерения проверяет ASCII-печатность и сбрасывает не-ASCII в пустую строку с предупреждением — это валидация, противоречащая SHOULD NOT. |
| 35 | MUST NOT | ✅ found | Therefore, this API needs to be structured to accept a `description`, but MUST NOT obligate a user to provide one. | `src/Метрики/Классы/ОтелМетр.os:295` |  |
| 36 | MUST | ✅ found | Meaning, the API MUST accept a string that supports at least BMP (Unicode Plane 0) encoded characters and hold at least 1023 characters. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:157` |  |
| 37 | MUST NOT | ✅ found | Therefore, this API needs to be structured to accept `advisory` parameters, but MUST NOT obligate the user to provide it. | `src/Метрики/Классы/ОтелМетр.os:295` |  |
| 38 | SHOULD NOT | ⚠️ partial | The API SHOULD NOT validate `advisory` parameters. | `src/Метрики/Классы/ОтелМетр.os:853` | ПроверитьСовет валидирует типы Совет/Границы/Ключи и логирует предупреждения — это валидация, противоречащая SHOULD NOT. |
| 39 | MUST | ✅ found | Therefore, this API MUST be structured to accept a variable number of `callback` functions, including none. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:172` |  |
| 40 | MUST | ✅ found | The API MUST support creation of asynchronous instruments by passing zero or more `callback` functions to be permanently registered to the newly created instrument. | `src/Метрики/Классы/ОтелМетр.os:295` |  |
| 41 | SHOULD | ✅ found | The API SHOULD support registration of `callback` functions associated with asynchronous instruments after they are created. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:65` |  |
| 42 | MUST | ✅ found | Where the API supports registration of `callback` functions after asynchronous instrumentation creation, the user MUST be able to undo registration of the specific callback after its registration b... | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:76` |  |
| 43 | MUST | ✅ found | Every currently registered Callback associated with a set of instruments MUST be evaluated exactly once during collection prior to reading data for that instrument set. | `src/Метрики/Классы/ОтелМетр.os:565` |  |
| 44 | MUST | ✅ found | Callback functions MUST be documented as follows for the end user: | `src/Метрики/Классы/ОтелМетр.os:271` |  |
| 45 | SHOULD | ✅ found | Callback functions SHOULD be reentrant safe. | `src/Метрики/Классы/ОтелМетр.os:276` |  |
| 46 | SHOULD NOT | ✅ found | Callback functions SHOULD NOT take an indefinite amount of time. | `src/Метрики/Классы/ОтелМетр.os:281` |  |
| 47 | SHOULD NOT | ✅ found | Callback functions SHOULD NOT make duplicate observations (more than one `Measurement` with the same `attributes`) across all registered callbacks. | `src/Метрики/Классы/ОтелМетр.os:284` |  |
| 48 | MUST | ✅ found | Callbacks registered at the time of instrument creation MUST apply to the single instruments which is under construction. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:172` |  |
| 49 | MUST | ✅ found | Idiomatic APIs for multiple-instrument Callbacks MUST distinguish the instrument associated with each observed `Measurement` value. | `src/Метрики/Классы/ОтелМетр.os:576` |  |
| 50 | MUST | ✅ found | Multiple-instrument Callbacks MUST be associated at the time of registration with a declared set of asynchronous instruments from the same `Meter` instance. | `src/Метрики/Классы/ОтелМетр.os:544` |  |
| 51 | MUST | ✅ found | The API MUST treat observations from a single Callback as logically taking place at a single instant, such that when recorded, observations from a single callback MUST be reported with identical t... | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:212` |  |
| 52 | MUST | ✅ found | The API MUST treat observations from a single Callback as logically taking place at a single instant, such that when recorded, observations from a single callback MUST be reported with identical t... | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:212` |  |
| 53 | SHOULD | ✅ found | The API SHOULD provide some way to pass `state` to the callback. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:199` |  |
...
| 58 | SHOULD | ⚠️ partial | The API SHOULD be documented that instrumentation authors needs to call this API each time they record a measurement to ensure they have the most up-to-date response. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:248` | Doc-комментарий описывает семантику Включен(), но не содержит явного указания вызывать API перед каждой записью измерения для актуальности ответа. |
...
| 65 | SHOULD NOT | ⚠️ partial | This API SHOULD NOT validate this value, that is left to implementations of the API. | `src/Метрики/Классы/ОтелСчетчик.os:22` | Метод Добавить проверяет 'Если Значение < 0 Тогда Возврат' — отрицательные значения тихо отбрасываются на уровне API; это форма валидации, противоречащая SHOULD NOT. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 68 | MUST NOT | ✅ found | There MUST NOT be any API for creating an Asynchronous Counter other than with a `Meter`. | `src/Метрики/Классы/ОтелМетр.os:295` |  |
| 69 | MUST | ✅ found | The API MUST treat observations from a single callback as logically taking place at a single instant, such that when recorded, observations from a single callback MUST be reported with identical times... | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:212` |  |
| 70 | MUST | ✅ found | The API MUST treat observations from a single callback as logically taking place at a single instant, such that when recorded, observations from a single callback MUST be reported with identical times... | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:213` |  |
| 71 | SHOULD | ⚠️ partial | The API SHOULD provide some way to pass `state` to the callback. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:65` | Явного параметра state у callback нет; передача состояния возможна только через захват замыкания лямбды (#Использовать lambdas). Это допустимый идиоматический подход по спецификации, но как отдельная фича API — не реализован. |
...
| 76 | MUST | ✅ found | If it is not possible to structurally enforce this obligation, this API MUST be documented in a way to communicate to users that this parameter is needed. | `src/Метрики/Классы/ОтелГистограмма.os:20` |  |
| 77 | SHOULD | ⚠️ partial | This API SHOULD be documented in a way to communicate to users that this value is expected to be non-negative. | `src/Метрики/Классы/ОтелГистограмма.os:13` | Документирующий комментарий процедуры Записать описывает параметр Значение как «измеренное значение», но не упоминает, что значение должно быть неотрицательным. |
| 78 | SHOULD NOT | ✅ found | This API SHOULD NOT validate this value, that is left to implementations of the API. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:87` |  |
...
| 84 | MUST | ✅ found | If it is not possible to structurally enforce this obligation, this API MUST be documented in a way to communicate to users that this parameter is needed. | `src/Метрики/Классы/ОтелДатчик.os:21` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 87 | MUST NOT | ✅ found | There MUST NOT be any API for creating an Asynchronous Gauge other than with a `Meter`. | `src/Метрики/Классы/ОтелМетр.os:406` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 94 | MUST NOT | ✅ found | There MUST NOT be any API for creating an Asynchronous UpDownCounter other than with a `Meter`. | `src/Метрики/Классы/ОтелМетр.os:353` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 95 | SHOULD | ✅ found | The API to register a new Callback SHOULD accept: * A `callback` function * A list (or tuple, etc.) of Instruments used in the `callback` function. | `src/Метрики/Классы/ОтелМетр.os:544` |  |
...
| 97 | SHOULD | ✅ found | All the metrics APIs SHOULD allow optional parameter(s) to be added to existing APIs without introducing breaking changes, if possible. | `src/Метрики/Классы/ОтелМетр.os:353` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 5 | MUST | ✅ found | Configuration ( i.e. MetricExporters, MetricReaders, Views, and (Development) MeterConfigurator) MUST be owned by the `MeterProvider`. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:21` |  |
| 6 | MUST | ✅ found | If configuration is updated (e.g., adding a `MetricReader`), the updated configuration MUST also apply to all already returned `Meters` (i.e. it MUST NOT matter whether a `Meter` was obtained ... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:260` |  |
| 7 | MUST NOT | ✅ found | If configuration is updated (e.g., adding a `MetricReader`), the updated configuration MUST also apply to all already returned `Meters` (i.e. it MUST NOT matter whether a `Meter` was obtained ... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:307` |  |
...
| 11 | SHOULD | ⚠️ partial | `Shutdown` SHOULD complete or abort within some timeout. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:180` | Метод Закрыть() не принимает параметр таймаута. Есть асинхронная обертка ЗакрытьАсинхронно(), но без явного механизма прерывания по таймауту в самом MeterProvider. |
...
| 15 | SHOULD | ✅ found | `ForceFlush` SHOULD return some ERROR status if there is an error condition; and if there is no error condition, it should return some NO ERROR status, language implementations MAY decide how to ... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:173` |  |
...
| 18 | MUST | ✅ found | This functionality MUST accept as inputs the Instrument selection criteria and the resulting stream configuration. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:228` |  |
...
| 25 | MUST NOT | ✅ found | Therefore, the instrument selection criteria parameter needs to be structured to accept a `unit`, but MUST NOT obligate a user to provide one. | `src/Метрики/Классы/ОтелСелекторИнструментов.os:161` |  |
...
| 31 | SHOULD | ✅ found | `name`: The metric stream name that SHOULD be used. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:333` |  |
| 32 | SHOULD | ✅ found | In order to avoid conflicts, if a `name` is provided the View SHOULD have an instrument selector that selects at most one instrument. | `src/Метрики/Классы/ОтелМетр.os:892` |  |
| 33 | MUST NOT | ✅ found | Therefore, the stream configuration parameter needs to be structured to accept a `name`, but MUST NOT obligate a user to provide one. | `src/Метрики/Классы/ОтелПредставление.os:157` |  |
| 34 | MUST | ✅ found | If the user does not provide a `name` value, name from the Instrument the View matches MUST be used by default. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:308` |  |
| 35 | SHOULD | ✅ found | `description`: The metric stream description that SHOULD be used. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:336` |  |
...
| 38 | MUST | ✅ found | The allow-list contains attribute keys that identify the attributes that MUST be kept, and all other attributes MUST be ignored. | `src/Метрики/Классы/ОтелМетр.os:715` |  |
| 39 | MUST | ✅ found | The allow-list contains attribute keys that identify the attributes that MUST be kept, and all other attributes MUST be ignored. | `src/Метрики/Классы/ОтелМетр.os:715` |  |
...
| 41 | SHOULD | ✅ found | If the user does not provide any value, the SDK SHOULD use the `Attributes` advisory parameter configured on the instrument instead. | `src/Метрики/Классы/ОтелМетр.os:711` |  |
| 42 | MUST | ✅ found | If the `Attributes` advisory parameter is absent, all attributes MUST be kept. | `src/Метрики/Классы/ОтелМетр.os:714` | Если ни View, ни advisory не задают КлючиАтрибутов — фильтр не устанавливается, все атрибуты сохраняются. |
| 43 | SHOULD | ✅ found | Additionally, implementations SHOULD support configuring an exclude-list of attribute keys. | `src/Метрики/Классы/ОтелПредставление.os:162` |  |
| 44 | MUST | ✅ found | The exclude-list contains attribute keys that identify the attributes that MUST be excluded, all other attributes MUST be kept. | `src/Метрики/Классы/ОтелМетр.os:719` |  |
| 45 | MUST | ✅ found | The exclude-list contains attribute keys that identify the attributes that MUST be excluded, all other attributes MUST be kept. | `src/Метрики/Классы/ОтелМетр.os:719` |  |
...
| 49 | MUST | ✅ found | If the user does not provide an `exemplar_reservoir` value, the `MeterProvider` MUST apply a default exemplar reservoir. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:330` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 52 | SHOULD | ✅ found | The SDK SHOULD use the following logic to determine how to process Measurements made with an Instrument: | `src/Метрики/Классы/ОтелМетр.os:685` |  |
| 53 | MUST | ✅ found | Instrument advisory parameters, if any, MUST be honored. | `src/Метрики/Классы/ОтелМетр.os:711` |  |
| 54 | SHOULD | ✅ found | If applying the View results in conflicting metric identities the implementation SHOULD apply the View and emit a warning. | `src/Метрики/Классы/ОтелМетр.os:917` |  |
| 55 | SHOULD | ⚠️ partial | If it is not possible to apply the View without producing semantic errors (e.g. the View sets an asynchronous instrument to use the Explicit bucket histogram aggregation) the implementation SHOULD emit a warni... | `src/Метрики/Классы/ОтелМетр.os:936` | Нет специальной проверки несовместимости: при попытке назначить histogram-агрегацию async-инструменту через View агрегатор просто заменяется без предупреждения. |
| 56 | MUST | ✅ found | If both a View and Instrument advisory parameters specify the same aspect of the Stream configuration, the setting defined by the View MUST take precedence over the advisory parameters. | `src/Метрики/Классы/ОтелМетр.os:707` |  |
| 57 | SHOULD | ✅ found | If the Instrument could not match with any of the registered `View`(s), the SDK SHOULD enable the instrument using the default aggregation and temporality. | `src/Метрики/Классы/ОтелМетр.os:686` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 58 | MUST | ✅ found | The SDK MUST provide the following `Aggregation` to support the Metric Points in the Metrics Data Model. | `src/Метрики/Модули/ОтелАгрегация.os:25` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 60 | SHOULD NOT | ⚠️ partial | Arithmetic sum of `Measurement` values in population. This SHOULD NOT be collected when used with instruments that record negative measurements (e.g. `UpDownCounter` or `ObservableGauge`). | `src/Метрики/Классы/ОтелАгрегаторГистограммы.os:65` | Sum собирается безусловно. Нет проверки типа инструмента и пропуска sum при UpDownCounter/ObservableGauge. |
| 61 | SHOULD | ✅ found | SDKs SHOULD use the default value when boundaries are not explicitly provided, unless they have good reasons to use something different (e.g. for backward compatibility reasons in a stable SDK release). | `src/Метрики/Классы/ОтелАгрегаторГистограммы.os:154` |  |
| 62 | SHOULD NOT | ➖ n_a | Implementations SHOULD NOT incorporate non-normal values (i.e., +Inf, -Inf, and NaNs) into the `sum`, `min`, and `max` fields, because these values do not map into a valid bucket. | - | OneScript Number = System.Decimal: значения +Inf/-Inf/NaN недопустимы платформой, попытка их получить вызовет исключение. Требование автоматически выполняется рантаймом. |
| 63 | MUST | ⚠️ partial | The implementation MUST maintain reasonable minimum and maximum scale parameters that the automatic scale parameter will not exceed. | `src/Метрики/Классы/ОтелАгрегаторЭкспоненциальнойГистограммы.os:218` | Минимальная шкала захардкожена (-10), максимальная ограничена параметром НачальнаяШкала (default 20), но MaxScale как отдельный конфигурируемый параметр спецификации не предусмотрен. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 66 | MUST | ⚠️ partial | Callback functions MUST be invoked for the specific `MetricReader` performing collection, such that observations made or produced by executing callbacks only apply to the intended `MetricReader` during c... | `src/Метрики/Классы/ОтелМетр.os:565` | Перед вызовом мульти-callback инструменты очищаются, но изоляция per-MetricReader не строгая: если несколько reader-ов конкурентно собирают метрики, наблюдения могут пересекаться. |
| 67 | SHOULD | ✅ found | The implementation SHOULD disregard the use of asynchronous instrument APIs outside of registered callbacks. | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:196` |  |
| 68 | SHOULD | ❌ not_found | The implementation SHOULD use a timeout to prevent indefinite callback execution. | - | Callback'и async-инструментов выполняются синхронно без таймаута: ВызватьМультиОбратныеВызовы и ВызватьCallbackИСобрать просто вызывают .Выполнить() без ограничения времени. |
| 69 | MUST | ✅ found | The implementation MUST complete the execution of all callbacks for a given instrument before starting a subsequent round of collection. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:242` |  |
| 70 | SHOULD NOT | ✅ found | The implementation SHOULD NOT produce aggregated metric data for a previously-observed attribute set which is not observed during a successful callback. | `src/Метрики/Классы/ОтелМетр.os:570` |  |
...
| 72 | SHOULD | ✅ found | Cardinality limit enforcement SHOULD occur after attribute filtering, if any. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:97` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 73 | SHOULD | ✅ found | A view with criteria matching the instrument an aggregation is created for has an `aggregation_cardinality_limit` value defined for the stream, that value SHOULD be used. | `src/Метрики/Классы/ОтелМетр.os:702` |  |
| 74 | SHOULD | ⚠️ partial | If there is no matching view, but the `MetricReader` defines a default cardinality limit value based on the instrument an aggregation is created for, that value SHOULD be used. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:290` | MetricReader.ЛимитМощности() применяется ко всем инструментам Meter единым числом; нет дифференциации лимита по типу инструмента (per-instrument-type default cardinality). |
| 75 | SHOULD | ✅ found | If none of the previous values are defined, the default value of 2000 SHOULD be used. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:318` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 76 | MUST | ✅ found | The SDK MUST create an Aggregator with the overflow attribute set prior to reaching the cardinality limit and use it to aggregate Measurements for which the correct Aggregator could not be created. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:412` |  |
...
| 79 | MUST | ✅ found | Regardless of aggregation temporality, the SDK MUST ensure that every Measurement is reflected in exactly one Aggregator, which is either an Aggregator associated with the correct attribute set or an aggregator ass... | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:108` |  |
| 80 | MUST NOT | ✅ found | Measurements MUST NOT be double-counted or dropped during an overflow. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:108` |  |
...
| 83 | SHOULD | ✅ found | Therefore, when a duplicate instrument registration occurs, and it is not corrected with a View, a warning SHOULD be emitted. | `src/Метрики/Классы/ОтелМетр.os:784` |  |
| 84 | SHOULD | ⚠️ partial | The emitted warning SHOULD include information for the user on how to resolve the conflict, if possible. | `src/Метрики/Классы/ОтелМетр.os:784` | Текст предупреждения сообщает имя, вид и единицу измерения конфликта, но не содержит конкретных рекомендаций (renaming View, изменение description) по разрешению конфликта. |
| 85 | SHOULD | ❌ not_found | If the potential conflict involves multiple `description` properties, setting the `description` through a configured View SHOULD avoid the warning. | - | ПроверитьКонфликтДескриптора сравнивает Описание непосредственно с зарегистрированным дескриптором без учёта View с заданным новым description; нет логики, подавляющей предупреждение, если View переопределяет description. |
| 86 | SHOULD | ❌ not_found | If the potential conflict involves instruments that can be distinguished by a supported View selector (e.g. name, instrument kind) a renaming View recipe SHOULD be included in the warning. | - | Текст Лог.Предупреждение в ПроверитьКонфликтДескриптора не содержит примера/recipe конфигурации View для разрешения конфликта. |
| 87 | SHOULD | ❌ not_found | Otherwise (e.g., use of multiple units), the SDK SHOULD pass through the data by reporting both `Metric` objects and emit a generic warning describing the duplicate instrument registration. | - | При повторной регистрации возвращается ранее зарегистрированный инструмент (Возврат Существующий), второй инструмент с другой единицей измерения не создаётся и не экспортируется отдельной метрикой. |
| 88 | MUST | ✅ found | the SDK MUST aggregate data from identical Instruments together in its export pipeline. | `src/Метрики/Классы/ОтелМетр.os:56` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 90 | SHOULD | ❌ not_found | When a Meter creates an instrument, it SHOULD validate the instrument name conforms to the instrument name syntax. | - | ОтелМетр.СоздатьСчетчик/СоздатьГистограмму/etc. не выполняют валидацию имени инструмента (нет проверки регулярным выражением [a-zA-Z][a-zA-Z0-9_./-]{0,254}). Имя принимается как есть. |
| 91 | SHOULD | ❌ not_found | If the instrument name does not conform to this syntax, the Meter SHOULD emit an error notifying the user about the invalid name. | - | Поскольку валидация имени отсутствует, ошибка о невалидном имени не эмитируется. |
...
| 93 | MUST | ✅ found | If a unit is not provided or the unit is null, the Meter MUST treat it the same as an empty unit string. | `src/Метрики/Классы/ОтелМетр.os:671` |  |
...
| 95 | MUST | ✅ found | If a description is not provided or the description is null, the Meter MUST treat it the same as an empty description string. | `src/Метрики/Классы/ОтелМетр.os:671` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 96 | SHOULD | ✅ found | When a Meter creates an instrument, it SHOULD validate the instrument advisory parameters. | `src/Метрики/Классы/ОтелМетр.os:853` |  |
| 97 | SHOULD | ✅ found | If an advisory parameter is not valid, the Meter SHOULD emit an error notifying the user and proceed as if the parameter was not provided. | `src/Метрики/Классы/ОтелМетр.os:858` |  |
| 98 | MUST | ✅ found | If multiple identical Instruments are created with different advisory parameters, the Meter MUST return an instrument using the first-seen advisory parameters and log an appropriate ... | `src/Метрики/Классы/ОтелМетр.os:781` |  |
| 99 | MUST | ✅ found | If both a View and advisory parameters specify the same aspect of the Stream configuration, the setting defined by the View MUST take precedence over the advisory parameters. | `src/Метрики/Классы/ОтелМетр.os:740` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 100 | MUST | ✅ found | If no View matches, or if a matching View selects the default aggregation, the `ExplicitBucketBoundaries` advisory parameter MUST be used. | `src/Метрики/Классы/ОтелМетр.os:746` |  |
...
| 102 | SHOULD | ✅ found | `Exemplar` sampling SHOULD be turned on by default. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:350` |  |
| 103 | MUST NOT | ✅ found | If `Exemplar` sampling is off, the SDK MUST NOT have overhead related to exemplar sampling. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:422` |  |
| 104 | MUST | ✅ found | A Metric SDK MUST allow exemplar sampling to leverage the configuration of metric aggregation. | `src/Метрики/Классы/ОтелМетр.os:119` |  |
...
| 108 | SHOULD | ✅ found | The default value SHOULD be `TraceBased`. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:350` |  |
...
| 112 | MUST | ✅ found | A new `ExemplarReservoir` MUST be created for every known timeseries data point, as determined by aggregation and view configuration. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:439` |  |
| 113 | SHOULD | ✅ found | The "offer" method SHOULD accept measurements, including: * The `value` of the measurement.* The complete set of `Attributes` of the measurement.* The Context of the measurement, which c... | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:41` |  |
| 114 | SHOULD | ✅ found | The "offer" method SHOULD have the ability to pull associated trace and span information without needing to record full context. | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:42` |  |
| 115 | MUST | ⚠️ partial | This MUST be clearly documented in the API and the reservoir MUST be given the `Attributes` associated with its timeseries point either at construction so that additional sampling performed by the re... | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:30` | Документирующий комментарий метода Предложить упоминает АтрибутыИзмерения и АтрибутыСерии, но не описывает явно семантику фильтрованного подмножества атрибутов и их расхождения с timeseries-набором. |
| 116 | MUST | ⚠️ partial | This MUST be clearly documented in the API and the reservoir MUST be given the `Attributes` associated with its timeseries point either at construction so that additional sampling performed by the re... | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:41` | АтрибутыСерии передаются в каждый вызов Предложить(), а не сохраняются в резервуаре при конструировании; функционально эквивалентно, но архитектура отличается от описанной в спеке. |
...
| 120 | SHOULD | ⚠️ partial | The `ExemplarReservoir` SHOULD avoid allocations when sampling exemplars. | `src/Метрики/Классы/ОтелРезервуарЭкземпляров.os:121` | Метод СоздатьЭкземпляр аллоцирует новое Соответствие на каждое принятое измерение и пересчитывает отфильтрованные атрибуты; явных оптимизаций по предотвращению аллокаций нет. |
...
| 123 | SHOULD | ✅ found | Base2 Exponential Histogram Aggregation SHOULD use a `SimpleFixedSizeExemplarReservoir` with a reservoir equal to the smaller of the maximum number of buckets configured on the aggregation or twenty (... | `src/Метрики/Классы/ОтелМетр.os:178` |  |
| 124 | SHOULD | ✅ found | All other aggregations SHOULD use `SimpleFixedSizeExemplarReservoir`. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:330` |  |
...
| 127 | SHOULD | ✅ found | Otherwise, a default size of `1` SHOULD be used. | `src/Метрики/Классы/ОтелФабрикаПростыхРезервуаров.os:48` |  |
...
| 129 | MUST | ✅ found | This implementation MUST store at most one measurement that falls within a histogram bucket, and SHOULD use a uniformly-weighted sampling algorithm based on the number of measurements the bucket has s... | `src/Метрики/Классы/ОтелВыровненныйРезервуарГистограммы.os:76` |  |
| 130 | SHOULD | ✅ found | This implementation MUST store at most one measurement that falls within a histogram bucket, and SHOULD use a uniformly-weighted sampling algorithm based on the number of measurements the bucket has s... | `src/Метрики/Классы/ОтелВыровненныйРезервуарГистограммы.os:55` |  |
...
| 134 | MUST | ✅ found | This extension MUST be configurable on a metric View, although individual reservoirs MUST still be instantiated per metric-timeseries (see Exemplar Reservoir - Paragraph 2). | `src/Метрики/Классы/ОтелМетр.os:722` |  |
...
| 136 | SHOULD | ✅ found | `Collect` SHOULD invoke Produce on registered MetricProducers. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:448` |  |
...
| 138 | SHOULD | ⚠️ partial | SDKs SHOULD return some failure for these calls, if possible. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:106` | После Закрыть() флаг Закрыт=Истина, но СброситьБуфер()/СобратьИЭкспортировать() не проверяют этот флаг и могут отработать повторно без возврата признака отказа. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 141 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD collect metrics, split into batches if necessary, call `Export(batch)` on each batch and `ForceFlush()` on the configured Push Metric Exporter. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:106` | СброситьБуфер() читателя собирает и вызывает Экспортировать(), но НЕ вызывает СброситьБуфер() (ForceFlush) на экспортёре после экспорта. |
| 142 | SHOULD | ⚠️ partial | `ForceFlush` MAY skip `Export(batch)` calls if the timeout is already expired, but SHOULD still call `ForceFlush()` on the configured Push Metric Exporter even if the timeout has passed. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:106` | Не реализована логика таймаута и отдельного вызова ForceFlush() на экспортёре по истечении таймаута. |
...
| 144 | SHOULD | ✅ found | `ForceFlush` SHOULD return some ERROR status if there is an error condition; and if there is no error condition, it should return some NO ERROR status, language implementations MAY decide how to mode... | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:235` |  |
| 145 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:106` | Параметр ТаймаутМс присутствует, но помечен как UnusedParameters — таймаут на уровне reader не enforced (не прерывает выполнение по истечении). |
...
| 147 | SHOULD | ❌ not_found | Metric Exporters SHOULD report an error condition for data output by the `MetricReader` with unsupported Aggregation or Aggregation Temporality, as this condition can be corrected by a change of `Met... | - | ОтелЭкспортерМетрик не проверяет совместимость агрегации/временности данных и не сигнализирует об ошибке для неподдерживаемых типов. |
...
| 149 | MUST | ✅ found | The SDK MUST provide a way for the exporter to get the Meter information (e.g. name, version, etc.) associated with each `Metric Point`. | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:175` |  |
| 150 | MUST NOT | ✅ found | `Export` MUST NOT block indefinitely, there MUST be a reasonable upper limit after which the call must time out with an error result (Failure). | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:47` |  |
| 151 | MUST | ✅ found | `Export` MUST NOT block indefinitely, there MUST be a reasonable upper limit after which the call must time out with an error result (Failure). | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:147` |  |
...
| 153 | SHOULD | ✅ found | This is a hint to ensure that the export of any `Metrics` the exporter has received prior to the call to `ForceFlush` SHOULD be completed as soon as possible, preferably before returning from this met... | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:66` |  |
| 154 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:66` | СброситьБуфер() возвращает ОтелРезультатыЭкспорта.Успех() безусловно (no-op), статус таймаута/ошибки не дифференцируется. |
| 155 | SHOULD | ✅ found | `ForceFlush` SHOULD only be called in cases where it is absolutely necessary, such as when using some FaaS providers that may suspend the process after an invocation, but before the exporter exports t... | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:55` |  |
| 156 | SHOULD | ⚠️ partial | `ForceFlush` SHOULD complete or abort within some timeout. | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:66` | Параметр ТаймаутМс есть, но помечен UnusedParameters — таймаут не учитывается; реализация — синхронный no-op (тривиально завершается). |
...
| 158 | SHOULD NOT | ✅ found | `Shutdown` SHOULD NOT block indefinitely (e.g. if it attempts to flush the data and the destination is unavailable). | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:76` |  |
...
| 160 | SHOULD | ❌ not_found | `MetricProducer` implementations SHOULD accept configuration for the `AggregationTemporality` of produced metrics. | - | Интерфейс ИнтерфейсПродюсерМетрик.Произвести(ФильтрМетрик) не принимает и нет конфигурации AggregationTemporality. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 163 | MUST | ✅ found | The SDK MUST provide configuration according to the SDK environment variables specification. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:96` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 164 | MUST | ✅ found | The SDK MUST handle numerical limits in a graceful way according to Error handling in OpenTelemetry. | `src/Метрики/Классы/ОтелБазовыйАгрегатор.os:1` |  |
| 165 | MUST | ✅ found | If the SDK receives float/double values from Instruments, it MUST handle all the possible values. | `src/Метрики/Классы/ОтелСчетчик.os:1` |  |
...
| 171 | MUST | ✅ found | MetricExporter - `ForceFlush` and `Shutdown` MUST be safe to be called concurrently. | `src/Экспорт/Классы/ОтелЭкспортерМетрик.os:6` |  |
...
| 2 | MUST | ✅ found | Each configuration option MUST be overridable by a signal specific option. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:629` |  |
| 3 | MUST | ✅ found | The implementation MUST honor the following URL components: scheme (http or https), host, port, path. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:796` |  |
...
| 5 | SHOULD | ✅ found | The option SHOULD accept any form allowed by the underlying gRPC client implementation. | `src/Экспорт/Классы/ОтелGrpcТранспорт.os:48` |  |
| 6 | MUST | ✅ found | Additionally, the option MUST accept a URL with a scheme of either http or https. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:796` |  |
| 7 | SHOULD | ✅ found | If the gRPC client implementation does not support an endpoint with a scheme of http or https then the endpoint SHOULD be transformed to the most sensible format for that implementation. | `src/Экспорт/Классы/ОтелGrpcТранспорт.os:48` |  |
...
| 10 | SHOULD | ➖ n_a | However, if they are already implemented, they SHOULD continue to be supported as they were part of a stable release of the specification. | - | Условное требование: применимо только если obsolete env vars (OTEL_EXPORTER_OTLP_SPAN_INSECURE, OTEL_EXPORTER_OTLP_METRIC_INSECURE) уже были реализованы. В данном SDK эти переменные никогда не были реализованы, требование неприменимо. |
| 11 | SHOULD | ❌ not_found | The default protocol SHOULD be http/protobuf, unless there are strong reasons for SDKs to select grpc as the default. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:630` | Дефолт протокола в коде - 'http/json' (см. ПротоколHttpJson = "http/json" и default параметра 'protocol'), а спека требует 'http/protobuf'. Хотя http/protobuf поддерживается в списке допустимых, по умолчанию выбран http/json. |
...
| 13 | MUST | ✅ found | For the per-signal variables (OTEL_EXPORTER_OTLP_<signal>_ENDPOINT), the URL MUST be used as-is without any modification. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:669` |  |
...
| 15 | MUST NOT | ✅ found | An SDK MUST NOT modify the URL in ways other than specified above. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:948` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 16 | SHOULD | ✅ found | SDKs SHOULD support both grpc and http/protobuf transports and MUST support at least one of them. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:632` |  |
| 17 | MUST | ✅ found | SDKs SHOULD support both grpc and http/protobuf transports and MUST support at least one of them. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:632` |  |
| 18 | SHOULD | ✅ found | If they support only one, it SHOULD be http/protobuf. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:632` |  |
| 19 | SHOULD | ❌ not_found | If no configuration is provided the default transport SHOULD be http/protobuf unless SDKs have good reasons to choose grpc as the default (e.g. for backward compatibility reasons when grpc was already the de... | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:630` | Default protocol установлен в 'http/json' (ПараметрСигналаИлиОбщий(... 'protocol', ПротоколHttpJson)), а не 'http/protobuf' как требует спецификация. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 20 | MUST | ✅ found | All attribute values MUST be considered strings. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:762` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 23 | SHOULD | ❌ not_found | OpenTelemetry protocol exporters SHOULD emit a User-Agent header to at a minimum identify the exporter, the language of its implementation, and the version of the exporter. | - | В коде HTTP/gRPC транспортов (ОтелHttpТранспорт, ОтелGrpcТранспорт) не задаётся заголовок User-Agent. Поиск 'User-Agent' / 'UserAgent' по src/ не дал совпадений. |
| 24 | SHOULD | ❌ not_found | The format of the header SHOULD follow RFC 7231. | - | User-Agent заголовок не формируется, формат RFC 7231 не применяется. |
| 25 | SHOULD | ❌ not_found | The resulting User-Agent SHOULD include the exporter's default User-Agent string. | - | Default User-Agent строки нет, поэтому при добавлении product identifier (тоже не реализовано) включать нечего. |
...
| 2 | MUST | ✅ found | Each `Propagator` type MUST define the specific carrier type and MAY define additional parameters. | `src/Пропагация/Классы/ОтелКомпозитныйПропагатор.os:18` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 4 | MUST NOT | ✅ found | If a value can not be parsed from the carrier, for a cross-cutting concern, the implementation MUST NOT throw an exception and MUST NOT store a new value in the `Context`, in order to preserve any previously existing valid value. | `src/Пропагация/Классы/ОтелW3CПропагатор.os:96` |  |
| 5 | MUST NOT | ✅ found | If a value can not be parsed from the carrier, for a cross-cutting concern, the implementation MUST NOT throw an exception and MUST NOT store a new value in the `Context`, in order to preserve any previously existing valid value. | `src/Пропагация/Классы/ОтелW3CПропагатор.os:101` |  |
...
| 12 | MUST | ✅ found | If the getter is intended to work with an HTTP request object, the getter MUST be case insensitive. | `src/Пропагация/Классы/ОтелГеттерТекстовойКарты.os:23` |  |
...
| 16 | MUST | ✅ found | The `GetAll` function is responsible for handling case sensitivity. If the getter is intended to work with an HTTP request object, the getter MUST be case insensitive. | `src/Пропагация/Классы/ОтелГеттерТекстовойКарты.os:44` |  |
...
| 22 | SHOULD | ✅ found | If pre-configured, `Propagator`s SHOULD default to a composite `Propagator` containing the W3C Trace Context Propagator and the Baggage `Propagator` specified in the Baggage API. | `src/Ядро/Модули/ОтелГлобальный.os:132` | OneScript SDK не выполняет автоматическую pre-configuration пропагаторов (платформа не делает это out-of-the-box), поэтому условие 'If pre-configured' не наступает; по умолчанию возвращается no-op. Конфигурация W3C+Baggage доступна через ОтелПостроительSdk.УстановитьПропагаторы(). |
...
| 28 | MUST NOT | ✅ found | It MUST NOT use `OpenTracing` in the resulting propagator name as it is not widely adopted format in the OpenTracing ecosystem. | `src/Пропагация/Классы/ОтелW3CПропагатор.os:1` |  |
| 29 | MUST NOT | ✅ found | Additional `Propagator`s implementing vendor-specific protocols such as AWS X-Ray trace header protocol MUST NOT be maintained or distributed as part of the Core OpenTelemetry repositories. | `src/Пропагация/Классы/ОтелW3CПропагатор.os:1` |  |
...
| 31 | MUST | ✅ found | A W3C Trace Context propagator MUST propagate a valid `traceparent` value using the same header. | `src/Пропагация/Классы/ОтелW3CПропагатор.os:51` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 4 | MUST | ✅ found | The SDK MUST interpret an empty value of an environment variable the same way as when the variable is unset. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:889` |  |
...
| 8 | SHOULD | ❌ not_found | If any value other than a true value, case-insensitive string "false", empty, or unset is used, a warning SHOULD be logged to inform users about the fallback to false being applied. | - | Boolean-парсинг (НРег(Значение) = "true") молча трактует любое не-"true" значение как false без логирования предупреждения для значений, не совпадающих с "false"/empty/unset. |
...
| 13 | SHOULD | ✅ found | For variables accepting a numeric value, if the user provides a value the implementation cannot parse, the implementation SHOULD generate a warning and gracefully ignore the setting, i.e., treat them... | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:909` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 14 | SHOULD | ✅ found | Enum values SHOULD be interpreted in a case-insensitive manner. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:258` |  |
| 15 | MUST | ✅ found | For sources accepting an enum value, if the user provides a value the implementation does not recognize, the implementation MUST generate a warning and gracefully ignore the setting. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:312` |  |
...
| 18 | MUST | ✅ found | Invalid or unrecognized input MUST be logged and MUST be otherwise ignored, i.e. the implementation MUST behave as if OTEL_TRACES_SAMPLER_ARG is not set. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:912` |  |
| 19 | MUST | ✅ found | Invalid or unrecognized input MUST be logged and MUST be otherwise ignored, i.e. the implementation MUST behave as if OTEL_TRACES_SAMPLER_ARG is not set. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:293` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 21 | SHOULD | ✅ found | "logging": Standard Output. It is a deprecated value left for backwards compatibility. It SHOULD NOT be supported by new implementations. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:258` |  |
| 22 | SHOULD | ✅ found | "logging": Standard Output. It is a deprecated value left for backwards compatibility. It SHOULD NOT be supported by new implementations. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:451` |  |
| 23 | SHOULD | ✅ found | "logging": Standard Output. It is a deprecated value left for backwards compatibility. It SHOULD NOT be supported by new implementations. | `src/Конфигурация/Модули/ОтелАвтоконфигурация.os:383` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ➖ n_a | Resource detectors SHOULD have a unique name for reference in configuration. | - | Условная фича 'Resource Detector Naming' не реализована: детекторы (ОтелДетекторРесурсаХоста/Процесса/Процессора) не имеют отдельной строки-имени для declarative configuration. |
| 2 | SHOULD | ➖ n_a | Names SHOULD be snake case and consist of lowercase alphanumeric and `_` characters, which ensures they conform to declarative configuration property name requirements. | - | Условная фича именования детекторов не реализована — формат имён не определён. |
| 3 | SHOULD | ➖ n_a | Resource detector names SHOULD reflect the root namespace of attributes they populate. | - | Условная фича именования детекторов не реализована. |
| 4 | SHOULD | ➖ n_a | Resource detectors which populate attributes from multiple root namespaces SHOULD choose a name which appropriately conveys their purpose. | - | Условная фича именования детекторов не реализована. |
| 5 | SHOULD | ➖ n_a | An SDK which identifies multiple resource detectors with the same name SHOULD report an error. | - | Условная фича именования детекторов не реализована — нет ни имён, ни проверки коллизий. |
| 6 | SHOULD | ➖ n_a | In order to limit collisions, resource detectors SHOULD document their name in a manner which is easily discoverable. | - | Условная фича именования детекторов не реализована — документировать нечего. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ➖ n_a | It SHOULD only be possible to create `Tracer` instances through a `TracerProvider` (see API). | `src/Трассировка/Классы/ОтелТрассировщик.os:282` | OneScript не поддерживает приватные конструкторы (ПриСозданииОбъекта всегда публичен). Конвенциональный путь создания — через ОтелПровайдерТрассировки.ПолучитьТрассировщик / ПостроительТрассировщика; ограничение платформы документировано. |
...
| 3 | MUST | ✅ found | The value of `enabled` MUST be used to resolve whether a `Tracer` is Enabled. | `src/Трассировка/Классы/ОтелТрассировщик.os:47` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ⚠️ partial | Enabled MUST return false when either: there are no registered SpanProcessors, Status: Development - Tracer is disabled (TracerConfig.enabled is false). | `src/Трассировка/Классы/ОтелТрассировщик.os:47` | Включен() проверяет TracerConfig.Включен() и Провайдер.Включен(), но не возвращает Ложь, когда у провайдера нет зарегистрированных процессоров спанов (отсутствует явная проверка пустого списка Процессоры в ОтелПровайдерТрассировки). |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | The `TraceIdRatioBased` MUST ignore the parent `SampledFlag`. | `src/Трассировка/Модули/ОтелСэмплер.os:333` |  |
...
| 3 | SHOULD | ⚠️ partial | The precision of the number SHOULD follow implementation language standards and SHOULD be high enough to identify when Samplers have different ratios. | `src/Трассировка/Модули/ОтелСэмплер.os:125` | Precision жёстко ограничена 6 знаками после запятой (Формат с ЧДЦ=6) вместо использования стандартного для платформы представления Decimal — для очень мелких различий (<1e-6) ratios будут одинаково отображаться. |
| 4 | SHOULD | ⚠️ partial | The precision of the number SHOULD follow implementation language standards and SHOULD be high enough to identify when Samplers have different ratios. | `src/Трассировка/Модули/ОтелСэмплер.os:125` | При фиксированной precision=6 два сэмплера с долями, отличающимися менее чем на 1e-6, дадут одинаковое описание. |
| 5 | MUST | ✅ found | The sampling algorithm MUST be deterministic. | `src/Трассировка/Модули/ОтелСэмплер.os:333` |  |
| 6 | MUST | ✅ found | implementations MUST use a deterministic hash of the `TraceId` when computing the sampling decision. | `src/Трассировка/Модули/ОтелСэмплер.os:346` |  |
| 7 | MUST | ✅ found | A `TraceIdRatioBased` sampler with a given sampling probability MUST also sample all traces that any `TraceIdRatioBased` sampler with a lower sampling probability would sample. | `src/Трассировка/Модули/ОтелСэмплер.os:348` |  |
| 8 | SHOULD | ❌ not_found | When this sampler observes a non-empty parent span context, meaning when it is used not as a root sampler, the SDK SHOULD emit a warning such as: | - | Семплер ПоДолеТрассировок не различает root/child использование и не пишет в лог предупреждение о том, что он используется как child sampler. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ❌ not_found | The `ProbabilitySampler` sampler MUST ignore the parent `SampledFlag`. | - | Сэмплер ProbabilitySampler (composable-форма с использованием W3C Trace Context Level 2 randomness и threshold) не реализован в SDK. |
| 2 | SHOULD | ❌ not_found | When (R >= T), the OpenTelemetry TraceState SHOULD be modified to include the key-value `th:T` for rejection threshold value (T), as specified for the OpenTelemetry TraceState `th` sub-key. | - | ProbabilitySampler не реализован, поэтому модификация TraceState с ключом th:T не выполняется (хотя класс ОтелСостояниеТрассировки распознаёт sub-key th — он используется только при парсинге). |
| 3 | SHOULD | ❌ not_found | When a ProbabilitySampler Sampler makes a decision for a non-root Span using TraceID randomness when the Trace random flag was not set, the SDK SHOULD issue a warning statement in its log with a compatibility warning. | - | ProbabilitySampler не реализован; предупреждение про отсутствие Trace random flag не выводится. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST NOT | ❌ not_found | Note: ComposableSamplers MUST NOT modify the parameters passed to delegate GetSamplingIntent methods, as they are considered read-only state. | - | Интерфейс ComposableSampler и метод GetSamplingIntent не реализованы — нет ни класса, ни цепочки делегирования через SamplingIntent. |
| 2 | MUST NOT | ❌ not_found | ComposableSamplers MUST NOT modify the OpenTelemetry TraceState (i.e., the `ot` sub-key of TraceState). | - | ComposableSampler отсутствует; нет соответствующего ограничения и тестов на неизменяемость ot sub-key из делегата. |
| 3 | SHOULD | ❌ not_found | The calling CompositeSampler SHOULD update the threshold of the outgoing TraceState (unless `!threshold_reliable`) and that the explicit randomness values MUST not be modified. | - | CompositeSampler не реализован; обновление threshold (sub-key th) исходящего TraceState на основе результата ComposableSampler отсутствует. |
| 4 | MUST | ❌ not_found | The calling CompositeSampler SHOULD update the threshold of the outgoing TraceState (unless `!threshold_reliable`) and that the explicit randomness values MUST not be modified. | - | CompositeSampler/ComposableSampler не реализованы — нет логики, которая обрабатывает explicit randomness ('rv') и могла бы её сохранять/изменять. |
| 5 | SHOULD | ❌ not_found | For the zero case a `ComposableAlwaysOff` instance SHOULD be returned instead. | - | ComposableProbability и ComposableAlwaysOff не реализованы; обработки нулевой ratio с возвратом ComposableAlwaysOff нет. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | The end timestamp MUST have been computed (the `OnEnding` method duration is not included in the span duration). | `src/Трассировка/Классы/ОтелСпан.os:486` |  |
| 2 | MUST | ✅ found | The Span object MUST still be mutable (i.e., `SetAttribute`, `AddLink`, `AddEvent` can be called) while `OnEnding` is called. | `src/Трассировка/Классы/ОтелСпан.os:492` |  |
| 3 | MUST | ✅ found | This method MUST be called synchronously within the `Span.End()` API, therefore it should not block or throw an exception. | `src/Трассировка/Классы/ОтелСпан.os:491` |  |
| 4 | MUST | ✅ found | The SDK MUST guarantee that the span can no longer be modified by any other thread before invoking `OnEnding` of the first `SpanProcessor`. | `src/Трассировка/Классы/ОтелСпан.os:484` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ❌ not_found | The ergonomic API SHOULD make it more convenient to emit event records following the event semantics. | - | В SDK реализован только базовый API эмиссии (ОтелЛоггер.Записать(ЗаписьЛога)). Отдельного эргономичного API для эмиссии event records (например, удобных методов уровня Информация/Ошибка/Событие) нет. |
| 2 | SHOULD | ❌ not_found | The design of the ergonomic API SHOULD be idiomatic for its language. | - | Эргономичный API не реализован, поэтому требование к его идиоматичному дизайну не применимо к существующему коду. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ➖ n_a | It SHOULD only be possible to create `Logger` instances through a `LoggerProvider` (see API). | - | OneScript не поддерживает приватные/internal конструкторы (`ПриСозданииОбъекта` всегда публичен). Идиоматическая модель создания логгеров - через ОтелПровайдерЛогирования.ПолучитьЛоггер()/ПостроительЛоггера; ограничение задокументировано в коде и docs/spec-compliance.md. |
...
| 4 | MUST | ✅ found | In the case where an invalid `name` (null or empty string) is specified, a working `Logger` MUST be returned as a fallback rather than returning null or throwing an exce... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:61` |  |
| 5 | SHOULD | ✅ found | In the case where an invalid `name` (null or empty string) is specified, a working `Logger` MUST be returned as a fallback rather than returning null or throwing an exce... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:67` |  |
| 6 | SHOULD | ✅ found | In the case where an invalid `name` (null or empty string) is specified, a working `Logger` MUST be returned as a fallback rather than returning null or throwing an exce... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:62` |  |
| 7 | MUST | ✅ found | Status: Development - The `LoggerProvider` MUST compute the relevant LoggerConfig using the configured LoggerConfigurator, and create a `Logger` whose behavior conforms ... | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:77` |  |
...
| 2 | MUST | ✅ found | If the LoggerProvider supports updating the LoggerConfigurator, then upon update the Logger MUST be updated to behave according to the new LoggerConfig. | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:178` |  |
...
| 4 | MUST | ✅ found | If a log record's SeverityNumber is specified (i.e. not 0) and is less than the configured minimum_severity, the log record MUST be dropped by the Logger. | `src/Логирование/Классы/ОтелЛоггер.os:181` |  |
...
| 8 | MUST | ✅ found | However, the changes MUST be eventually visible. | `src/Логирование/Классы/ОтелПровайдерЛогирования.os:251` |  |
...
| 5 | MUST | ✅ found | Before processing a log record, the implementation MUST apply the filtering rules defined by the LoggerConfig (in case Enabled was not called prior to emitting the rec... | `src/Логирование/Классы/ОтелЛоггер.os:112` |  |
| 6 | MUST | ✅ found | Minimum severity: If the log record's SeverityNumber is specified (i.e. not 0) and is less than the configured minimum_severity, the log record MUST be dropped. | `src/Логирование/Классы/ОтелЛоггер.os:181` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | Enabled MUST return false when either: there are no registered LogRecordProcessors, Logger is disabled, the provided severity is less than minimum_severity, trace_based and unsampled, or all processors return false. | `src/Логирование/Классы/ОтелЛоггер.os:49` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ❌ not_found | This processor SHOULD be provided by SDK. | - | В подсистеме логирования отсутствует процессор-мост Event-to-span event. В src/Логирование/Классы/ нет реализации, конвертирующей события LogRecord в SpanEvent на текущем спане. |
| 2 | MUST | ❌ not_found | The processor MUST bridge a `LogRecord` to a span event if and only if all of the following conditions are met: | - | Процессор-мост Event-to-span event не реализован. |
| 3 | MUST | ❌ not_found | If any of these conditions is not met, the processor MUST do nothing. | - | Процессор-мост Event-to-span event не реализован. |
| 4 | MUST | ❌ not_found | When a `LogRecord` is bridged, the processor MUST add exactly one span event with the following mapping: | - | Процессор-мост Event-to-span event не реализован. |
| 5 | MUST | ❌ not_found | the span event name MUST be the `LogRecord`’s Event Name. | - | Процессор-мост Event-to-span event не реализован. |
| 6 | MUST | ❌ not_found | if the `LogRecord` has a Timestamp set, it MUST be used as the span event timestamp. | - | Процессор-мост Event-to-span event не реализован. |
| 7 | MUST | ❌ not_found | Otherwise, if the `LogRecord` has an ObservedTimestamp set, it MUST be used as the span event timestamp. | - | Процессор-мост Event-to-span event не реализован. |
| 8 | MUST | ❌ not_found | all `LogRecord` Attributes MUST be copied to the span event as span event attributes. | - | Процессор-мост Event-to-span event не реализован. |
| 9 | MUST NOT | ❌ not_found | Note that bridging a `LogRecord` to a span event MUST NOT prevent that `LogRecord` from continuing through the normal log processing pipeline. | - | Процессор-мост Event-to-span event не реализован, поэтому требование о непрерывности конвейера обработки логов также не покрыто. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | OpenTelemetry SDKs MUST handle `advisory` parameters as described here. | `src/Метрики/Классы/ОтелМетр.os:853` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ➖ n_a | It SHOULD only be possible to create `Meter` instances through a `MeterProvider` (see API). | `src/Метрики/Классы/ОтелМетр.os:650` | OneScript не поддерживает приватные конструкторы (ПриСозданииОбъекта всегда публичен). Документированный путь создания — через ОтелПровайдерМетрик.ПолучитьМетр() / ПостроительМетра(). |
...
| 4 | MUST | ✅ found | In the case where an invalid `name` (null or empty string) is specified, a working Meter MUST be returned as a fallback rather than returning null or throwing an exception, its `name` SHOULD keep ... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:79` |  |
| 5 | SHOULD | ✅ found | In the case where an invalid `name` (null or empty string) is specified, a working Meter MUST be returned as a fallback rather than returning null or throwing an exception, its `name` SHOULD keep ... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:85` |  |
| 6 | SHOULD | ✅ found | In the case where an invalid `name` (null or empty string) is specified, a working Meter MUST be returned as a fallback rather than returning null or throwing an exception, its `name` SHOULD keep ... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:80` |  |
| 7 | MUST | ✅ found | The `MeterProvider` MUST compute the relevant MeterConfig using the configured MeterConfigurator, and create a `Meter` whose behavior conforms to that `MeterConfig`. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:297` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | The function MUST accept the following parameter: * `meter_scope`: The `InstrumentationScope` of the `Meter`. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:301` |  |
...
| 2 | MUST | ✅ found | This implies that all data points with delta temporality aggregation for an instrument MUST share the same start timestamp. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:356` |  |
...
| 4 | SHOULD | ⚠️ partial | For synchronous instruments, the start timestamp SHOULD be the time of the first measurement for the series. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:328` | ВремяСтарта ставится в момент создания инструмента (ПриСозданииОбъекта), а не при первом измерении серии. Для большинства случаев совпадает, но строго не соответствует «time of the first measurement». |
| 5 | SHOULD | ⚠️ partial | For asynchronous instrument, the start timestamp SHOULD be: - The creation time of the instrument, if the first series measurement occurred in the first collection interval, - Otherwise, the timestamp ... | `src/Метрики/Классы/ОтелБазовыйНаблюдаемыйИнструмент.os:217` | Для асинхронных инструментов startTimeUnixNano каждый раз ставится равным ВремяСейчас (время сбора), а не времени создания инструмента или предыдущего интервала. Логика по спецификации не реализована. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ✅ found | Distinct meters MUST be treated as separate namespaces for the purposes of detecting duplicate instrument registrations. | `src/Метрики/Классы/ОтелМетр.os:656` |  |
| 2 | MUST | ✅ found | `Meter` MUST behave according to the MeterConfig computed during Meter creation. | `src/Метрики/Классы/ОтелПровайдерМетрик.os:298` |  |
...
| 2 | MUST | ⚠️ partial | If a `Meter` is disabled, it MUST behave equivalently to No-op Meter. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:88` | Синхронные инструменты при МетрВключен=Ложь возвращают управление без записи; но ВызватьCallbackИСобрать() в ОтелБазовыйНаблюдаемыйИнструмент не проверяет МетрВключен, поэтому асинхронные callback продолжают вызываться и формировать точки данных. |
| 3 | MUST | ⚠️ partial | The value of `enabled` MUST be used to resolve whether an instrument is Enabled. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:259` | Включен(Атрибуты) = МетрВключен.Получить() И Включен.Получить() реализован для синхронных инструментов; для наблюдаемых (ОтелБазовыйНаблюдаемыйИнструмент) метод Включен() и проверка MeterConfig.enabled отсутствуют. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ⚠️ partial | The synchronous instrument `Enabled` MUST return `false` when either: * Status: Development - The MeterConfig of the `Meter` used to create the instrument has parameter `enabled=fal... | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:259` | Включен() возвращает Ложь, когда MeterConfig.enabled=false (через МетрВключен), но не проверяет случай, когда все resolved views настроены с Drop Aggregation - в этом случае инструмент остаётся 'включённым', хотя по спецификации MUST вернуть false. |
| 2 | SHOULD | ✅ found | Otherwise, it SHOULD return `true`. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:260` |  |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | SHOULD | ⚠️ partial | To construct a `MetricReader` when setting up an SDK, at least the following SHOULD be provided: * The `exporter` to use, which is a `MetricExporter` instance. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:495` | Конструктор принимает Экспортер, ИнтервалЭкспортаМс, ЛимитМощности, агрегацию гистограмм. MetricFilter и MetricProducer добавляются отдельными методами, а селектор aggregation/temporality не передаётся в конструктор как функция от вида инструмента. |
| 2 | SHOULD | ⚠️ partial | This function SHOULD be obtained from the `exporter`. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:514` | Aggregation-функция инициализируется внутри читателя (ИнициализироватьСелекторАгрегации), не запрашивается у экспортера. |
| 3 | SHOULD | ✅ found | If not configured, the default aggregation SHOULD be used. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:188` |  |
...
| 5 | SHOULD | ✅ found | If not configured, the Cumulative temporality SHOULD be used. | `src/Метрики/Модули/ОтелСелекторВременнойАгрегации.os:1` |  |
...
| 8 | SHOULD | ✅ found | A common implementation of `MetricReader`, the periodic exporting `MetricReader` SHOULD be provided to be used typically with push-based metrics collection. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:481` |  |
...
| 10 | MUST | ✅ found | For synchronous instruments with Cumulative aggregation temporality, MetricReader.Collect MUST receive data points exposed in previous collections regardless of whether new measurements have been reco... | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:170` |  |
...
| 15 | MUST | ✅ found | The ending timestamp (i.e. `TimeUnixNano`) MUST always be equal to time the metric data point took effect, which is equal to when MetricReader.Collect was invoked. | `src/Метрики/Классы/ОтелБазовыйСинхронныйИнструмент.os:341` |  |
| 16 | MUST | ✅ found | The SDK MUST support multiple `MetricReader` instances to be registered on the same `MeterProvider`, and the MetricReader.Collect invocation on one `MetricReader` instance SHOULD NOT introduce side-ef... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:355` |  |
| 17 | SHOULD NOT | ⚠️ partial | The SDK MUST support multiple `MetricReader` instances to be registered on the same `MeterProvider`, and the MetricReader.Collect invocation on one `MetricReader` instance SHOULD NOT introduce side-ef... | `src/Метрики/Классы/ОтелПровайдерМетрик.os:163` | Реализована логика «все, кроме последнего, экспортируют без очистки» (СброситьБуферБезОчистки) и per-reader КумулятивноеСостояниеАсинх. Однако базовые аккумуляторы инструмента остаются общими, и сброс ВремяСтарта/Аккумуляторов одним последним читателем затрагивает временной диапазон других читателей — изоляция неполная. |
...
| # | Уровень | Статус | Требование | Расположение в коде | Пояснение |
|---|---|---|---|---|---|
| 1 | MUST | ❌ not_found | When `maxExportBatchSize` is configured, the reader MUST ensure no batch provided to `Export` exceeds the `maxExportBatchSize` by splitting the batch of metric data points into smaller batches. | - | Параметр maxExportBatchSize не реализован в ОтелПериодическийЧитательМетрик; разбиение батча отсутствует. |
| 2 | MUST | ❌ not_found | The initial batch of metric data MUST be split into as many “full” batches of size `maxExportBatchSize` as possible – even if this splits up data points that belong to the same metric into different batches. | - | Логика дробления на полные батчи отсутствует — maxExportBatchSize не поддерживается. |
...
| 4 | MUST NOT | ✅ found | The reader MUST NOT combine metrics from different `Collect()` calls into the same batch provided to `Export`. | `src/Метрики/Классы/ОтелПериодическийЧитательМетрик.os:224` |  |
...
| 2 | SHOULD | ✅ found | Implementation SHOULD use the filter as early as possible to gain as much performance gain possible (memory allocation, internal metric fetching, etc). | `src/Метрики/Классы/ИнтерфейсПродюсерМетрик.os:13` |  |
| 3 | SHOULD | ⚠️ partial | If the batch of Metric Points includes resource information, `Produce` SHOULD require a resource as a parameter. | `src/Метрики/Классы/ИнтерфейсПродюсерМетрик.os:13` | Произвести() не принимает Ресурс в качестве параметра — ресурс берётся из ОтелДанныеМетрики, что не соответствует архитектуре «resource as parameter». |
| 4 | SHOULD | ⚠️ partial | `Produce` SHOULD provide a way to let the caller know whether it succeeded, failed or timed out. | `src/Метрики/Классы/ИнтерфейсПродюсерМетрик.os:13` | Произвести() возвращает Массив; нет отдельного статуса успех/ошибка/таймаут и нет списка failedReasons. |
