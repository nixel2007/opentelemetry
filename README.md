...
Позволяет собирать и отправлять телеметрию (трассировку, логи, метрики) в формате OTLP в любой совместимый коллектор - [Grafana LGTM](https://grafana.com/oss/), [OpenTelemetry Collector](https://opentelemetry.io/docs/collector/) и другие. Поддерживает три транспорта: HTTP JSON, HTTP Protobuf и gRPC.
...
- **Экспорт по OTLP/HTTP** - отправка данных в формате JSON или Protobuf на эндпоинты `/v1/traces`, `/v1/logs`, `/v1/metrics`
...
### Использование HTTP/protobuf-транспорта

HTTP-транспорт поддерживает два протокола. По умолчанию используется `http/protobuf` (бинарный Protobuf). Для переключения на JSON задайте протокол явно:

```bsl
#Использовать opentelemetry

// HTTP/protobuf - протокол по умолчанию (порт 4318)
Транспорт = Новый ОтелHttpТранспорт("http://localhost:4318");
...
Все экспортеры работают со всеми тремя транспортами. Достаточно заменить `ОтелHttpТранспорт` на `ОтелGrpcТранспорт`:
...
| **NonRecordingSpan** | `ОтелНезаписывающийСпан` / `ОтелСпаны` | Спан-заглушка (no-op). Создавать через `ОтелСпаны.Обернуть(КонтекстСпана)` или `ОтелСпаны.Невалидный()` |
...
|-----------|-------|----------|
| **HTTP-транспорт** | `ОтелHttpТранспорт` | OTLP HTTP (порт 4318); протокол `http/protobuf` (по умолчанию) или `http/json` |
...
| **Proto-кодировщик трассировки** | `ОтелПротоКодировщикTrace` | Бинарная сериализация спанов для HTTP/protobuf |
| **Proto-кодировщик метрик** | `ОтелПротоКодировщикMetrics` | Бинарная сериализация метрик для HTTP/protobuf |
| **Proto-кодировщик логов** | `ОтелПротоКодировщикLogs` | Бинарная сериализация логов для HTTP/protobuf |
| **Конвертер** | `ОтелКонвертерOtlpВProto` | Конвертация camelCase -> snake_case для gRPC |
