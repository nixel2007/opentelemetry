...
def generate_markdown(merged, sections, sections_index, stats, warnings):
    ...
    lines.append(
        "| Нет hard-cancel фоновых заданий | У `ФоновоеЗадание` только "
        "`ОжидатьЗавершения(Таймаут)`, нет `Прервать()`/`ОтменитьЗадание()` "
        "([OneScript#1672](https://github.com/EvilBeaver/OneScript/issues/1672)) "
        "| Soft-timeout: перестаём ждать и отбрасываем результат, задание продолжает "
        "работу в фоне; остановка фоновых циклов кооперативная, через атомарный флаг |"
    )
    lines.append(
