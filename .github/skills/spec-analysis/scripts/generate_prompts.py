...
**Ошибка 11: "Детекторы ресурсов внутри SDK - partial"**
Спецификация: "Custom resource detectors related to generic platforms (e.g. Docker, Kubernetes) MUST be implemented as packages separate from the SDK"
Код: SDK содержит только generic детекторы (host, process, CPU), НЕ содержит cloud-specific (Docker, K8s, AWS, GCP)
❌ Неправильно: `partial` - "детекторы внутри SDK, а не в отдельных пакетах"
✅ Правильно: `found` - "требование относится к cloud/vendor-specific детекторам, которых в SDK нет. Generic детекторы (host, process) допустимы внутри SDK"

**Ошибка 12: "Не нашёл метод обновления конфигуратора - not_found"**
Спецификация: "If the MeterProvider supports updating the MeterConfigurator, then upon update the Meter MUST be updated"
Код: `ОтелПровайдерМетрик.ОбновитьКонфигуратор(НовыйКонфигуратор)` вызывает `ПрименитьКонфигурацииКоВсемМетрам()`
❌ Неправильно: `not_found` - "MeterProvider не поддерживает обновление MeterConfigurator"
✅ Правильно: `found` - "метод ОбновитьКонфигуратор() обновляет конфигуратор и применяет ко всем метрам"

**Ошибка 13: "Delta start timestamp не сбрасывается - partial"**
Спецификация: "For delta aggregations, the start timestamp MUST equal the previous collection interval's timestamp"
Код: `ОчиститьТочкиДанных(Временность)` при delta сбрасывает `ВремяСтарта = ОтелУтилиты.ТекущееВремяВНаносекундах()`; при cumulative сохраняет исходное
❌ Неправильно: `partial` - "Delta temporality start timestamp consistency not enforced"
✅ Правильно: `found` - "ОчиститьТочкиДанных при delta обновляет ВремяСтарта на текущее время (= конец предыдущего интервала)"

**Ошибка 14: "OTLP gRPC не поддерживает схемы dns:// / unix:// / host:port - partial"**
Спецификация: "OTLP/gRPC SHOULD support the following schemes: http, https, dns, unix..." и "host:port SHOULD be supported as a shortcut"
Код: `ОтелGrpcТранспорт.ОткрытьСоединение()` передаёт endpoint в OPI_GRPC как есть, без специальной обработки схем dns://, unix://, ipv4:, ipv6:, host:port-без-схемы. В комментариях указано: используемая библиотека OPI_GRPC построена на Rust-клиенте tonic, который принимает endpoint **только** в виде http://... или https://... (схема обязательна), DNS резолвится средствами рантайма, unix-сокеты не поддерживаются.
❌ Неправильно: `partial` - "не поддерживаются схемы dns://, unix://, host:port"
✅ Правильно: `found` - "поддержка ограничена возможностями нижележащего gRPC-клиента (tonic в OPI_GRPC принимает только http/https со схемой); это документированное ограничение транспорта, спецификация формулирует требование как SHOULD"
> Если в коде есть комментарий «OPI_GRPC (tonic-based)» / «схема обязательна» рядом с местом передачи endpoint - это явное указание на это ограничение.

**Ошибка 15: "B3 пропагатор отсутствует в основном пакете - not_found"**
Спецификация: "Implementations MUST distribute the B3 propagator as a separate plugin/extension package"
Код: класса `ОтелB3Пропагатор` нет в `src/Пропагация/Классы/`; в `lib.config` его нет; `ОтелАвтоконфигурация.СоздатьПропагаторы()` распознаёт значения `b3`/`b3multi` и пытается создать класс через `Новый("ОтелB3Пропагатор", ...)` (рефлексивно, с graceful warning при отсутствии).
❌ Неправильно: `not_found` - "В основной библиотеке нет реализации B3"
✅ Правильно: `found` - "B3 распространяется отдельным пакетом opentelemetry-propagator-b3 (как требует спека); основной SDK подгружает его рефлексивно через ОтелАвтоконфигурация.СоздатьПропагаторB3() при наличии в окружении"
...
**Архитектурная особенность: API и SDK объединены в одном пакете:**
- В данной реализации API и SDK **не разделены** на отдельные компоненты — весь код находится в одном пакете.
- Это означает, что методы вроде `ОтелСчетчик.Добавить`, `ОтелРеверсивныйСчетчик.Добавить` и т.п. одновременно являются и API-точкой входа, и SDK-реализацией.
- Требование спецификации **"API SHOULD NOT validate this value"** направлено против валидации в **чистом API-прокси** (без SDK). В данной архитектуре проверка значения в `Добавить` — это **SDK-уровневая защита**, а не нарушение API.
- Если видишь валидацию в методе инструмента с мягкой реакцией (warning + ignore, не exception) — ставь `found`, а не `partial`.
- Это распространяется на: `Counter.Add` (отклонение отрицательных значений), `advisory` параметры (`ПроверитьСовет()`), любую входную валидацию инструментов метрик. SDK-спецификация **явно требует** эту валидацию; наличие её в объединённом API+SDK → `found`.
...
- Приватные конструкторы (OneScript: `ПриСозданииОбъекта` всегда публичен). Требования вида "Span MUST NOT be created directly" / "API MUST NOT provide ways to create Span other than via Tracer" → `n_a` с пояснением «OneScript не поддерживает приватные конструкторы; ограничение документировано в коде (ОтелСпан.os) и docs/spec-compliance.md»
- Модификаторы доступа на уровне методов (private/internal/protected). OneScript поддерживает только бинарное `Экспорт` / не-`Экспорт`; не-`Экспорт` метод недоступен из любого другого класса, включая SDK-инфраструктуру (процессоры, экспортёры, ридеры). Требования вида «SHOULD NOT provide access to Span's attributes besides SpanContext», «SHOULD NOT expose internal state», «attributes/events/links MUST NOT be readable by user code» → `n_a` с пояснением «OneScript не поддерживает internal/package-private модификаторы; SDK-геттеры (Атрибуты(), События(), Линки() и т.п.) обязаны быть Экспорт, иначе процессоры/экспортёры не смогут читать данные спана. Аналогично приватным конструкторам.»
- Hard cancel фоновых заданий (OneScript `ФоновоеЗадание` имеет только `ОжидатьЗавершения(timeout)`, нет `Прервать()`/`ОтменитьЗадание()` — см. https://github.com/EvilBeaver/OneScript/issues/1672). Требования вида «timeout MUST/SHOULD prevent indefinite execution» реализуются как **soft-timeout** (перестать ждать + отбросить результат), задание продолжит работу в фоне. Это `partial` с пояснением, не `n_a`.

> **ВАЖНО про concurrency**: ФоновыеЗадания (background jobs) в OneScript **имеют общую память** — объекты передаются по ссылке, состояние разделяется между основным потоком и фоновыми заданиями. Поэтому требования вида «MUST be safe to be called concurrently», «thread-safe», «MUST NOT be modified concurrently» **применимы** и проверяются по коду: ищи `БлокировкаРесурса`, `АтомарноеБулево`, `АтомарноеЦелое`, иные атомарные/lock-примитивы или explicit synchronization. **НЕ ставь n_a** на concurrency-требования со ссылкой на ФоновыеЗадания — это ошибка. Если защита есть → `found`; если её нет → `partial`/`not_found`.

**Требования к Instrumentation Libraries** — данный проект реализует только API+SDK, Instrumentation Libraries в составе отсутствуют. Требования, явно адресованные Instrumentation Libraries (субъект — «Instrumentation Libraries SHOULD/MUST/SHOULD NOT ...»), → `n_a` с пояснением: «Требование адресовано Instrumentation Libraries (политика их поведения); данный пакет реализует только API+SDK, IL не включены». Примеры таких требований из Trace API: строки 82–85 (описание Description, публикация конвенций, SetStatus(Ok), leave status Unset). Это НЕ относится к требованиям, где IL упоминается в контексте («when IL calls SetStatus...»), но субъект — сам SDK.

**Требования к официальному списку пропагаторов OTel-организации** — требования вида «The official list of propagators that MUST be maintained by the OpenTelemetry organization and MUST be distributed as OpenTelemetry extension packages» адресованы **организации OpenTelemetry**, а не отдельным SDK-имплементациям. Данный пакет является независимой реализацией, а не официальным дистрибутивом OTel. → `n_a` с пояснением: «Требование адресовано OpenTelemetry Organization (официальный реестр пропагаторов); данный пакет является независимой SDK-реализацией, не официальным дистрибутивом OTel». Аналогично для связанных требований вида «Fields MUST return header names» для B3 — если B3 пропагатор отсутствует в репозитории (тест: grep -r "B3" src/), ставь `n_a`.

**Рекомендации по использованию для вызывающих** — требования вида «`ForceFlush` SHOULD only be called in cases where it is absolutely necessary...» — это **рекомендация для пользователей SDK** (caller guidance), а не требование к реализации SDK. SDK не может программно ограничить, в каких случаях вызывается ForceFlush. → `n_a` с пояснением: «Требование является рекомендацией по использованию для вызывающих кода (caller guidance); SDK не может программно обеспечить это ограничение».
