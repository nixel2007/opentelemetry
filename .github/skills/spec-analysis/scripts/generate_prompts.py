...
- `ОтелЛимитыСпана` = SpanLimits, `МаксСобытий` = EventCountLimit, `МаксЛинков` = LinkCountLimit
- `СгенерироватьTraceId` = GenerateTraceId, `СгенерироватьSpanId` = GenerateSpanId
- Если спецификация требует "class SHOULD be named X" - проверяй по **семантике**, а не по английскому написанию
- Русское имя, **семантически совпадающее** с требуемым английским → `found`, НЕ `partial`
